...
import { Heart, Scale, Search, ShoppingCart, Leaf, Sparkles, View, Star, Share2, Bot, Bell, X, Minus, Plus, PlusCircle, Repeat, Edit2, AlertCircle, Clock, Check, Droplets, Info, Truck, Undo2, Shirt, PlayCircle, Instagram, Send, Youtube, Calendar, Users, Percent, Newspaper, Building, Briefcase, Handshake, Mail, MapPin, Globe, Phone, Palette, Trello } from 'lucide-react';
import { useUIState } from '@/providers/ui-state';
import type { Product, WishlistCollection, ColorInfo, SizeAvailabilityStatus, ProductReview, CartItem, Brand } from '@/lib/types';
...
const GridPreview = ({ settings, product, onQuickViewClick }: { settings: Record<string, boolean>, product: any, onQuickViewClick: () => void }) => {
...
const ListPreview = ({ settings, product }: { settings: Record<string, boolean>, product: any }) => {
    return (
...
const DetailPreview = forwardRef<HTMLDivElement, { settings: Record<string, boolean>, product: any }>(({ settings, product: mockProduct }, ref) => {
    ...
    const product: Product = {
      id: '1', slug: 'mock-product', name: mockProduct.name, brand: mockProduct.brand, price: 24500, originalPrice: 35000, outlet: true, description: mockProduct.description, images: mockProduct.images, category: 'Трикотаж', subcategory: 'Свитера', sku: mockProduct.sku, color: 'mock', season: 'mock', sustainability: ['Переработанные материалы'],
    ...
    const activeColor = React.useMemo(() => {
        const selection = activeColorSelection?.productId === product.id ? activeColorSelection.colorName : undefined;
    ...
    const imagesForCurrentColor = React.useMemo(() => {
        ...
        return colorImages.length > 0 ? colorImages.map((img:any) => ({...img, url: img.url, alt: img.alt})) : mockProduct.images.map((img: any) => ({...img, url: img.url, alt: img.alt}));
    ...
    const [notifyDialogPreOrderDate, setNotifyDialogPreOrderDate] = useState<string | undefined>(undefined);
    ...
    const isInWishlist = React.useMemo(() => wishlist.some(item => item.id === product.id), [product.id, wishlist]);
    ...
    const handleSelectSize = (sizeName: string) => {
        ...
        const isNowAvailable = availableSubscriptions.some(s => s.productId === product.id && s.size === sizeName);
        if (isNowAvailable) {
            removeAvailableSubscription(product.id, sizeName);
            toast({ title: "Уведомление просмотрено", description: `Размер ${sizeName} снова в наличии!`});
        }

        const isNewlyAvailable = newlyAvailableSizes.some(s => s.productId === product.id && s.size === sizeName);
        const sizeInfo = activeColor?.sizeAvailability?.find(s => s.size === sizeName);
        const isAvailable = (sizeInfo?.status === 'in_stock' && (sizeInfo.quantity || 0) > 0) || isNewlyAvailable;
        ...
        const isSubscribed = subscribedSizes.some(s => s.productId === product.id && s.size === sizeName);
        if (isSubscribed) {
    ...
    const composition = React.useMemo(() => parseComposition(product.composition), [product.composition]);

    const details = [
        { label: 'Артикул', value: product.sku, condition: settings.sku },
        { label: 'Сезон', value: product.season, condition: true },
        { label: 'Состав', value: composition.map(c => `${c.material} ${c.percentage}%`).join(', ') || 'не указан', condition: settings.composition },
    ...
    return (
        <>
            <div className="grid md:grid-cols-2 gap-3 lg:gap-3 p-4 md:p-4" ref={ref}>
                {/* Image Gallery */}
                {settings.image && (
                    ...
                    <div className="relative aspect-[4/5] w-full rounded-lg overflow-hidden border cursor-pointer" onClick={() => handleOpenImageViewer(imagesForCurrentColor.findIndex(img => img.id === activeImage.id))}>
                        ...
                        <div className="absolute top-3 left-3 flex items-center gap-2">
                            ...
                            {settings.discount_badge && product.outlet && product.originalPrice && (
                                <Badge variant="destructive">-{((product.originalPrice - product.price) / product.originalPrice * 100).toFixed(0)}%</Badge>
                            )}
                        </div>
                        {settings.bestseller_rank && product.bestsellerRank && (
                            ...
                            onClick={(e) => { e.stopPropagation(); setIsBestsellerOpen(true); }}
                            >
                    ...
                    <div className="grid grid-cols-4 gap-3">
                        ...
                        <div
                            key={image.id}
                            className={cn(
                            "relative aspect-square w-full rounded-md overflow-hidden cursor-pointer border-2",
                 ...
                 {/* Product Info */}
                <div className="py-4">
                    {settings.brand && <p className="text-muted-foreground font-medium">{product.brand}</p>}
                    {settings.product_name && <h1 className="text-base lg:text-sm font-headline font-bold mt-1">{product.name}</h1>}
                    
                    {settings.rating && (
                    <div className="mt-4 flex items-center gap-2">
                        <div className="flex items-center">
                            {[...Array(5)].map((_, i) => <Star key={i} className={cn("h-5 w-5", i < 4 ? 'text-yellow-400 fill-yellow-400' : 'text-muted-foreground/30')} />)}
                        </div>
                        <button className="text-sm text-muted-foreground hover:text-primary underline" onClick={() => setIsReviewsOpen(true)}>
                    ...
                    <div className="mt-4 space-y-2">
                        <div className="flex items-center gap-2">
                            {settings.price && (
                                <TooltipProvider>
                                    <Tooltip>
                                    <TooltipTrigger asChild>
                                        <p className="text-base font-bold text-primary cursor-pointer">{product.price.toLocaleString('ru-RU')} ₽</p>
                                    </TooltipTrigger>
                                    {cashbackAmount > 0 && (
                                        <TooltipContent>
                            ...
                            {settings.old_price && product.outlet && product.originalPrice && (
                                <p className="text-base text-muted-foreground line-through">{product.originalPrice.toLocaleString('ru-RU')} ₽</p>
                            )}
                        </div>
                        ...
                        {priceWithBonuses !== null && (
                            <TooltipProvider>
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <div className="text-sm text-green-600 font-semibold flex items-center gap-1.5 cursor-pointer">
                                            <span>
                                            ...
                                            <button onClick={handleTogglePriceDisplay} className="text-muted-foreground hover:text-primary"><Repeat className="h-4 w-4"/></button>
                                        </div>
                                    </TooltipTrigger>
                                    <TooltipContent>
                                        <p>
                    ...
                    {settings.color_swatches && product.availableColors && product.availableColors.length > 0 && (
                        <div className="mb-6">
                            <h3 className="text-sm font-medium mb-2 text-muted-foreground">Цвет: <span className="font-semibold text-foreground">{activeColor?.name}</span></h3>
                            <div className="flex flex-wrap gap-2">
                                {product.availableColors.map(color => (
                                    <TooltipProvider key={color.id}>
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <button 
                                                    className={cn("h-8 w-8 rounded-full border-2", activeColor?.id === color.id ? 'border-primary ring-2 ring-offset-2 ring-primary' : 'border-border/50')}
                                                    style={{ backgroundColor: color.hex }}
                                                    onClick={() => setActiveColorSelection({productId: product.id, colorName: color.name})}
                    ...
                    {settings.size_selector && allSizes.length > 0 && (
                        <div className="mb-6">
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <Button variant="link" size="sm" className="text-xs p-0 h-auto" onClick={() => setIsSizeGuideOpen(true)}>
                                    AI Гид по размерам
                                </Button>
                            </div>
                            <TooltipProvider>
                                    ...
                                    {allSizes.map((sizeName: string) => {
                                        ...
                                        <Tooltip key={sizeName}>
                                            <TooltipTrigger asChild>
                                                ...
                                                className={cn(
                                                    "w-12 h-10 relative", 
                                                    !isAvailable && !cartItem && !isNowAvailable && !isPreOrder && product.availability !== 'coming_soon' && "text-muted-foreground/50 line-through decoration-muted-foreground/50",
                                                    isSubscribed && 'bg-green-100 text-green-800 border-green-300 hover:bg-green-200',
                                                    isNowAvailable && !isSelected && 'bg-blue-100 text-blue-800 border-blue-300 animate-pulse',
                                                )}
                                                onClick={() => handleSelectSize(sizeName)}
                                                disabled={!isAvailable && !cartItem && !isNowAvailable && !isPreOrder && product.availability !== 'coming_soon'}
                                                >
                                                {cartItem && (
                                                    <div className="absolute -top-1.5 -right-1.5 flex items-center justify-center h-5 w-5 bg-primary text-primary-foreground rounded-full text-xs">
                                                    {cartItem.quantity}
                                                    </div>
                                                )}
                                                {isSubscribed && <Bell className="absolute -top-1.5 -right-1.5 h-4 w-4 text-green-700" />}
                                                {isNowAvailable && !isSelected && <Check className="absolute -top-1.5 -right-1.5 h-4 w-4 text-blue-700" />}
                                                {sizeName}
                                                </Button>
                                            </TooltipTrigger>
                                            {!isAvailable && !cartItem && !isPreOrder && <TooltipContent><p>Сообщить о поступлении</p></TooltipContent>}
                                            {isPreOrder && !cartItem && <TooltipContent><p>Оформить предзаказ</p></TooltipContent>}
                                            {isSubscribed && <TooltipContent><p>Отказаться от ожидания</p></TooltipContent>}
                                            {isNowAvailable && <TooltipContent><p>Товар снова в наличии!</p></TooltipContent>}
                                            {cartItem && <TooltipContent><p>Изменить количество в корзине</p></TooltipContent>}
                                        </Tooltip>
                                    )
                                    })}
                                </div>
                    ...
                    <div className="space-y-4 text-sm text-muted-foreground mt-6 border-t pt-6">
                        <div className="grid grid-cols-2 gap-y-2 gap-x-4">
                            {details.filter(d => d.condition && d.value).map(detail => (
                    ...
                    {settings.wardrobe_compatibility && product.wardrobeCompatibility && (
                        <Button variant="link" className="mt-2 text-muted-foreground" onClick={() => setIsCompatibilityOpen(true)}>
                    ...
                    <div className="flex items-center gap-2 mt-8">
                        {selectedSize ? (
                            <>
                                <div className="flex items-center gap-1 border rounded-md p-0.5">
                                    <Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => handleQuantityChange(-1)}><Minus className="h-4 w-4" /></Button>
                                    <Input readOnly value={quantity} className="w-12 h-9 text-center border-none focus-visible:ring-0 p-0 font-bold" />
                                    <Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => handleQuantityChange(1)} disabled={maxQuantity !== undefined && quantity >= maxQuantity}>
                                        <Plus className="h-4 w-4"/>
                                    </Button>
                                </div>
                                <Button size="lg" className="flex-1" onClick={handleAddToCart}><ShoppingCart className="mr-2 h-5 w-5"/> Добавить</Button>
                            </>
                        ) : (
                            <Button size="lg" className="flex-1" disabled={allSizes.length > 0}>Выберите размер</Button>
                        )}
                        {settings.wishlist_button && (
                            <Popover>
                                <PopoverTrigger asChild>
                                <Button size="icon" variant="outline" className="h-11 w-11" onClick={(e) => {e.stopPropagation(); e.preventDefault();}}><Heart className={cn("h-5 w-5", isInWishlist && "fill-current text-red-500")} /></Button>
                                </PopoverTrigger>
                        ...
                        {settings.compare_button && <Button size="icon" variant="outline" className="h-11 w-11" onClick={() => toggleComparisonItem(product)}><Scale className="h-5 w-5"/></Button>}
                        {settings.share_button && <Button size="icon" variant="outline" className="h-11 w-11"><Share2 className="h-5 w-5"/></Button>}
                    </div>
                    )}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                    {settings['3d_viewer_button'] && product.hasAR && ( <Button size="lg" variant="outline" onClick={() => setIs3dOpen(true)}><View className="mr-2 h-5 w-5 text-accent"/> Просмотр в 3D</Button>)}
                    {settings.ar_button && product.hasAR && (<Button size="lg" variant="outline" onClick={() => setIsArOpen(true)}><Sparkles className="mr-2 h-5 w-5 text-accent"/> Digital-Примерка</Button>)}
                    </div>
                </div>
            </div>

            {settings.community_looks && <CommunityLooksPreview productId={product.id}/>}
            {settings.price_comparison && <div className="my-16"><PriceComparisonTable productId={product.id} /></div>}
            {settings.recommendations && <RecommendedProducts productId={product.id} />}
            {settings.reviews_block && <div className="mt-16"><ProductReviewsDialog product={product} isOpen={isReviewsOpen} onOpenChange={setIsReviewsOpen} /></div> }


            {isReviewsOpen && <ProductReviewsDialog product={product} isOpen={isReviewsOpen} onOpenChange={setIsReviewsOpen} />}
            {isSizeGuideOpen && <SizeGuideDialog product={product} isOpen={isSizeGuideOpen} onOpenChange={setIsSizeGuideOpen} />}
            {isSimilarProductsOpen && <SimilarProductsDialog product={product} isOpen={isSimilarProductsOpen} onOpenChange={setIsSimilarProductsOpen} />}
            {isCompatibilityOpen && <WardrobeCompatibilityDialog product={product} isOpen={isCompatibilityOpen} onOpenChange={setIsCompatibilityOpen} />}
            {isBestsellerOpen && <BestsellerRankingDialog product={product} isOpen={isBestsellerOpen} onOpenChange={setIsBestsellerOpen} />}
            {product.hasAR && is3dOpen && <Product3dViewer product={product} isOpen={is3dOpen} onOpenChange={setIs3dOpen} />}
            {product.hasAR && isArOpen && <ArViewerDialog product={product} isOpen={isArOpen} onOpenChange={setIsArOpen} />}
            ...
            <ProductImageViewer productName={product.name} images={imagesForCurrentColor} isOpen={isImageViewerOpen} onOpenChange={setIsImageViewerOpen} startIndex={imageViewerStartIndex} />
            {notifyDialogSize && (
                <NotifyMeDialog product={product} size={notifyDialogSize} mode={notifyDialogMode} preOrderDate={notifyDialogPreOrderDate} isOpen={!!notifyDialogSize} onOpenChange={(open) => !open && setNotifyDialogSize(null)} onConfirm={handleConfirmSubscription} />
            )}
            {manageCartItem && (
             ...
             {unsubscribeDialogSize && (
                <UnsubscribeDialog product={product} size={unsubscribeDialogSize} isOpen={!!unsubscribeDialogSize} onOpenChange={(open) => !open && setUnsubscribeDialogSize(null)} onConfirm={handleConfirmUnsubscribe} />
            )}
        </>
    );
});
...
export default function ProductDisplayInfoPage() {
  ...
  const [productDisplaySettings, setProductDisplaySettings] = React.useState<Record<string, Record<string, boolean>>>({ grid: {}, list: {}, detail: {} });
  const [brandDisplaySettings, setBrandDisplaySettings] = React.useState<Record<string, Record<string, boolean>>>({ grid: {}, list: {}, detail: {} });
  
  const [activePreview, setActivePreview] = useState<'grid' | 'list' | 'detail'>('detail');
  const [activeBrandPreview, setActiveBrandPreview] = useState<'grid' | 'list' | 'detail'>('detail');
  ...
  useEffect(() => {
    async function fetchData() {
        ...
        try {
            ...
            const initialProductSettings: Record<string, Record<string, boolean>> = { grid: {}, list: {}, detail: {} };
            ...
            const initialBrandSettings: Record<string, Record<string, boolean>> = { grid: {}, list: {}, detail: {} };
  ...
  const handleSettingChange = (type: 'product' | 'brand', viewId: 'grid' | 'list' | 'detail', paramId: string, checked: boolean) => {
  ...
  const mockProduct = {
      ...
      sizes: ['XS', 'S', 'M', 'L', 'XL'],
      availableColors: [
          { id: 'color1', name: "Черный", hex: '#000000', sizeAvailability: [{size: 'XS', status: 'in_stock', quantity: 10}, {size: 'S', status: 'in_stock', quantity: 5}, {size: 'M', status: 'out_of_stock'}, {size: 'L', status: 'pre_order', preOrderDate: '2024-09-15'}, {size: 'XL', status: 'out_of_stock'}] },
          { id: 'color2', name: "Серый", hex: '#808080', sizeAvailability: [{size: 'S', status: 'in_stock', quantity: 2}, {size: 'M', status: 'in_stock', quantity: 8}] },
          { id: 'color3', name: "Бежевый", hex: '#F5F5DC', sizeAvailability: [{size: 'XS', status: 'in_stock', quantity: 12}, {size: 'S', status: 'in_stock', quantity: 15}, {size: 'M', status: 'in_stock', quantity: 20}, {size: 'L', status: 'in_stock', quantity: 10}, {size: 'XL', status: 'in_stock', quantity: 5}] },
      ],
      images: [
        { "id": "product-1-1", "url": "https://images.unsplash.com/photo-1652904875075-4534c3439c62?w=800", "alt": "Вязаный свитер", "hint": "knit sweater", "colorName": "Черный" },
        { "id": "product-1-2", "url": "https://picsum.photos/seed/prod1-2/800/1000", "alt": "Деталь свитера", "hint": "fabric detail", "colorName": "Черный" },
        { "id": "product-1-3", "url": "https://picsum.photos/seed/gray-sweater/800/1000", "alt": "Свитер на модели", "hint": "fashion model", "colorName": "Серый" },
        { "id": "product-1-4", "url": "https://picsum.photos/seed/beige-sweater/800/1000", "alt": "Текстура ткани", "hint": "sweater texture", "colorName": "Бежевый" }
      ]
  };
      ...
      <Table>
        <TableHeader>
            ...
            {data.views.map((view: any) => (
                <TableHead key={view.id} className="text-center font-semibold">{view.name}</TableHead>
            ))}
            </TableRow>
        </TableHeader>
        <TableBody>
            ...
            <React.Fragment key={`${group.group}-${groupIndex}`}>
                ...
                <TableCell colSpan={data.views.length + 1} className="font-semibold text-foreground">
                    {group.group}
                </TableCell>
                </TableRow>
                        ...
                        <TableCell key={`${item.id}-${view.id}`} className="text-center">
                            <div className="flex justify-center items-center">
                                <Checkbox 
                                    ...
                                    onCheckedChange={(checked) => handleSettingChange(type, view.id, item.id, !!checked)}
                                    disabled={item.disabled?.[view.id] || (item.id === 'discount_badge' && !settings[view.id]?.['old_price'])}
                                />
                            </div>
                        </TableCell>
                    ))}
            ...
            <header>
             ...
             <Tabs defaultValue="products" className="w-full">
                ...
                <TabsContent value="products" className="mt-6 space-y-4">
                    <Card>
                        <CardHeader><CardTitle>Структура отображения товаров</CardTitle></CardHeader>
                    ...
                    <Card>
                        <CardHeader><CardTitle>Интерактивный предпросмотр товара</CardTitle></CardHeader>
                        <CardContent>
                            <RadioGroup value={activePreview} onValueChange={(v) => setActivePreview(v as any)} className="flex space-x-2 pt-2 mb-4 justify-center">
                                {productDisplayData.views.map((view: any) => (
                                    <Label key={view.id} htmlFor={`preview-${view.id}`} className={cn("flex-1 p-2 border rounded-md cursor-pointer text-center", activePreview === view.id && "bg-secondary")}>
                                        <RadioGroupItem value={view.id} id={`preview-${view.id}`} className="sr-only" />
                                        {view.name}
                                    </Label>
                                ))}
                            </RadioGroup>
                            <div className="p-4 md:p-4 bg-secondary/30 rounded-lg flex items-start justify-center">
                                <div className={cn("w-full transition-all duration-300", 
                                    ...
                                    {activePreview === 'grid' && <GridPreview settings={productDisplaySettings.grid} product={mockProduct} onQuickViewClick={() => setQuickViewProduct(mockProduct)} />}
                                    {activePreview === 'list' && <ListPreview settings={productDisplaySettings.list} product={mockProduct} />}
                                    {activePreview === 'detail' && <DetailPreview settings={productDisplaySettings.detail} product={mockProduct} />}
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                ...
                <TabsContent value="brands" className="mt-6 space-y-4">
                     <Card>
                        <CardHeader><CardTitle>Структура отображения брендов</CardTitle></CardHeader>
                        <CardContent className="p-0">
                             {renderTable(brandDisplayData, brandDisplaySettings, 'brand')}
                        </CardContent>
                     </Card>
                     <Card>
                        <CardHeader><CardTitle>Интерактивный предпросмотр бренда</CardTitle></CardHeader>
                        <CardContent>
                            <RadioGroup value={activeBrandPreview} onValueChange={(v) => setActiveBrandPreview(v as any)} className="flex space-x-2 pt-2 mb-4 justify-center">
                                {brandDisplayData.views.map((view: any) => (
                                    <Label key={view.id} htmlFor={`brand-preview-${view.id}`} className={cn("flex-1 p-2 border rounded-md cursor-pointer text-center", activeBrandPreview === view.id && "bg-secondary")}>
                                        <RadioGroupItem value={view.id} id={`brand-preview-${view.id}`} className="sr-only" />
                                        {view.name}
                                    </Label>
                                ))}
                            </RadioGroup>
                             <div className="p-4 md:p-4 bg-secondary/30 rounded-lg flex items-start justify-center">
                                <div className={cn("w-full transition-all duration-300", 
                                    ...
                                    {activeBrandPreview === 'grid' && <BrandCard brand={mockBrand} productCount={mockProductCount} displaySettings={brandDisplaySettings.grid} />}
                                    {activeBrandPreview === 'list' && <BrandListItem brand={mockBrand} productCount={mockProductCount} displaySettings={brandDisplaySettings.list} />}
                                    {activeBrandPreview === 'detail' && (
                                        <div className="border rounded-lg bg-background overflow-hidden">
                                            <BrandProfilePage params={Promise.resolve({brandId: mockBrand.slug})} isPreview={true} displaySettings={brandDisplaySettings.detail} />
                                        </div>
                                    )}
                                </div>
                             </div>
