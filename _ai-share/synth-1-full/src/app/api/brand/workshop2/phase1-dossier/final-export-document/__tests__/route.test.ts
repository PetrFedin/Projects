...
describe('POST /api/brand/workshop2/phase1-dossier/final-export-document', () => {
  ...
  it('builds factory pack html when format=factory_pack', async () => {
    await putWorkshop2ServerDossierRecord({
      collectionId: 'c1',
      articleId: 'a1',
      dossier: { assignments: [] } as never,
    ...
    const docRes = await buildDoc(docReq as never);
    expect(docRes.status).toBe(200);
    const docJson = (await docRes.json()) as { ok: boolean; html?: string; format?: string };
    expect(docJson.ok).toBe(true);
    expect(docJson.format).toBe('factory_pack');
    expect(docJson.html).toContain('Factory pack');
  });
});
