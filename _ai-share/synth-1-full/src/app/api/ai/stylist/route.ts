...
export async function POST(req: NextRequest) {
  ...
  if (parsed.excludeOversized || parsed.excludeBright) {
    body.preferences = { ...(body.preferences ?? {}), excludeOversized: parsed.excludeOversized, excludeBright: parsed.excludeBright };
  }
  ...
  if (body.temperature == null) {
    try {
      const wRes = await fetch("https://api.open-meteo.com/v1/forecast?latitude=55.7558&longitude=37.6173&current=temperature_2m");
      if (wRes.ok) {
  ...
  if (lastMessage.includes("расскажи про") || lastMessage.includes("что такое") || lastMessage.includes("сколько стоит")) {
    ...
    if (product) {
      ...
      const personaName = body.mood === "street" ? "Urban Explorer" : body.mood === "avant" ? "Visionary" : "Minimalist Master";
  ...
  if (!body?.occasion || !body?.audience || !body?.season) {
    return NextResponse.json({ error: "audience, occasion, and season are required" }, { status: 400 });
  }

  if (parsed.rainy && body.includeCategories?.length && !body.includeCategories.includes("Outerwear")) {
  ...
  if (result.looks?.length) {
    try {
      const looksSummary = result.looks
        .map((l) =>
          l.items
            .map((it) => (it.title ? `${it.title}${it.price ? ` (${it.price.toLocaleString("ru-RU")} ₽)` : ""}` : ""))
            .filter(Boolean)
            .join(", ")
        )
        .filter(Boolean)
