...
const userBrands = [
  {
    ...
    // Поведенческая аналитика пользователя
    behavioralProfile: {
        ...
        <div className="space-y-6 lg:col-span-4">
          <div className="rounded-xl border border-muted/20 bg-muted/30 p-2 shadow-inner">
            {userBrands.map((brand) => (
              ...
              >
                <div className="border-border-subtle bg-bg-surface relative h-12 w-12 shrink-0 overflow-hidden rounded-2xl border shadow-sm">
                  {typeof brand.logo === 'string' ? (
                    <Image src={brand.logo} alt={brand.name} fill className="object-contain p-3" />
                  ) : (
                    <div className="bg-bg-surface2 flex h-full w-full items-center justify-center">
                      <Building2 className="text-text-muted h-6 w-6" />
                    </div>
                  )}
                </div>
                    ...
                    <Badge
                      className={cn(
                        ...
                        brand.level === 2
                          ...
                          : 'bg-text-muted text-text-inverse'
                      )}
                    >
                      {brand.status}
                    </Badge>
        ...
        <div className="lg:col-span-8">
          <AnimatePresence mode="wait">
            ...
            >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative h-20 w-20 overflow-hidden rounded-[1.5rem] bg-white p-4 shadow-xl">
                      {typeof selectedBrand.logo === 'string' ? (
                        ...
                        <div className="bg-bg-surface2 flex h-full w-full items-center justify-center">
                          <Building2 className="text-text-muted h-8 w-8" />
                        </div>
                      )}
                    </div>
              ...
              {/* Поведенческий профиль и синергия */}
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div className="space-y-6">
                  <h4 className="text-text-muted flex items-center gap-2 border-b pb-3 text-xs font-black uppercase tracking-[0.2em]">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {[
                      ...
                      <div
                        key={i}
                        className="border-border-subtle bg-bg-surface group flex items-center justify-between rounded-2xl border p-4 shadow-sm transition-all hover:border-accent/20"
                      >
                        <div className="flex items-center gap-3">
                          <div className="bg-bg-surface2 text-text-muted flex h-8 w-8 items-center justify-center rounded-xl transition-all group-hover:bg-accent/5 group-hover:text-accent">
                            {item.icon}
                          </div>
                ...
                <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedBrand.brandSynergy.mixesWith.map((b, i) => (
                            <Badge
                              key={i}
                              variant="outline"
                              className="border-border-subtle bg-bg-surface2 rounded-xl px-3 py-1.5 text-[10px] font-black uppercase tracking-tight"
                            >
                  ...
                  <div className="space-y-6">
                    <h4 className="text-text-muted flex items-center gap-2 px-4 text-xs font-black uppercase tracking-[0.2em]">
