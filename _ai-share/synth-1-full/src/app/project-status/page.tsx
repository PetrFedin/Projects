...
function StatusBadge({ status }: { status: Status }) {
  ...
  return (
    <Badge variant="outline" className={cn('flex items-center gap-1.5 w-fit font-normal text-xs', config.color, `border-current/30`)}>
...
export default function ProjectStatusPage() {
    ...
    const stats = useMemo(() => {
        if (!featureRegistry) return { featureCount: 0, componentCount: 0, elementCount: 0, inProgressCount: 0, plannedCount: 0 };
        let featureCount = 0;
  ...
  if (isLoading) {
      return (
          <div className="container mx-auto px-4 py-12 space-y-6">
               <header className="flex flex-col md:flex-row justify-between md:items-start gap-3">
    ...
    <div className="container mx-auto px-4 py-12 space-y-6">
      <header className="flex flex-col md:flex-row justify-between md:items-start gap-3">
        <div className="flex-1">
            <h1 className="text-sm md:text-sm font-bold font-headline">Реестр проекта</h1>
         ...
         <Card className="w-full md:w-auto md:min-w-[320px]">
            ...
            <CardContent className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                <div className="font-semibold">Функции:</div><div className="text-right">{stats.featureCount}</div>
                <div className="font-semibold">Компоненты:</div><div className="text-right">{stats.componentCount}</div>
                <div className="font-semibold">Элементы:</div><div className="text-right">{stats.elementCount}</div>
                <div className="font-semibold text-green-600">В реализации:</div><div className="text-right font-semibold text-green-600">{stats.inProgressCount}</div>
                <div className="font-semibold text-amber-600">Запланировано:</div><div className="text-right font-semibold text-amber-600">{stats.plannedCount}</div>
            </CardContent>
        </Card>
      </header>
            ...
            <Table>
              ...
              <TableBody>
                {flattenedData.map((row, index) => {
                    const isSectionTitle = !row.componentName && !row.elementName && !row.description && !row.comment;
                    
                    const isFirstOfFunc = index === 0 || flattenedData[index - 1].functionName !== row.functionName && row.functionName !== '';
                    const isFirstOfComp = index === 0 || flattenedData[index - 1].componentName !== row.componentName && row.componentName !== '';


                    if (isSectionTitle) {
                         return (
                             <TableRow key={index} className="bg-muted/50 hover:bg-muted/50">
                                <TableCell colSpan={6} className="font-bold text-base text-foreground">{row.functionName}</TableCell>
                            </TableRow>
                        )
                    }
                    return (
                        <TableRow key={index}>
                            ...
                            <TableCell className="align-top"><StatusBadge status={row.status} /></TableCell>
                            <TableCell className="text-xs text-muted-foreground align-top">{row.description}</TableCell>
                            <TableCell className="text-xs italic text-muted-foreground align-top">{row.comment}</TableCell>
                        </TableRow>
                    )
                })}
              </TableBody>
