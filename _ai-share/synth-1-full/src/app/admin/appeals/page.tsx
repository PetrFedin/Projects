...
export default function AppealsPage() {
    ...
    return (
        <div className="space-y-4">
             <header>
                <h1 className="text-base font-bold font-headline">Апелляции</h1>
                <p className="text-muted-foreground">Запросы от брендов на пересмотр статуса промо-кампаний.</p>
            </header>
                                        ...
                                        <div className="flex flex-col">
                                            {isClient ? (
                                                <>
                                                    ...
                                                    <span className="text-xs text-muted-foreground">{formatDistanceToNow(appealDate, { addSuffix: true, locale: ru })}</span>
                                                </>
                                            ) : (
                                         ...
                                         <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                <DropdownMenuItem onClick={() => handleApprove(appeal.id)}>
                                                 ...
                                                 <DropdownMenuSeparator />
                                                 <DropdownMenuRadioGroup value={rejectionReason || ''} onValueChange={setRejectionReason}>
                                                    <DropdownMenuRadioItem value="policy">Не соотв. политике</DropdownMenuRadioItem>
                                                    <DropdownMenuRadioItem value="quality">Низкое качество</DropdownMenuRadioItem>
                                                    <DropdownMenuRadioItem value="other">Другое</DropdownMenuRadioItem>
                                                </DropdownMenuRadioGroup>
                                                <DropdownMenuItem className="text-destructive" onClick={() => handleReject(appeal.id)} disabled={!rejectionReason}>
