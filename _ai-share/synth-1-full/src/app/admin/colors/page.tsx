...
export default function ColorsPage() {
    ...
    const handleAddColor = () => {
        if (newColor.name.trim() && !palette.some(c => c.hex.toLowerCase() === newColor.hex.toLowerCase())) {
             ...
             <header className="flex justify-between items-center">
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    <DialogTrigger asChild>
                        <Button onClick={() => { setNewColor({ name: '', hex: '#000000' }); setIsDialogOpen(true); }}>
                    ...
                    <DialogContent>
                        ...
                        <div className="py-4 space-y-4">
                           <div className="grid grid-cols-1 gap-3">
                                <div className="space-y-2">
                                     ...
                                     <Input id="color-name" value={newColor.name} onChange={handleNameChange} placeholder="Например, Глубокий синий" />
                                </div>
                                <div className="space-y-2">
                                    <Label>Выберите цвет</Label>
                                    <div className="flex items-center gap-2">
                                        <div className="relative h-10 w-10 rounded-md border p-0 overflow-hidden">
                                          <Input 
                                              type="color" 
                                              value={newColor.hex} 
                                              onChange={handleColorChange} 
                                              className="absolute -top-2 -left-2 h-12 w-12 cursor-pointer p-0 border-none"
                                          />
                                        </div>
                        ...
                        <DialogFooter>
                            ...
                            <Button onClick={handleAddColor} disabled={!newColor.name.trim()}>Добавить</Button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            </header>
                            ...
                            <TableBody>
                                {palette.map(color => (
                                    <TableRow key={color.hex}>
                                        <TableCell>
                                            <div className="h-8 w-12 rounded-md border" style={{ backgroundColor: color.hex }}></div>
                                        </TableCell>
                                            ...
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    ...
                                                    <DropdownMenuItem 
                                                        ...
                                                        onClick={() => setPalette(prev => prev.filter(c => c.hex !== color.hex))}
                                                    >
                                                        Удалить
                                                    </DropdownMenuItem>
                                                </DropdownMenuContent>
