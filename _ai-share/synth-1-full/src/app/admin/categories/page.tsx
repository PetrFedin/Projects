                    ...
                    <Table>
                        ...
                        <TableBody>
                            {history.map((entry, index) => (
                                <TableRow key={index}>
                                    <TableCell className="text-xs text-muted-foreground">{new Date(entry.date).toLocaleString('ru-RU')}</TableCell>
...
const womenOnlyCategories = ["Платья и сарафаны", "Юбки", "Бюстгальтеры", "Коктейльные", "Вечерние", "Балетки", "Ботфорты", "Кроп-топы", "Купальники", "Парео", "Пляжные платья", "Береты", "Клатч", "Вечерняя", "Bootcut", "Flare", "Повседневные", "Мини", "Миди", "Макси", "Карандаш", "А-силуэт", "Плиссе", "Босоножки", "Челси на каблуке"];
...
function getAudienceForCategory(cat: Omit<FlatCategory, 'audience' | 'attributes' | 'status' | 'history'>): string[] {
    ...
    if (kidsKeywords.some(kw => fullPath.toLowerCase().includes(kw))) {
        if (babyKeywords.some(kw => fullPath.toLowerCase().includes(kw))) return ['Дети - Новорожденные'];
...
function getDefaultAttributesForCategory(category: Omit<FlatCategory, 'audience' | 'attributes' | 'status' | 'history'>): CategoryAttributes {
    const attributes: CategoryAttributes = {};
    const { level1, level2, level3 } = category;
    
    const getRelevantOptions = (optionsObject: Record<string, string[]> | {value: string; label: string}[]): string[] => {
    ...
    const attributeConfig: { key: keyof CategoryAttributes; options: string[] }[] = [
        ...
        { key: 'sleeveLength', options: getRelevantOptions(allAttributeOptions.sleeveOptionsByCategory) },
        ...
        { key: 'fastening', options: getRelevantOptions(allAttributeOptions.fasteningOptionsByCategory) },
        ...
        { key: 'stitching', options: getRelevantOptions(allAttributeOptions.stitchingOptionsByCategory) },
        ...
        { key: 'fabricTexture', options: getRelevantOptions(allAttributeOptions.fabricTextureOptionsByCategory) },
        { key: 'processingTech', options: getRelevantOptions(allAttributeOptions.processingTechOptionsByCategory) },
        ...
        { key: 'backDetails', options: getRelevantOptions(allAttributeOptions.backDetailOptionsByCategory) },
        { key: 'hemType', options: getRelevantOptions(allAttributeOptions.hemTypeOptionsByCategory) },
        { key: 'waistband', options: getRelevantOptions(allAttributeOptions.waistbandOptionsByCategory) },
        { key: 'transformation', options: getRelevantOptions(allAttributeOptions.transformationOptionsByCategory) },
        { key: 'reinforcement', options: getRelevantOptions(allAttributeOptions.reinforcementOptionsByCategory) },
        { key: 'seam', options: getRelevantOptions(allAttributeOptions.seamOptionsByCategory) },
        { key: 'combination', options: getRelevantOptions(allAttributeOptions.combinationOptionsByCategory) },
        { key: 'drapery', options: getRelevantOptions(allAttributeOptions.draperyOptionsByCategory) },
        { key: 'hemFinish', options: getRelevantOptions(allAttributeOptions.hemFinishOptionsByCategory) },
    ];
...
export default function CategoriesPage() {
    ...
    const [editingCategory, setEditingCategory] = useState<{ index: number; category: FlatCategory } | null>(null);
    ...
    useEffect(() => {
        ...
        setCategoriesData(result.map(cat => ({
            ...cat, 
    ...
    const filteredCategories = useMemo(() => {
        return categoriesData.map((cat, index) => ({...cat, originalIndex: index})).filter(cat => {
    ...
    const level1Options = useMemo(() => [...new Set(categoriesData.map(c => c.level1))], [categoriesData]);
    const level2Options = useMemo(() => {
        if (!filterLevel1) return [];
        return [...new Set(categoriesData.filter(c => c.level1 === filterLevel1 && c.level2).map(c => c.level2!))];
    ...
    const handleAudienceChange = (index: number, newAudience: string[] | undefined) => {
        setCategoriesData(prev => prev.map((cat, i) => {
            if (i === index) {
                ...
                return { ...cat, audience: newAudience || [], history: [newHistoryEntry, ...cat.history] };
            }
            return cat;
        }));
    };
    ...
    const handleStatusChange = (index: number, newStatus: boolean) => {
         setCategoriesData(prev => prev.map((cat, i) => {
             if (i === index) {
                 ...
                 return { ...cat, status: newStatus ? 'active' : 'inactive', history: [newHistoryEntry, ...cat.history] };
             }
             return cat;
         }));
    }
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                 <header>
                    <h1 className="text-base font-bold font-headline">Справочник категорий</h1>
                    <p className="text-muted-foreground">Управление иерархией категорий товаров и их атрибутами.</p>
                </header>
                <Button>
             ...
             <Card>
                <CardHeader>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                        <CardDescription>Всего записей: {isLoading ? '...' : filteredCategories.length}</CardDescription>
                        <div className="flex gap-2">
                            ...
                            <Select onValueChange={(v) => setFilterLevel2(v === 'all' ? undefined : v)} disabled={!filterLevel1 || isLoading}>
                                <SelectTrigger className="w-full sm:w-[200px]">
                            ...
                            <TableBody>
                                {filteredCategories.map((cat) => (
                                    <TableRow key={`${cat.level1}-${cat.level2}-${cat.level3}-${cat.originalIndex}`}>
                                        <TableCell className="font-medium py-2">{cat.level1}</TableCell>
                                        ...
                                        <TableCell className="py-2">
                                            <Combobox
                                                ...
                                                value={cat.audience}
                                                onChange={(val) => handleAudienceChange(cat.originalIndex, val as string[] | undefined)}
                                                multiple
                                        ...
                                        <TableCell className="py-2">
                                            <Button 
                                                variant="outline" 
                                                size="sm"
                                                onClick={() => setEditingCategory({ index: cat.originalIndex, category: categoriesData[cat.originalIndex] })}
                                            >
                                        ...
                                        <TableCell className="py-2">
                                            <Switch 
                                                ...
                                                onCheckedChange={(checked) => handleStatusChange(cat.originalIndex, checked)}
                                            />
                                        </TableCell>
                                                ...
                                                <TooltipProvider>
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <Button size="icon" variant="ghost" onClick={() => openHistoryDialog(categoriesData[cat.originalIndex])}>
            ...
            {editingCategory && (
             ...
             {viewingHistory && (
                <HistoryDialog
                    ...
                    onOpenChange={(open) => { if (!open) setViewingHistory(null); }}
