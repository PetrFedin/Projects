...
import { PlusCircle, Info, Footprints, Shirt, ShoppingBag, LandPlot, Briefcase, Backpack } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
import { sizeChartDataHats, sizeChartDataGloves, sizeChartDataBelts, sizeChartDataScarves, sizeChartDataGlasses, sizeChartDataRings, sizeChartDataWatches, sizeChartDataSocks, sizeChartDataWallets, sizeChartDataUmbrellas } from "@/lib/accessory-sizes";
import { Skeleton } from "@/components/ui/skeleton";


const FitGuideDialog = dynamic(() => import('@/components/admin/fit-guide-dialog').then(mod => mod.FitGuideDialog));
const FitGuideOuterwearDialog = dynamic(() => import('@/components/admin/fit-guide-outerwear-dialog').then(mod => mod.FitGuideOuterwearDialog));
const FitGuideSuitsDialog = dynamic(() => import('@/components/admin/fit-guide-suits-dialog').then(mod => mod.FitGuideSuitsDialog));
const FitGuideSkirtsDialog = dynamic(() => import('@/components/admin/fit-guide-skirts-dialog').then(mod => mod.FitGuideSkirtsDialog));
const FitGuideShirtsDialog = dynamic(() => import('@/components/admin/fit-guide-shirts-dialog').then(mod => mod.FitGuideShirtsDialog));
const FitGuideTopsDialog = dynamic(() => import('@/components/admin/fit-guide-tops-dialog').then(mod => mod.FitGuideTopsDialog));
const FitGuideJeansDialog = dynamic(() => import('@/components/admin/fit-guide-jeans-dialog').then(mod => mod.FitGuideJeansDialog));
const FitGuideTrousersDialog = dynamic(() => import('@/components/admin/fit-guide-trousers-dialog').then(mod => mod.FitGuideTrousersDialog));
const FitGuideKnitwearDialog = dynamic(() => import('@/components/admin/fit-guide-knitwear-dialog').then(mod => mod.FitGuideKnitwearDialog));
const FitGuideLingerieDialog = dynamic(() => import('@/components/admin/fit-guide-lingerie-dialog').then(mod => mod.FitGuideLingerieDialog));
const FitGuideSportswearDialog = dynamic(() => import('@/components/admin/fit-guide-sportswear-dialog').then(mod => mod.FitGuideSportswearDialog));
const FitGuideBeachwearDialog = dynamic(() => import('@/components/admin/fit-guide-beachwear-dialog').then(mod => mod.FitGuideBeachwearDialog));
const FitGuideMaternityDialog = dynamic(() => import('@/components/admin/fit-guide-maternity-dialog').then(mod => mod.FitGuideMaternityDialog));
const FitGuideAdaptiveDialog = dynamic(() => import('@/components/admin/fit-guide-adaptive-dialog').then(mod => mod.FitGuideAdaptiveDialog));
const FitGuideShoesDialog = dynamic(() => import('@/components/admin/fit-guide-shoes-dialog').then(mod => mod.FitGuideShoesDialog));
const FitGuideBootsDialog = dynamic(() => import('@/components/admin/fit-guide-boots-dialog').then(mod => mod.FitGuideBootsDialog));
const FitGuideFlatsDialog = dynamic(() => import('@/components/admin/fit-guide-flats-dialog').then(mod => mod.FitGuideFlatsDialog));
const FitGuideSandalsDialog = dynamic(() => import('@/components/admin/fit-guide-sandals-dialog').then(mod => mod.FitGuideSandalsDialog));
const FitGuideSneakersDialog = dynamic(() => import('@/components/admin/fit-guide-sneakers-dialog').then(mod => mod.FitGuideSneakersDialog));
const FitGuideHomeShoesDialog = dynamic(() => import('@/components/admin/fit-guide-home-shoes-dialog').then(mod => mod.FitGuideHomeShoesDialog));
const FitGuideBagsDialog = dynamic(() => import('@/components/admin/fit-guide-bags-dialog').then(mod => mod.FitGuideBagsDialog));
const FitGuideAccessoriesDialog = dynamic(() => import('@/components/admin/fit-guide-accessories-dialog').then(mod => mod.FitGuideAccessoriesDialog));
const FitGuideMiscDialog = dynamic(() => import('@/components/admin/fit-guide-misc-dialog').then(mod => mod.FitGuideMiscDialog));


type BagType = 'Mini / Micro' | 'Clutch' | 'Crossbody' | 'Satchel' | 'Tote' | 'Shopper' | 'Hobo' | 'Bucket' | 'Backpack' | 'Brief / Office' | 'Belt / Waist Bag' | 'Sling / Cross-sling Bag' | 'Travel / Weekender / Duffel' | 'Cabin / Suitcase (жёсткий)' | 'Vanity / Cosmetic Case' | 'Laptop / Tech Bag' | 'Gym / Sport Bag';
type AccessoryType = 'Головные уборы' | 'Перчатки' | 'Ремни' | 'Шарфы, палантины, платки' | 'Очки' | 'Кольца' | 'Часы' | 'Носки и чулки' | 'Кошельки' | 'Зонты';
...
const accessoriesCategories = [
    { title: "👒 Головные уборы", data: sizeChartDataHats, dialog: "accessories", accessoryKey: "Головные уборы" },
    { title: "🧤 Перчатки", data: sizeChartDataGloves, dialog: "accessories", accessoryKey: "Перчатки" },
    { title: "🧢 Ремни", data: sizeChartDataBelts, dialog: "accessories", accessoryKey: "Ремни" },
    { title: "🧣 Шарфы, палантины, платки", data: sizeChartDataScarves, dialog: "accessories", accessoryKey: "Шарфы, палантины, платки" },
    { title: "🕶 Очки", data: sizeChartDataGlasses, dialog: "accessories", accessoryKey: "Очки (оптика и солнцезащита)" },
    { title: "💍 Кольца", data: sizeChartDataRings, dialog: "accessories", accessoryKey: "Украшения (кольца, браслеты, цепи, серьги)" },
    { title: "⌚ Часы", data: sizeChartDataWatches, dialog: "accessories", accessoryKey: "Часы" },
    { title: "🧦 Носки и чулки", data: sizeChartDataSocks, dialog: "accessories", accessoryKey: "Носки и чулки" },
    { title: "👜 Кошельки", data: sizeChartDataWallets, dialog: "accessories", accessoryKey: "Кошельки и кардхолдеры" },
    { title: "☂️ Зонты", data: sizeChartDataUmbrellas, dialog: "accessories", accessoryKey: "Зонты" },
];

const miscCategories = [
    { title: "🧴 Косметички / несессеры", dialog: "misc", miscKey: "Косметички / несессеры / travel-поуч" },
...
export default function SizesPage() {
    ...
    const [isLoading, setIsLoading] = useState({ 
        outerwear: true, suits: true, dresses: true, tops: true, jeans: true, 
        skirts: true, trousers: true, knitwear: true, lingerie: true, sportswear: true, 
        beachwear: true, maternity: true, adaptive: true, shoes: true, boots: true, flats: true,
        sandals: true, sneakers: true, homeshoes: true, bags: true
    });
    ...
    useEffect(() => {
        const fetchData = async (url: string, setData: (data: any[]) => void, key: keyof typeof isLoading) => {
    ...
    const womenClothingCategories = [
        { title: "Женская верхняя одежда", data: sizeChartDataOuterwear, dialog: "outerwear", loading: isLoading.outerwear },
        { title: "Женские костюмы и жакеты", data: sizeChartDataSuits, dialog: "suits", loading: isLoading.suits },
        { title: "Женские платья и сарафаны", data: sizeChartDataDresses, dialog: "dresses", loading: isLoading.dresses },
        { title: "Женские юбки", data: sizeChartDataSkirts, dialog: "skirts", loading: isLoading.skirts },
        { title: "Женские рубашки и блузы", data: sizeChartDataShirts, dialog: "shirts" },
        { title: "Женские топы и футболки", data: sizeChartDataTops, dialog: "tops", loading: isLoading.tops },
        { title: "Женские джинсы", data: sizeChartDataJeans, dialog: "jeans", loading: isLoading.jeans },
        { title: "Женские брюки", data: sizeChartDataTrousers, dialog: "trousers", loading: isLoading.trousers },
        { title: "Женский трикотаж", data: sizeChartDataKnitwear, dialog: "knitwear", loading: isLoading.knitwear },
        { title: "Женское нижнее бельё и домашняя одежда", data: sizeChartDataLingerie, dialog: "lingerie", loading: isLoading.lingerie },
        { title: "Женская спортивная одежда", data: sizeChartDataSportswear, dialog: "sportswear", loading: isLoading.sportswear },
        { title: "Женская пляжная мода", data: sizeChartDataBeachwear, dialog: "beachwear", loading: isLoading.beachwear },
        { title: "Женская одежда для беременных", data: sizeChartDataMaternity, dialog: "maternity", loading: isLoading.maternity },
        { title: "♿ Женская одежда для ограниченной подвижности", data: sizeChartDataAdaptive, dialog: "adaptive", loading: isLoading.adaptive },
    ];
    
    const womenShoesCategories = [
        { title: "Женские туфли и лодочки", data: sizeChartDataShoes, dialog: "shoes", loading: isLoading.shoes },
        { title: "Женские ботильоны и сапоги", data: sizeChartDataBoots, dialog: "boots", loading: isLoading.boots },
        { title: "Женские балетки, лоферы и мюли", data: sizeChartDataFlats, dialog: "flats", loading: isLoading.flats },
        { title: "Женские сандалии и босоножки", data: sizeChartDataSandals, dialog: "sandals", loading: isLoading.sandals },
        { title: "Женские кроссовки и спортивная обувь", data: sizeChartDataSneakers, dialog: "sneakers", loading: isLoading.sneakers },
        { title: "Женская домашняя и медицинская обувь", data: sizeChartDataHomeShoes, dialog: "homeshoes", loading: isLoading.homeshoes },
    ];
    ...
    const renderCategoryCard = (cat: any, audience: 'women' | 'men') => {
            ...
            <Card key={cat.title}>
                <CardHeader>
                    <button className="w-fit" onClick={handleTitleClick}>
                        <CardTitle className="cursor-pointer hover:text-accent flex items-center gap-2">
                ...
                {cat.loading ? (
                                ...
                                <TableRow>
                                    {headers.map(header => (
                                        <TableHead key={header}>
                                            <TooltipProvider>
                                                <Tooltip>
                                                    <TooltipTrigger>{headerDescriptions[header]?.title || header}</TooltipTrigger>
                                                    <TooltipContent>
                    ...
                    <CardContent>
                        <p className="text-sm text-muted-foreground">Данные для этой категории будут добавлены позже.</p>
                    </CardContent>
                )}
            </Card>
        );
     ...
     const renderBagCategoryCard = (audience: 'women' | 'men') => {
         ...
         const unisexBagTypes = ['Tote', 'Shopper', 'Backpack', 'Brief / Office', 'Belt / Waist Bag', 'Sling / Cross-sling Bag', 'Travel / Weekender / Duffel', 'Laptop / Tech Bag', 'Gym / Sport Bag'];

         const bagsToShow = audience === 'men' ? cat.data.filter((row: any) => unisexBagTypes.includes(row.type)) : cat.data;
                            ...
                            {bagsToShow.map((row: any, rowIndex: number) => (
                                <TableRow key={rowIndex}>
                                    {headers.map(header => (
                                        <TableCell key={header} className="text-xs">
                                          {header === 'type' ? (
                                              <button className="text-left font-medium hover:text-accent underline" onClick={() => openDialog('bags', row['type'], 'bags')}>
                                                  {row[header]}
                                              </button>
                                          ) : row[header]}
                                        </TableCell>
    ...
    const renderContentForSection = (section: string, audience: 'women' | 'men') => {
        ...
        if (audience === 'women') {
            switch(section) {
                case 'clothing': categoriesToRender = womenClothingCategories; break;
                case 'shoes': categoriesToRender = womenShoesCategories; break;
                case 'bags': return renderBagCategoryCard(audience);
                case 'accessories': categoriesToRender = accessoriesCategories; break;
                case 'misc': categoriesToRender = miscCategories; break;
                default: return null;
            }
        } else { // men
            switch(section) {
                case 'clothing': categoriesToRender = menClothingCategories; break;
                case 'shoes': categoriesToRender = menShoesCategories; break;
                case 'bags': return renderBagCategoryCard(audience);
                case 'accessories': categoriesToRender = accessoriesCategories; break;
                case 'misc': categoriesToRender = miscCategories; break;
                default: return null;
        ...
        return categoriesToRender.map(cat => {
             ...
             return (
                 <Card key={cat.title}>
                    <CardHeader>
                        <button className="w-fit" onClick={() => openDialog(cat.dialog, cat.accessoryKey || cat.miscKey, section)}>
                            <CardTitle className="cursor-pointer hover:text-accent flex items-center gap-2">
                                {cat.title} <Info className="h-4 w-4 text-muted-foreground"/>
                            </CardTitle>
                        </button>
       ...
       <div className="space-y-6">
            <Tabs value={activeSection} onValueChange={setActiveSection} className="w-full">
                <TabsList className="mb-4 h-auto flex-wrap justify-start">
                    ...
                    <TabsTrigger value="misc"><Briefcase className="mr-2 h-4 w-4"/>Прочее</TabsTrigger>
                </TabsList>
        ...
        <>
            <div className="space-y-4">
                 <div className="flex justify-between items-center">
                    <header>
                        <h1 className="text-base font-bold font-headline">Справочник размеров</h1>
                     ...
                     <TabsContent value="kids" className="mt-6">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Раздел для детских размерных сеток находится в разработке.</CardDescription>
                            </CardHeader>
                        </Card>
                     </TabsContent>
                </Tabs>
            ...
            {/* Clothing Dialogs */}
            {dialogStates['outerwear'] && <FitGuideOuterwearDialog isOpen={dialogStates['outerwear']} onOpenChange={() => closeDialog('outerwear')} audience={activeTab as 'women' | 'men'} />}
            {dialogStates['suits'] && <FitGuideSuitsDialog isOpen={dialogStates['suits']} onOpenChange={() => closeDialog('suits')} audience={activeTab as 'women' | 'men'} />}
            {dialogStates['dresses'] && <FitGuideDialog isOpen={dialogStates['dresses']} onOpenChange={() => closeDialog('dresses')} />}
            {dialogStates['skirts'] && <FitGuideSkirtsDialog isOpen={dialogStates['skirts']} onOpenChange={() => closeDialog('skirts')} />}
            {dialogStates['shirts'] && <FitGuideShirtsDialog isOpen={dialogStates['shirts']} onOpenChange={() => closeDialog('shirts')} audience={activeTab as 'women' | 'men'} />}
            {dialogStates['tops'] && <FitGuideTopsDialog isOpen={dialogStates['tops']} onOpenChange={() => closeDialog('tops')} audience={activeTab as 'women' | 'men'} />}
            {dialogStates['jeans'] && <FitGuideJeansDialog isOpen={dialogStates['jeans']} onOpenChange={() => closeDialog('jeans')} audience={activeTab as 'women' | 'men'} />}
            {dialogStates['trousers'] && <FitGuideTrousersDialog isOpen={dialogStates['trousers']} onOpenChange={() => closeDialog('trousers')} audience={activeTab as 'women' | 'men'} />}
            {dialogStates['knitwear'] && <FitGuideKnitwearDialog isOpen={dialogStates['knitwear']} onOpenChange={() => closeDialog('knitwear')} audience={activeTab as 'women' | 'men'} />}
            {dialogStates['lingerie'] && <FitGuideLingerieDialog isOpen={dialogStates['lingerie']} onOpenChange={() => closeDialog('lingerie')} />}
            {dialogStates['sportswear'] && <FitGuideSportswearDialog isOpen={dialogStates['sportswear']} onOpenChange={() => closeDialog('sportswear')} />}
            {dialogStates['beachwear'] && <FitGuideBeachwearDialog isOpen={dialogStates['beachwear']} onOpenChange={() => closeDialog('beachwear')} />}
            {dialogStates['maternity'] && <FitGuideMaternityDialog isOpen={dialogStates['maternity']} onOpenChange={() => closeDialog('maternity')} />}
            {dialogStates['adaptive'] && <FitGuideAdaptiveDialog isOpen={dialogStates['adaptive']} onOpenChange={() => closeDialog('adaptive')} />}

            {/* Shoes Dialogs */}
            {dialogStates['shoes'] && <FitGuideShoesDialog isOpen={dialogStates['shoes']} onOpenChange={() => closeDialog('shoes')} />}
            {dialogStates['boots'] && <FitGuideBootsDialog isOpen={dialogStates['boots']} onOpenChange={() => closeDialog('boots')} />}
            {dialogStates['flats'] && <FitGuideFlatsDialog isOpen={dialogStates['flats']} onOpenChange={() => closeDialog('flats')} />}
            {dialogStates['sandals'] && <FitGuideSandalsDialog isOpen={dialogStates['sandals']} onOpenChange={() => closeDialog('sandals')} />}
            {dialogStates['sneakers'] && <FitGuideSneakersDialog isOpen={dialogStates['sneakers']} onOpenChange={() => closeDialog('sneakers')} audience={activeTab as 'women' | 'men'} />}
            {dialogStates['homeshoes'] && <FitGuideHomeShoesDialog isOpen={dialogStates['homeshoes']} onOpenChange={() => closeDialog('homeshoes')} />}

             {/* Other Dialogs */}
            {dialogStates['bags'] && <FitGuideBagsDialog isOpen={dialogStates['bags']} onOpenChange={() => closeDialog('bags')} selectedBagType={selectedBagType} />}
            {dialogStates['accessories'] && <FitGuideAccessoriesDialog isOpen={dialogStates['accessories']} onOpenChange={() => closeDialog('accessories')} selectedAccessory={selectedAccessory} />}
             {dialogStates['misc'] && <FitGuideMiscDialog isOpen={dialogStates['misc']} onOpenChange={() => closeDialog('misc')} selectedCategory={selectedMiscCategory} />}
        </>
    );
}
