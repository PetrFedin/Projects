        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Правила и заготовка под ML. Контракт строк: <code className="text-[10px] bg-muted px-1 rounded">PartnerMarkdownHint</code>.
            </p>
          </div>
        </div>
            ...
            <TableBody>
              {PARTNER_MARKDOWN_HINTS.map((h) => (
                <TableRow key={`${h.sku}-${h.region}-${h.channel}`}>
                  ...
                  <TableCell className="text-sm max-w-[200px]">{h.reason}</TableCell>
          ...
          <Card key={`m-${h.sku}`}>
            <CardHeader className="pb-2">
              <div className="flex justify-between gap-2">
                <CardTitle className="text-sm font-mono">{h.sku}</CardTitle>
                <Badge variant="secondary" className="text-[9px]">
