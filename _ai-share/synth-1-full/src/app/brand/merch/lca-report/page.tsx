      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <Card className="border-emerald-200 bg-emerald-50/20">
          <CardHeader className="py-3">
            ...
            <CardTitle className="text-2xl font-bold text-emerald-700">{averages.score}/100</CardTitle>
          </CardHeader>
        </Card>
        <Card className="border-blue-200 bg-blue-50/20">
          <CardHeader className="py-3">
            <CardDescription className="text-xs">Avg Water Intensity</CardDescription>
            <CardTitle className="text-2xl font-bold text-blue-700">{averages.water} L <span className="text-xs font-normal">/ SKU</span></CardTitle>
          </CardHeader>
        </Card>
        <Card className="border-slate-200 bg-slate-50/20">
          <CardHeader className="py-3">
            <CardDescription className="text-xs">Avg CO₂ Emission</CardDescription>
            <CardTitle className="text-2xl font-bold text-slate-700">{averages.co2} kg <span className="text-xs font-normal">/ SKU</span></CardTitle>
          </CardHeader>
        </Card>
      </div>
                ...
                <TableRow key={r.product.sku}>
                  ...
                  <TableCell className="text-center">
                    <Badge variant="outline" className={
                      r.lca.grade === 'A' ? 'bg-emerald-500 text-white border-none' :
                      r.lca.grade === 'B' ? 'bg-lime-500 text-white border-none' :
                      r.lca.grade === 'C' ? 'bg-yellow-500 text-white border-none' :
                      r.lca.grade === 'D' ? 'bg-orange-500 text-white border-none' :
                      'bg-rose-500 text-white border-none'
                    }>
                      {r.lca.grade}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right font-bold text-xs">{r.lca.totalScore}</TableCell>
