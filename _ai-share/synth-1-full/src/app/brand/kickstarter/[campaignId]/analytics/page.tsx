...
import { Users, Eye, Target, Percent, DollarSign, ChevronLeft, Loader2, Factory, BarChart2, TrendingUp } from 'lucide-react';
import { Button } from '@/components/ui/button';
import Link from 'next/link';
import { ChartContainer, ChartTooltip, ChartTooltipContent } from '@/components/ui/chart';
import { Area, AreaChart, Bar, BarChart as RechartsBarChart, CartesianGrid, LabelList, Pie, PieChart, Cell, XAxis, YAxis, Tooltip } from 'recharts';
...
export default function CampaignAnalyticsPage({ params: paramsPromise }: { params: Promise<{ campaignId: string }> }) {
    const params = use(paramsPromise);
    const [project, setProject] = useState(kickstarterProjects.find(p => p.id === params.campaignId));
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-3">
             ...
             <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
                 <Suspense fallback={<Card className="h-24"><CardContent className="p-4"><Loader2 className="h-5 w-5 animate-spin"/></CardContent></Card>}>
                    <StatCard title="Всего просмотров" value={totalViews.toLocaleString('ru-RU')} description="+15% за неделю" icon={Eye} />
                 </Suspense>
                 <Suspense fallback={<Card className="h-24"><CardContent className="p-4"><Loader2 className="h-5 w-5 animate-spin"/></CardContent></Card>}>
                    <StatCard title="Предзаказы" value={project.currentQuantity.toString()} description={`${(project.currentQuantity / project.targetQuantity * 100).toFixed(0)}% от цели`} icon={Target} />
                 </Suspense>
                 <Suspense fallback={<Card className="h-24"><CardContent className="p-4"><Loader2 className="h-5 w-5 animate-spin"/></CardContent></Card>}>
                    <StatCard title="Конверсия" value={`${conversionRate.toFixed(2)}%`} description="в предзаказ" icon={Percent} />
                 </Suspense>
                 <Suspense fallback={<Card className="h-24"><CardContent className="p-4"><Loader2 className="h-5 w-5 animate-spin"/></CardContent></Card>}>
                    <StatCard title="Собрано средств" value={`${project.currentRevenue.toLocaleString('ru-RU')} ₽`} description="от всех предзаказов" icon={DollarSign} />
                 </Suspense>
            </div>
                    ...
                    <ChartContainer config={{}} className="h-[300px] w-full">
                        <AreaChart data={dailyData}>
                            <CartesianGrid vertical={false} />
                             <XAxis dataKey="date" tickFormatter={(val) => new Date(val).toLocaleDateString('ru-RU', { day: '2-digit', month: '2-digit' })} tickLine={false} axisLine={false} />
                             <YAxis yAxisId="left" orientation="left" stroke="hsl(var(--chart-1))" tickFormatter={(val) => `${Number(val) / 1000}k`}/>
                             <YAxis yAxisId="right" orientation="right" stroke="hsl(var(--chart-2))" />
                            <Tooltip content={<ChartTooltipContent formatter={(value, name) => `${Number(value).toLocaleString('ru-RU')}`} />} />
                            <Area yAxisId="left" dataKey="views" name="Просмотры" type="monotone" fill="hsl(var(--chart-1))" fillOpacity={0.1} stroke="hsl(var(--chart-1))" />
                            <Area yAxisId="right" dataKey="orders" name="Заказы" type="monotone" fill="hsl(var(--chart-2))" fillOpacity={0.1} stroke="hsl(var(--chart-2))" />
                        </AreaChart>
                    </ChartContainer>
                </CardContent>
            </Card>
                    ...
                    <CardHeader>
                     ...
                     <CardContent className="h-[250px] w-full">
                        <div className="relative w-full h-full overflow-hidden rounded-md">
                            <Image src="https://i.imgur.com/gKjGZAn.png" alt="Карта предзаказов" layout="fill" objectFit="cover" />
                        </div>
                    </CardContent>
                </Card>
