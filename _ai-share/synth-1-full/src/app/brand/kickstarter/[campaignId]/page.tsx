...
import { Users, Clock, ShoppingCart, Target, Wand2, Loader2, Check, CheckCircle, Share2, Lock } from 'lucide-react';
...
const mockComments = [
    { author: 'Анна', text: 'Очень жду! Выглядит потрясающе.', avatar: 'https://picsum.photos/seed/comment1/40/40' },
    { author: 'Михаил', text: 'Будут ли другие цвета?', avatar: 'https://picsum.photos/seed/comment2/40/40' },
];
                    ...
                    <Tabs defaultValue="story">
                        ...
                        <TabsContent value="updates" className="mt-6 space-y-6">
                            {updates.map((update, index) => (
                                <Card key={index} className={cn(update.forBackersOnly && "border-primary/20 bg-primary/5")}>
                                    <CardHeader>
                        ...
                        <TabsContent value="comments" className="mt-6 space-y-6">
                             ...
                             <div className="flex items-start gap-3 pt-6 border-t">
                                 <Avatar><AvatarFallback>В</AvatarFallback></Avatar>
                    ...
                    <div className="sticky top-24 space-y-6">
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-primary">{project.currentRevenue.toLocaleString('ru-RU')} ₽</p>
                                    <p className="text-sm text-muted-foreground">собрано из {project.targetQuantity.toLocaleString('ru-RU')} шт. цели ({progress.toFixed(0)}%)</p>
                                </div>
                                <Separator />
                                        ...
                                        <CardContent className="p-4">
                                            ...
                                            <ul className="text-xs mt-3 space-y-1">
                                                {(tier.rewards || []).map((reward: string) => (
                                                    <li key={reward} className="flex items-center gap-2"><Check className="h-4 w-4 text-green-500" /> {reward}</li>
                                                ))}
                                            </ul>
                                    ...
                                    return (
                                        <div key={index}>
                                            <div className="flex justify-between items-baseline mb-1">
                                                <p className="font-semibold text-sm">{goal.target.toLocaleString('ru-RU')} шт.</p>
                                                <p className={cn("text-xs font-medium", goal.achieved ? "text-green-600" : "text-muted-foreground")}>
