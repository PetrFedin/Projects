...
export const productEditSchema = z.object({
  ...
  markup: z.preprocess(val => val === '' ? undefined : Number(val), z.number().min(0, "Наценка не может быть отрицательной").optional().or(z.literal(''))),
  ...
  availableColors: z.array(z.object({
    ...
    discounts: z.array(z.object({
      percentage: z.preprocess(val => val === '' ? undefined : Number(val), z.number().min(0).max(100)),
      startDate: z.string(),
      endDate: z.string(),
    })).optional(),
    sizeAvailability: z.array(z.object({
      ...
      quantity: z.preprocess(val => val === '' ? undefined : Number(val), z.number().min(0).optional()),
  ...
  composition: z.array(z.object({
    ...
    percentage: z.preprocess(
        (val) => (val === '' ? undefined : Number(val)),
        z.number().min(0, "Процент не может быть больше 100").max(100, "Процент не может быть больше 100").optional().or(z.literal(''))
    ),
  })).optional(),
  ...
  clothing: z.object({
    ...
    footwear: z.object({
        heelHeight: z.preprocess((val) => val === '' ? undefined : Number(val), z.number().optional().or(z.literal(''))),
