...
export default function PreOrdersPage() {
    ...
    const filteredOrders = preOrdersData.filter(order => {
        const collectionMatch = filterCollection === 'all' || order.product?.season === filterCollection;
    ...
    return (
        <div className="space-y-4">
            <SectionInfoCard
                ...
                badges={<><Badge variant="outline" className="text-[9px]">B2C → Production</Badge><Button variant="outline" size="sm" className="text-[9px] h-7 ml-1" asChild><Link href="/brand/b2b-orders">B2B Заказы</Link></Button><Button variant="outline" size="sm" className="text-[9px] h-7" asChild><Link href="/brand/production">Production</Link></Button></>}
            />
            <header>
                <h1 className="text-base font-bold font-headline">Предзаказы</h1>
                <p className="text-muted-foreground">Управляйте запросами на предзаказ от клиентов и связывайтесь с ними для оформления покупки.</p>
            </header>
                                ...
                                <TableRow key={order.id}>
                                    <TableCell>
                                        <div className="flex items-center gap-3">
                                            <Image src={order.product?.images[0].url || ''} alt={order.product?.name || ''} width={40} height={50} className="rounded-md object-cover" />
                                            <div>
                                    ...
                                    <TableCell>
                                        <Badge variant="outline" className="flex items-center gap-1.5 w-fit">
                                            <span className={cn("h-2 w-2 rounded-full", statusConfig[order.status as keyof typeof statusConfig].color)}></span>
