...
import { ChevronLeft, File, Truck, MoreVertical, CheckCircle, Clock, FileText, Edit, Copy, XCircle, Send, Paperclip } from "lucide-react";
        ...
        <div className="space-y-4">
             <div className="flex items-center gap-3 mb-8">
                <Button variant="outline" size="icon" asChild>
                    <Link href="/shop/b2b-orders"><ChevronLeft className="h-4 w-4" /></Link>
                </Button>
                <h1 className="text-sm font-semibold tracking-tight">
                    Заказ у бренда <span className="text-muted-foreground">Syntha</span> / <span className="font-mono text-muted-foreground">{params.orderId}</span>
                </h1>
                <Badge variant="outline" className="ml-2">Согласован</Badge>
                ...
                <div className="md:col-span-2 space-y-4">
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Заказ № {params.orderId} от {new Date().toLocaleDateString('ru-RU')}</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                <TableHeader>
                                ...
                                <TableBody>
                                    {orderItems.map((item, index) => (
                                        <TableRow key={`${item.id}-${index}`} className="hover:bg-muted/50">
                                            ...
                                            <TableCell className="text-right">{(item.price * 0.4 * item.orderedQuantity).toLocaleString('ru-RU')} ₽</TableCell>
