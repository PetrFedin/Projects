...
import { DollarSign, Search, ShoppingCart, BarChart as BarChartIcon, Users, TrendingUp, AlertTriangle } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
import { Area, AreaChart, CartesianGrid, XAxis, YAxis, Bar, BarChart as RechartsBarChart, Cell } from "recharts";
...
export default function ShopAnalyticsPage() {
    return (
        <div className="space-y-4">
             <header>
                <h1 className="text-base font-bold font-headline">Аналитика розничных продаж</h1>
                <p className="text-muted-foreground">Анализируйте розничные продажи, чтобы принимать верные решения о закупках.</p>
            </header>
                ...
                <CardContent>
                    <ChartContainer config={chartConfig} className="h-[250px] w-full">
                        <AreaChart data={salesData}>
                            ...
                            <XAxis dataKey="date" tickFormatter={(val) => new Date(val).toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' })} tickLine={false} axisLine={false} />
                            <YAxis tickFormatter={(val) => `${Number(val) / 1000}k`} tickLine={false} axisLine={false} />
                            <ChartTooltip content={<ChartTooltipContent formatter={(value) => `${Number(value).toLocaleString('ru-RU')} ₽`} />} />
                            <Area dataKey="revenue" type="monotone" fill="hsl(var(--chart-1))" fillOpacity={0.4} stroke="hsl(var(--chart-1))" />
                        </AreaChart>
                    </ChartContainer>
                </CardContent>
            </Card>
                            ...
                            {lostSalesData.map(item => (
                                <TableRow key={item.query}>
                                    ...
                                    <TableCell className="text-right">
                                        <Button variant="outline" size="sm">Найти у брендов</Button>
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
