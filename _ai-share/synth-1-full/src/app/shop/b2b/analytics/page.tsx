...
import { BarChart, Smile, Frown, TrendingUp, AlertTriangle, Flame, Filter, Search, Camera, FileOutput, Sparkles, Download, FileSpreadsheet } from 'lucide-react';
...
import { Bar as RechartsBar, BarChart as RechartsBarChart, Cell, Pie, PieChart, XAxis, YAxis } from 'recharts';
import { Combobox } from '@/components/ui/combobox';


const abcData = [
    { sku: 'SYN-K001-BLK', product: 'Кашемировый свитер', abc: 'A', xyz: 'X', revenueShare: 25, salesStability: 'Стабильный', season: 'FW24', brand: 'Syntha', category: 'Трикотаж', audience: 'Унисекс' },
    { sku: 'SYN-C004-BEI', product: 'Легендарный тренч', abc: 'A', xyz: 'Y', revenueShare: 20, salesStability: 'Колеблющийся', season: 'FW24', brand: 'Syntha', category: 'Верхняя одежда', audience: 'Женский' },
    { sku: 'ACN-J013-BLU', product: 'Широкие джинсы', abc: 'B', xyz: 'X', revenueShare: 12, salesStability: 'Стабильный', season: 'SS24', brand: 'Acne Studios', category: 'Джинсы', audience: 'Женский' },
    { sku: 'APC-T001-WHT', product: 'Хлопковая футболка', abc: 'B', xyz: 'X', revenueShare: 10, salesStability: 'Стабильный', season: 'SS24', brand: 'A.P.C.', category: 'Топы и футболки', audience: 'Мужской' },
    { sku: 'SYN-S011-BLK', product: 'Кожаные пенни-лоферы', abc: 'C', xyz: 'Z', revenueShare: 5, salesStability: 'Случайный', season: 'FW24', brand: 'Syntha', category: 'Обувь', audience: 'Мужской' },
];
...
const initialMarketComparisonData = [
    { brand: 'Syntha', yourMargin: 65.2, yourSellThrough: 85, marketMargin: 62.5, marketSellThrough: 80, season: 'FW24' },
    { brand: 'A.P.C.', yourMargin: 61.5, yourSellThrough: 78, marketMargin: 64.0, marketSellThrough: 75, season: 'SS24' },
    { brand: 'Acne Studios', yourMargin: 68.0, yourSellThrough: 85, marketMargin: 67.0, marketSellThrough: 85, season: 'SS24' },
];
...
export default function B2BAnalyticsPage() {
    const allBrandProducts = useMemo(() => allMockProducts.filter(p => ['Syntha', 'A.P.C.', 'Acne Studios'].includes(p.brand)), []);
    ...
    const filteredAbcData = useMemo(() => {
        ...
        return abcData.filter(item => {
            ...
            const searchMatch = searchQuery === '' || item.product.toLowerCase().includes(searchQuery.toLowerCase()) || item.sku.toLowerCase().includes(searchQuery.toLowerCase());
            const categoryMatch = category1Filter === 'all' || (product?.category === category1Filter && (category2Filter === 'all' || product?.subcategory === category2Filter));
    ...
    }, [allBrandProducts, seasonFilter, brandFilter, searchQuery, category1Filter, category2Filter, category3Filter, audienceFilter, mounted]);
    ...
    return (
        <div className="space-y-4">
             <header className="flex justify-between items-end">
                <div>
                    <h1 className="text-base font-bold font-headline">Аналитика закупок</h1>
                    <p className="text-muted-foreground">Глубокий анализ SKU для принятия решений о закупках.</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Классификация товаров по вкладу в выручку и стабильности спроса.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader>
                                ...
                                {filteredAbcData.map(item => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <TableCell><Badge variant={item.abc === 'A' ? 'default' : (item.abc === 'B' ? 'secondary' : 'outline')}>{item.abc}</Badge></TableCell>
                                            <TableCell><Badge variant="outline">{item.xyz}</Badge></TableCell>
                 ...
                 <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Анализ Sell-Through и маржинальности по брендам за выбранный период.</CardDescription>
                    </CardHeader>
                    <CardContent>
                       <Table>
                            <TableHeader>
            ...
            <SkuAnalytics brandProducts={allBrandProducts} initialSku={selectedSkuId} onSkuChange={setSelectedSkuId} />
                   ...
                   <Table>
                        ...
                        <TableBody>
                            {filteredMarketComparisonData.map(item => (
                                <TableRow key={item.brand}>
                                    <TableCell className="font-medium">{item.brand}</TableCell>
                                    <TableCell className={item.yourMargin > item.marketMargin ? 'text-green-600' : 'text-red-600'}>{item.yourMargin.toFixed(1)}%</TableCell>
                                    <TableCell>{item.marketMargin.toFixed(1)}%</TableCell>
                                    <TableCell className={item.yourSellThrough > item.marketSellThrough ? 'text-green-600' : 'text-red-600'}>{item.yourSellThrough}%</TableCell>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                 <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><Search className="h-5 w-5 text-accent"/> Упущенные продажи</CardTitle>
                        <CardDescription>Товары, которые пользователи искали, но не нашли в вашем ассортименте.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-amber-500"/> "Потерянные" размеры</CardTitle>
                        <CardDescription>Размеры, которые закончились и которые чаще всего запрашивают клиенты.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader>
             ...
             <Card>
                <CardHeader>
                    ...
                    <CardDescription>Общий процент возвратов по вашим розничным продажам: 8%.</CardDescription>
                </CardHeader>
                <CardContent>
