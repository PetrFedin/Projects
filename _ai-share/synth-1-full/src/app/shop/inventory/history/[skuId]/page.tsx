...
const mockHistory = [
    ...
    { event: 'Цена изменена', date: '2024-07-15 11:30', user: 'Бренд "Syntha"', details: 'Старая цена: 22000 ₽, Новая цена: 24500 ₽' },
    { event: 'Запрос на скидку 20%', date: '2024-07-20 09:00', user: 'Магазин "Podium"', status: 'отклонен брендом' },
    { event: 'Товар архивирован', date: '2024-07-25 18:00', user: 'Автоматически (нет в наличии)', details: 'Остаток 0 более 30 дней' },
...
export default function SkuHistoryPage({ params: paramsPromise }: { params: Promise<{ skuId: string }> }) {
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-3">
                <Button variant="outline" size="icon" asChild>
                    <Link href="/shop/inventory"><ChevronLeft className="h-4 w-4" /></Link>
                </Button>
                <div>
                    <h1 className="text-sm font-semibold tracking-tight">
                        История действий: {product.name}
                    </h1>
                            ...
                            return (
                                <li key={index} className="mb-10 ml-6">            
                                    ...
                                    <h3 className="mb-1 flex items-center text-base font-semibold text-gray-900 dark:text-white">
                                        {item.event} 
                                        {item.status && <span className="bg-red-100 text-red-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full dark:bg-red-900 dark:text-red-300 ml-3">{item.status}</span>}
                                    </h3>
                                    <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">{new Date(item.date).toLocaleString('ru-RU')}</time>
                                    <p className="text-sm text-muted-foreground">Инициатор: {item.user}</p>
                                    {item.details && <p className="mt-1 text-sm">Детали: <span className="font-semibold">{item.details}</span></p>}
                                </li>
                            )
                        })}
                    </ol>
