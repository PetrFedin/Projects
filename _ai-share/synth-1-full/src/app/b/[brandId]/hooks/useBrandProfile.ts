...
export function useBrandProfile(params: any, isPreview: boolean, initialDisplaySettings?: any) {
    ...
    const [b2bPartnerStatus, setB2bPartnerStatus] = useState<'none' | 'pending' | 'friend' | 'active' | 'spot'>('none');
        ...
        async function fetchData() {
            ...
            try {
                const foundBrand = brands.find(b =>
                    b.slug === brandId || b.slug?.toLowerCase() === brandId?.toLowerCase() ||
                    b.slug?.includes?.(brandId) || b.id?.includes?.(brandId)
                ) ?? brands[0];
                if (cancelled) return;
