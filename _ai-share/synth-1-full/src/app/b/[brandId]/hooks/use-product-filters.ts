...
export function useProductFilters(brandProducts: Product[]) {
    const [activeAudience, setActiveAudience] = useState<AudienceFilter[]>(['Взрослые - Женщины', 'Взрослые - Мужчины', 'Взрослые - Унисекс', 'Дети - Девочки', 'Дети - Мальчики', 'Дети - Унисекс']);
    ...
    const filteredProducts = useMemo(() => {
        return brandProducts.filter(p => {
            ...
            if (!activeAudience.includes('Все')) {
                ...
                if (activeAudience.includes('Взрослые - Женщины') && (p.audience === 'Женский' || p.audience === 'Унисекс')) matchesAudience = true;
                if (activeAudience.includes('Взрослые - Мужчины') && (p.audience === 'Мужской' || p.audience === 'Унисекс')) matchesAudience = true;
                if (activeAudience.includes('Взрослые - Унисекс') && p.audience === 'Унисекс') matchesAudience = true;
                if (activeAudience.includes('Дети - Девочки') && (p.audience === 'Девочки' || p.audience === 'Детский' || p.audience === 'Унисекс')) matchesAudience = true;
                if (activeAudience.includes('Дети - Мальчики') && (p.audience === 'Мальчики' || p.audience === 'Детский' || p.audience === 'Унисекс')) matchesAudience = true;
                if (activeAudience.includes('Дети - Унисекс') && (p.audience === 'Детский' || p.audience === 'Унисекс')) matchesAudience = true;
            ...
            if (filterOutlet.length > 0) {
                ...
                if (filterOutlet.includes('outlet') && !filterOutlet.includes('marketplace') && !isOutlet) return false;
                if (filterOutlet.includes('marketplace') && !filterOutlet.includes('outlet') && isOutlet) return false;
            }

            // 3. Category
            if (filterCategory.length > 0) {
                const isMatch = filterCategory.includes(p.category) || 
    ...
    }, [brandProducts, activeAudience, filterOutlet, filterCategory, filterColor, filterAttributes, selectedSizeRow, filterAvailability]);
