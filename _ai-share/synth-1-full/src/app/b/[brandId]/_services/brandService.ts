...
export const getStoryImages = (brandProducts: Product[] = []): ImagePlaceholder[] => {
    const images = PlaceHolderImages.filter(img => img.id.startsWith("story-") || img.id.startsWith("ugc-")).slice(0, 4);
    return images.map((img, idx) => ({
        ...img,
        description: idx === 0 ? "Наши бестселлеры: идеальная посадка и качество." :
                     idx === 1 ? "LIVE: Как мы создаем коллекции." :
                     idx === 2 ? "Новая капсула в деталях." :
...
export const filterBrandProducts = (
    ...
    return brandProducts.filter(p => {
        ...
        if (filterAvailability.length > 0 && !filterAvailability.includes(p.availability || 'in_stock')) return false;
        return true;
    });
};
...
export const getFilteredMediaData = (brand: any) => {
    return {
        live: (brand?.events?.filter((e: any) => e.type === 'Прямой эфир') || []).map((e: any, i: number) => ({
            id: e.id || `live-${i}`,
            title: e.title,
            date: e.date,
            status: new Date(e.date) > new Date() ? 'planned' : 'live',
            imageUrl: e.imageUrl || 'https://images.unsplash.com/photo-1492707892479-7bc8d5a4ee93?w=800&q=80'
        })),
        blog: brand?.articles?.filter((a: any) => a.type === 'blog') || [],
        events: (brand?.events?.filter((e: any) => e.type !== 'Прямой эфир') || []).map((e: any, i: number) => ({
            id: e.id || `event-${i}`,
            title: e.title,
            date: e.date,
            type: e.type,
            imageUrl: e.imageUrl || 'https://images.unsplash.com/photo-1540575861501-7cf05a4b125a?w=800&q=80',
            location: e.location || 'Москва'
        })),
        social: brand?.socials_data || [],
        video: brand?.videos || [],
        customerMentions: brand?.mentions || [],
        press: brand?.articles?.filter((a: any) => a.type === 'press').map((a: any) => ({
            ...a,
