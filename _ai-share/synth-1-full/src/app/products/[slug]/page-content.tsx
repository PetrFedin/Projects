...
import { Heart, ShoppingCart, Share2, Sparkles, View, Bell, Minus, Plus, Check, Repeat, Users, Clock, Star, Scale, PlusCircle, Droplets, Info, Truck, Undo2, Shirt, Glasses, Footprints } from 'lucide-react';
...
export default function ProductPageContent({ product, isQuickView = false }: { product: Product, isQuickView?: boolean }) {
  ...
  const details = [
    ...
    { label: 'Состав', value: composition.map(c => `${c.material} ${c.percentage}%`).join(', ') || 'не указан' },
  ...
  return (
    <>
      <div className={cn("grid gap-3 lg:gap-3 p-4 md:p-4", isQuickView ? 'grid-cols-1 md:grid-cols-2' : 'md:grid-cols-2')}>
        {/* Image Gallery */}
          ...
          <div className="relative aspect-[4/5] w-full rounded-lg overflow-hidden border cursor-pointer" onClick={() => handleOpenImageViewer(imagesForCurrentColor.findIndex(img => img.id === activeImage.id))}>
            ...
            />
             <div className="absolute top-3 left-3 flex items-center gap-2">
                {product.isPromoted && <Badge variant="default" className="text-sm bg-accent text-accent-foreground">Промо</Badge>}
        ...
        <div className="py-4">
          ...
          <h1 className="text-base lg:text-sm font-headline font-bold mt-1">{product.name}</h1>
          ...
          <div className="mt-4 flex items-center gap-3">
                ...
                <div className="flex items-center">
                    {[...Array(5)].map((_, i) => <Star key={i} className={`h-5 w-5 ${i < Math.round(averageRating) ? 'text-yellow-400 fill-yellow-400' : 'text-muted-foreground/30'}`} />)}
                </div>
          ...
          <div className="flex items-center gap-2 mt-8">
            {selectedSize ? (
                ...
                <Button size="lg" className="flex-1" onClick={handleAddToCart} disabled={allSizes.length > 0}>
                    <ShoppingCart className="mr-2 h-5 w-5"/> {isInCart ? "Добавлено" : "Добавить в корзину" }
                </Button>
            )}
             <Popover>
                <PopoverTrigger asChild>
                   <Button size="icon" variant="outline" className="h-11 w-11">
                    <Star className={cn("h-5 w-5", isInWishlist && 'fill-amber-500 text-amber-500')} />
                  </Button>
                </PopoverTrigger>
             ...
             <Button size="icon" variant="outline" className="h-11 w-11" onClick={() => toggleComparisonItem(product)}>
      ...
      {!isQuickView && (
        <div className="container mx-auto px-4">
            <div className="my-16">
                 <Button variant="link" className="p-0 h-auto" onClick={() => setIsSimilarProductsOpen(true)}>
                    <h2 className="text-base font-headline font-bold text-center mb-8 flex items-center gap-2">
            ...
            {/* Related Products */}
            <div className="my-16">
                <h2 className="text-base font-headline font-bold text-center mb-8">С этим также покупают</h2>
      ...
      {isSizeGuideOpen && (
          <SizeGuideDialog product={product} isOpen={isSizeGuideOpen} onOpenChange={setIsSizeGuideOpen} />
      )}
      {isSimilarProductsOpen && (
          <SimilarProductsDialog product={product} isOpen={isSimilarProductsOpen} onOpenChange={setIsSimilarProductsOpen} />
      )}
      {isCompatibilityOpen && (
          <WardrobeCompatibilityDialog product={product} isOpen={isCompatibilityOpen} onOpenChange={setIsCompatibilityOpen} />
      )}
      {isBestsellerOpen && (
          <BestsellerRankingDialog product={product} isOpen={isBestsellerOpen} onOpenChange={setIsBestsellerOpen} />
      )}
