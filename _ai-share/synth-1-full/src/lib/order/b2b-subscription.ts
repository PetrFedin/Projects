...
export class SubscriptionEngine {
  ...
  public static async processSubscriptions(currentDateStr: string): Promise<OrderAggregate[]> {
    ...
    const activeSubs = this.subscriptions.filter(
      (s) => s.status === 'active' && s.nextRunDate <= currentDateStr
    );

    console.log(
      `[SubscriptionEngine] Processing ${activeSubs.length} subscriptions for ${currentDateStr}...`
    );
    ...
    for (const sub of activeSubs) {
      ...
      console.log(
        `[SubscriptionEngine] Generated order ${orderId} for subscription ${sub.id}. Next run: ${sub.nextRunDate}`
      );
    }
