...
export const runVisualQA = (componentName: string, content: string): QAReport => {
  ...
  report.checks.push({
    ...
    message: spacingCheck
      ? 'Spacing follows design system scales'
      : 'Inconsistent spacing detected (e.g. p-10)',
  });
  ...
  report.checks.push({
    ...
    message: typographyCheck
      ? 'Hierarchy is clear'
      : 'Oversized text detected without proper headline utility',
  });
  ...
  if (content.includes('<Table')) {
    ...
    report.checks.push({
      ...
      message: tableCheck
        ? 'Table row height is optimized for density'
        : 'Table rows might be too large',
    });
  }
