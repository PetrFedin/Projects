...
export function findSubcategories(parentCategory: string, structure: any): string[] {
    ...
    const findChildren = (currentCategory: string, currentStructure: any) => {
        for (const key in currentStructure) {
            ...
            if (typeof currentStructure[key] === 'object' && currentStructure[key] !== null && !Array.isArray(currentStructure[key])) {
...
function filterTreeForAudience(structure: any, audience: 'Женский' | 'Мужской'): any {
  ...
  const menExclusions = ["Платья и сарафаны", "Юбки", "Бюстгальтеры", "Коктейльные", "Вечерние", "Балетки", "Ботфорты", "Кроп-топы", "Купальники", "Парео", "Пляжные платья", "Береты", "Клатч", "Тот", "Шоппер", "Вечерняя", "Bootcut", "Flare"];
 ...
 */
export function getFilteredCategoryStructure(audience: string, fullCategoryStructure: CategoryStructure): CategoryStructure {
  switch (audience) {
    case 'Женский':
