...
export const refineLayoutClasses = (classes: string, options: RefinementOptions = { density: 1, hierarchy: 1, clutterReduction: 1 }): string => {
    let refined = classes;
    
    // 1. Spacing Normalization
    refined = refined.split(' ').map(cls => normalizeSpacing(cls)).join(' ');
    
    // 2. Typography Normalization
    refined = refined.split(' ').map(cls => normalizeTypography(cls)).join(' ');
    
    // 3. Radius Normalization
    refined = refined.split(' ').map(cls => normalizeRadius(cls)).join(' ');
...
export const refineToolbar = (classes: string): string => {
    return refineLayoutClasses(classes + " flex items-center justify-between gap-2 p-1 bg-slate-100 border border-slate-200 rounded-xl shadow-inner", { density: 1, hierarchy: 1, clutterReduction: 1 });
};

export const refineTableRows = (classes: string): string => {
    return refineLayoutClasses(classes + " h-10 border-b border-slate-50 hover:bg-slate-50/50 transition-all group", { density: 1, hierarchy: 1, clutterReduction: 1 });
};
