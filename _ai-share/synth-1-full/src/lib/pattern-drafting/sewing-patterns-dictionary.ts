import {
  sewingPatternsMessages as ru,
  type SewingPatternsMessages,
} from '@/lib/pattern-drafting/sewing-patterns-messages';
...
export const sewingPatternsMessagesEn: SewingPatternsMessages = {
  ...
  formDescriptionApparel:
    'cm, decimals with a dot. Height affects FWL. Step 1 category set ease; adjust manually.',
  ...
  commitIntentSuccessSimple: (pathLabel) =>
    `${pathLabel} — leafId verified against the handbook snapshot.`,
  ...
  commitIntentHint:
    'Category + measures are checked against the category-handbook on the server (anti-spoof).',
  ...
  metricsBlockDescription:
    'Draft numbers for the selected part: to compare with the brand and size charts.',
...
const dict: Record<SewingPatternsLocale, SewingPatternsMessages> = {
  ru,
  en: sewingPatternsMessagesEn,
};
