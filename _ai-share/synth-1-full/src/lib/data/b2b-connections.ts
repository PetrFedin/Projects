...
export type B2BRole = 'buyer' | 'brand_manager' | 'sales_rep' | 'distributor_agent' | 'production' | 'finance' | 'admin';

/** Ключевые группы навигации по профилю */
export const PROFILE_NAV_GROUPS: Record<AppProfile, string[]> = {
  brand: [
    'org', 'logistics', 'compliance', 'overview', 'product',
    'b2b_wholesale', 'b2c_retail', 'production', 'marketing', 'content',
    'analytics', 'finance', 'protection', 'auctions', 'ai-tools', 'esg',
    'collabs', 'hr', 'academy', 'comm',
  ],
  shop: [
    'overview', 'retail', 'b2b', 'partners', 'analytics', 'management',
  ],
  distributor: [
    'overview', 'staff', 'calendar', 'messages', 'brands', 'matrix',
    'orders', 'contracts', 'retailers', 'analytics', 'commissions', 'vmi', 'settings',
  ],
  factory: [
    'management', 'production', 'resources', 'settings',
  ],
  admin: [
    'overview', 'users', 'content', 'commerce', 'analytics', 'settings', 'integrations',
  ],
...
export const B2B_ROLE_SECTIONS: Record<B2BRole, string[]> = {
  buyer: [
    'order-mode', 'create-order', 'quick-order', 'order-by-collection', 'order-templates', 'collaborative-order', 'order-drafts', 'collection-terms', 'reorder', 'matrix', 'assortment-planning', 'ez-order', 'working-order',
    'b2b-orders', 'order-analytics', 'fulfillment-dashboard', 'margin-report', 'margin-calculator', 'multi-currency',
    'size-mapping', 'trade-shows', 'passport', 'discover', 'payment', 'tracking',
    'video-consultation', 'vip-room-booking', 'shopify-sync', 'social-feed', 'gamification', 'store-locator', 'partner-onboarding',
    'b2b-apply', 'partners', 'replenishment', 'smart-replenishment', 'shopping-lists', 'dealer-cabinet', 'ai-smart-order',
    'multiple-carts', 'product-favorites', 'assortment-curation', 'product-banners', 'volume-pack-rules', 'ai-search',
  ],
  brand_manager: [
    'b2b-orders', 'trade-shows', 'passport', 'buyer-applications', 'linesheet-campaigns', 'linesheets', 'linesheet-versions',
    'showroom', 'retailers', 'distributors', 'territory', 'pre-order-quota', 'commissions',
    'credit-risk', 'last-call', 'shopify-sync', 'po', 'shipments', 'order-approval-workflow', 'engagement',
    'embedded-payment', 'integrations-erp-plm', 'calendar', 'tasks', 'events', 'messages',
  ],
  sales_rep: [
    'retailers', 'b2b-orders', 'trade-shows', 'buyer-applications', 'commissions',
    'messages', 'calendar', 'tasks',
  ],
  distributor_agent: [
    'orders', 'retailers', 'brands', 'matrix', 'analytics', 'commissions', 'vmi',
    'territory', 'pre-order-quota', 'calendar', 'messages',
  ],
  production: [
    'production', 'b2b-orders', 'factories', 'materials', 'suppliers', 'vmi',
    'calendar', 'tasks', 'warehouse',
  ],
  finance: [
    'finance', 'b2b-orders', 'escrow', 'commissions', 'budget-actual', 'analytics-bi',
    'disputes', 'returns-claims', 'calendar',
  ],
...
export function getContextLinks(context: keyof typeof CONTEXT_TO_ROUTES): { label: string; href: string; entityType: 'order' | 'task' | 'event' | 'partner' | 'production' | 'product' | 'escrow' | 'dispute' }[] {
