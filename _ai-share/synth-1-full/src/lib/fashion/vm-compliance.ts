...
export function getVmComplianceScore(
  sku: string,
  storeId: string = 'STORE-MOSCOW-01'
): VmComplianceV1 {
  ...
  const status: VmComplianceV1['photoReportStatus'] =
    score > 90 ? 'approved' : score > 80 ? 'pending' : 'rejected';

  const notes =
    status === 'approved'
      ? ['Perfect alignment with SS26 Planogram']
      : ['Color blocking mismatch', 'Incorrect hanger type'];
