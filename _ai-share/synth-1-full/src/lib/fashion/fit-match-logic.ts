  ...
  return garmentSizes.map(gs => {
    ...
    gs.measurements.forEach(m => {
      ...
      if (m.label.includes('груди') && profile.chest) {
        ...
        if (diff < 2) { score -= 30; notes.push('Будет тесно в груди'); }
        else if (diff > 12) { score -= 15; notes.push('Оверсайз в груди'); }
      }

      if (m.label.includes('талии') && profile.waist) {
        const diff = (val * 2) - profile.waist;
        if (diff < 0) { score -= 40; notes.push('Мало в талии'); }
        else if (diff > 15) { score -= 10; notes.push('Свободно в талии'); }
      }

      if (m.label.includes('бёдер') && profile.hips) {
        const diff = (val * 2) - profile.hips;
        if (diff < 2) { score -= 30; notes.push('Узко в бёдрах'); }
      }
    });
