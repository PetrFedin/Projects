...
const RULES: { slot: FashionSlot; patterns: RegExp[] }[] = [
  { slot: 'footwear', patterns: [/обув/i, /ботин/i, /кроссов/i, /туфл/i, /лофер/i, /shoe/i, /sneaker/i, /boot/i] },
  { slot: 'outer', patterns: [/пальто/i, /куртк/i, /пухов/i, /пиджак/i, /жакет/i, /coat/i, /jacket/i, /blazer/i] },
  { slot: 'dress', patterns: [/плать/i, /сарафан/i, /dress/i, /gown/i] },
  { slot: 'bottom', patterns: [/брюк/i, /джинс/i, /шорт/i, /юбк/i, /брюки/i, /pant/i, /skirt/i, /jean/i] },
  { slot: 'top', patterns: [/футбол/i, /рубаш/i, /свитер/i, /лонгслив/i, /топ/i, /поло/i, /tee/i, /shirt/i, /knit/i, /свитшот/i] },
  { slot: 'accessory', patterns: [/сумк/i, /ремен/i, /шарф/i, /шапк/i, /очк/i, /кепк/i, /пояс/i, /bag/i, /belt/i, /scarf/i, /cap/i] },
];

export function inferFashionSlot(product: Product): FashionSlot {
  const hay = `${product.name} ${product.category} ${product.subcategory ?? ''} ${product.subcategory2 ?? ''}`.toLowerCase();
...
export function analyzeOutfitGaps(products: (Product | null)[]): OutfitGapResult {
  ...
  else if (missing.includes('top') && missing.includes('bottom')) hint = 'Нужны верх и низ (или платье).';
