  ...
  byCategory.forEach((items, cat) => {
    for (let i = 0; i < items.length; i++) {
      for (let j = i + 1; j < items.length; j++) {
        ...
        if (a.color === b.color) { score += 40; reasons.push('Совпадение цвета'); }
        if (Math.abs(a.price - b.price) < a.price * 0.1) { score += 30; reasons.push('Близкая цена'); }
        if (a.material === b.material) { score += 20; reasons.push('Тот же материал'); }
        ...
        if (score >= 60) {
          overlaps.push({
            ...
            reason: reasons.join(', '),
            actionHint: score >= 80 ? 'Каннибализация: рассмотрите вывод одного SKU' : 'Близкие аналоги: разведите по маркетингу',
          });
        }
      }
    }
