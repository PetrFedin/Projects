...
export function loadLookbookProjects(): LookbookProjectV1[] {
  ...
  try {
    ...
    return raw ? JSON.parse(raw) : [
      { id: 'proj-01', title: 'SS26 Early Concept', skus: [], status: 'draft', updatedAt: Date.now() },
      { id: 'proj-02', title: 'Holiday Capsule 25', skus: [], status: 'published', updatedAt: Date.now() - 86400000 },
    ];
  } catch {
    return [];
  }
