...
export const getSizeDetails = (category: string, sizeName: string): SizeDetails => {
  ...
  if (['топы', 'трикотаж', 'рубашки и блузы', 'верхняя одежда', 'платья'].includes(cat)) {
    ...
    if (data) {
      const fit = (sizeName === 'XS' || sizeName === '40') ? 'Slim Fit' : 
                  (sizeName === 'XL' || sizeName === '50') ? 'Oversize' : 'Regular Fit';
      return {
        label: 'ОГ / ОТ / ОБ',
  ...
  if (cat === 'обувь') {
    const shoeMap: Record<string, string> = {
      '36': '23.0', '37': '23.5', '38': '24.5', '39': '25.0', '40': '25.5', 
      '41': '26.0', '42': '26.5', '43': '27.5', '44': '28.0', '45': '28.5'
    };
