...
export class B2BPriorityStrategy implements AllocationStrategy {
  ...
  allocate(batch: IntakeBatch, demand: DemandProfile): AllocationPlan {
    ...
    for (const b2b of demand.b2bBackorders) {
      const stockItem = availableStock.find(i => i.articleId === b2b.articleId && i.size === b2b.size);
    ...
    for (const retail of demand.retailDemand) {
      const stockItem = availableStock.find(i => i.articleId === retail.articleId && i.size === retail.size);
      if (stockItem && stockItem.quantity > 0) {
    ...
    for (const ecom of demand.ecomDemand) {
      const stockItem = availableStock.find(i => i.articleId === ecom.articleId && i.size === ecom.size);
      if (stockItem && stockItem.quantity > 0) {
