...
export function setCurrentBuyerTerritories(territories: string[]) {
  if (typeof window !== 'undefined')
    localStorage.setItem(STORAGE_KEY_TERRITORY, JSON.stringify(territories));
}
...
export function isProductVisibleToBuyer(product: ProductVisibility, rights: BuyerRights): boolean {
  if (product.allowedTerritories && product.allowedTerritories.length > 0) {
    const hasTerritory = product.allowedTerritories.some((t) =>
      rights.allowedTerritories.includes(t)
    );
...
const MOCK_TERRITORIES_BY_INDEX: string[][] = [
  ['Moscow', 'Russia'],
  ['SPb', 'Russia'],
  ['Moscow', 'SPb', 'Russia'],
  [],
...
const MOCK_CHANNELS_BY_INDEX: B2BChannelId[][] = [
  ['wholesale', 'retail_a'],
  ['wholesale', 'retail_b'],
