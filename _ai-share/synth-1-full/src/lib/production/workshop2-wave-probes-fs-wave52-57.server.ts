...
export function buildWorkshop2Wave57PostFreezeLiveProbe(env: Workshop2ProcessEnvLike = process.env) {
  ...
  const checks: FsProbeCheck[] = [
    ...
    { id: 'invoice_pdf_playwright_script', ok: read('scripts/workshop2-invoice-pdf-playwright.mjs').includes('WORKSHOP2_INVOICE_PDF_ENGINE'), hintRu: 'Playwright invoice PDF.' },
