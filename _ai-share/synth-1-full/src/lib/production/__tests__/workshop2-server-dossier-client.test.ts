...
describe('workshop2-server-dossier-client', () => {
  ...
  it('maps save 409 to version_conflict', async () => {
    ...
    global.fetch = jest
      .fn<ReturnType<typeof fetch>, Parameters<typeof fetch>>()
      .mockResolvedValue(makeResponse({ error: 'version_conflict', currentVersion: 5 }, 409)) as unknown as typeof fetch;
    const out = await saveWorkshop2DossierToServer({
      collectionId: 'c1',
      articleId: 'a1',
      dossier: { assignments: [] } as never,
  ...
  it('loads immutable html by snapshot id', async () => {
    ...
    global.fetch = (jest
      .fn<ReturnType<typeof fetch>, Parameters<typeof fetch>>()
      .mockResolvedValue(makeResponse({ ok: true, snapshotId: 'w2-exp-snap-1', html: '<html>immutable</html>' })) as unknown as typeof fetch);
  ...
  it('lists final export snapshots', async () => {
    ...
    global.fetch = (jest
      ...
      .mockResolvedValue(
        makeResponse({
          ok: true,
          snapshots: [{ snapshotId: 'w2-exp-snap-1', createdAt: '2026-04-28T12:00:00.000Z', createdBy: 'Export Bot', dossierVersion: 8 }],
        })
      ) as unknown as typeof fetch);
  ...
  it('loads single final export snapshot meta', async () => {
    ...
    global.fetch = (jest
      ...
      .mockResolvedValue(
        makeResponse({
          ok: true,
          snapshot: { snapshotId: 'w2-exp-snap-1', createdAt: '2026-04-28T12:00:00.000Z', createdBy: 'Export Bot', dossierVersion: 8 },
        })
      ) as unknown as typeof fetch);
  ...
  it('lists dossier audit events', async () => {
    ...
    global.fetch = (jest
      ...
      .mockResolvedValue(
        makeResponse({
          ok: true,
          events: [{ id: '1', collectionId: 'c1', articleId: 'a1', version: 2, eventType: 'merge_auto', eventPayload: {}, createdAt: '2026-04-28T12:00:00.000Z' }],
        })
      ) as unknown as typeof fetch);
  ...
  it('lists dossier versions', async () => {
    ...
    global.fetch = (jest
      ...
      .mockResolvedValue(
        makeResponse({
          ok: true,
          versions: [{ id: '2', collectionId: 'c1', articleId: 'a1', version: 2, createdAt: '2026-04-28T12:00:00.000Z' }],
        })
      ) as unknown as typeof fetch);
