...
const TAB_SOURCE_SECTIONS: Record<Workshop2OperationalPipelineTab, readonly DossierSection[]> = {
...
export function buildWorkshop2OperationalTzBridge(
  ...
  return {
    contractLine,
    overallLine: `Досье в целом: ${r.overall.pct}% · ворота передачи в производство: ${
      r.overall.readyForHandoff ? 'пройдены' : 'не пройдены'
    }.`,
    focusPctLabel: `Связанные разделы ТЗ (среднее по выбранным): ${pct}%`,
    blockerLines: dedup,
  };
}
