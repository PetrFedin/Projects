...
export const COLLECTION_STEPS: CollectionStep[] = [
  {
    ...
    description: 'Сезон, выручка и маржа, аудитория, настроение и ограничения — старт для ассортимента и производства.',
    area: 'Ассортимент',
    mandatory: true,
    canSkipForNow: false,
    dependsOn: [],
  ...
  },
  {
    ...
    description: 'Создание артикулов коллекции: категории, дропы, размерная сетка. Основа для производства всех нужных SKU.',
    area: 'Ассортимент',
    mandatory: true,
    canSkipForNow: false,
    dependsOn: ['brief'],
  ...
  },
  {
    ...
    description: 'Одна точка входа: выбранная коллекция, все артикулы, прогресс и быстрые переходы в производство.',
    area: 'Производство',
    mandatory: false,
    canSkipForNow: true,
  ...
  },
  {
    ...
    description: 'Landed cost, опт/розница, маржа по стилям; фиксация и согласование стоимости сырья и условий производства — до выбора фабрики и календаря серии. Переписка, задачи и слоты в календаре с ответственными.',
    area: 'Финансы',
    mandatory: true,
    canSkipForNow: false,
    dependsOn: ['assortment-map'],
  ...
  },
  {
    ...
    description: 'Ткани и подклад, фурнитура, нормы расхода; кандидаты поставщиков, RFQ, сравнение условий — до фиксации в ТЗ и закупке.',
    area: 'Производство',
    mandatory: true,
    canSkipForNow: true,
    dependsOn: ['assortment-map'],
  ...
  },
  {
    ...
    description: 'Формальное «ок» по ТЗ, материалам, фурнитуре и конструктиву от всех сторон: тред в сообщениях, чек-лист задач, встречи/слоты в календаре с назначенными ответственными. Здесь же фиксируется итог для закупки и семпла (MVP: плюс ручной статус этапа).',
    area: 'Производство',
    mandatory: true,
    canSkipForNow: false,
  ...
  },
  {
    ...
    description: 'Утверждённый поставщик по номенклатуре: заказ и поставка сырья/фурнитуры либо отбор со стока (VMI), контроль прихода под семпл и серию. Согласования сроков и отгрузок — в сообщениях; задачи закупки; в календаре — ответственные по поставке и приёмке.',
    area: 'Производство',
    mandatory: true,
    canSkipForNow: true,
  ...
  },
  {
    ...
    description: 'Пошив образцов по утверждённому ТЗ, перенос лекал в цех семпла, логистика отправки семплов (к кому и когда). Комментарии по примерке и доработкам — в чатах и fit; сроки и ответственные — в календаре и задачах до Gold Sample.',
    area: 'Производство',
    mandatory: true,
    canSkipForNow: false,
  ...
  },
  {
    ...
    description: 'Материалы для байеров параллельно с семплами или после эталона — оптовые условия и презентация коллекции.',
    area: 'B2B',
    mandatory: false,
    canSkipForNow: true,
    dependsOn: ['samples'],
  ...
  },
  {
    ...
    description: 'Выбор фабрики/ателье, мощности и календарь: раскрой, пошив, контрольные точки, готовность партии — до PO. Согласования и назначения ответственных за производство — в сообщениях, задачах и общем календаре платформы (наряду с Gantt цеха).',
    area: 'Производство',
    mandatory: true,
    canSkipForNow: false,
  ...
  },
  {
    ...
    description: 'Формальные заказы по стилям и дропам после согласованных площадки и сроков; загрузка линий и контроль в Gantt.',
    area: 'Производство',
    mandatory: true,
    canSkipForNow: false,
  ...
  },
  {
    ...
    description: 'PO, BOM, QC на уровне операций; переписка с фабрикой и поставщиками, задачи и напоминания — через сообщения, задачи и календарь; сводка и аудит — вкладка «Операции».',
    area: 'Производство',
    mandatory: false,
    canSkipForNow: true,
    dependsOn: ['po'],
  ...
  },
  {
    ...
    description: 'Мобильный ОТК и рабочее место QC: чек-листы, дефекты, статусы партий перед складом.',
    area: 'Производство',
    mandatory: true,
    canSkipForNow: true,
  ...
  },
  {
    ...
    description: 'Входящая приёмка с производства на склад бренда: сверка с PO, качество, остатки по коллекции — до комплектации под опт.',
    area: 'Производство',
    mandatory: true,
    canSkipForNow: false,
    dependsOn: ['qc'],
  ...
  },
  {
    ...
    description: 'Подготовка партий к отгрузке байерам/в магазины клиентов: этикетки, короба, паллеты, соответствие заказам B2B по коллекции.',
    area: 'B2B',
    mandatory: true,
    canSkipForNow: true,
  ...
  },
  {
    ...
    description: 'Исходящие отгрузки по оптовым заказам: ASN/статусы, маршруты до торговых точек партнёров — граница «производство коллекции → клиент».',
    area: 'B2B',
    mandatory: true,
    canSkipForNow: true,
...
export const COLLECTION_BRAND_NARRATIVE_STEP_IDS: readonly string[] = COLLECTION_STEPS.map((s) => s.id);
