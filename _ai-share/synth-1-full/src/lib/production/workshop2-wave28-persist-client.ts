/**
 * Wave 28: PUT досье + localStorage + cross-tab audit.
 */
...
import {
  ...
  type Workshop2Wave28PersistField,
} from '@/lib/production/workshop2-wave28-cross-tab-audit';

export type Workshop2Wave28PersistResult = {
  ok: boolean;
...
export async function putWorkshop2Wave28DossierPatch(input: {
  collectionId: string;
  articleId: string;
  ...
  /**
   * Wave F (#10): не писать LS до успешного PG PUT — иначе silent «success» в local cache.
   */
  pgOnlyFailClosed?: boolean;
}): Promise<Workshop2Wave28PersistResult> {
  const stamped: Workshop2DossierPhase1 = {
    ...input.apply(input.base),
    updatedAt: new Date().toISOString(),
    updatedBy: input.updatedByLabel.slice(0, 120),
  };
  if (input.pgOnlyFailClosed) {
