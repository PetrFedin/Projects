 ...
 */
export function createHttpArticleWorkspacePort(_opts: ArticleWorkspaceHttpPortOptions): ArticleWorkspaceDataPort {
  return {
    mode: 'http',
    ...
    async mergeBundle(_ref: ArticleRef, _patch: Partial<ArticleWorkspaceBundle>): Promise<ArticleWorkspaceBundle> {
      throw new Error(
        'ArticleWorkspace HTTP: не реализовано. Используйте createLocalArticleWorkspacePort() или задайте реализацию в bootstrap.'
      );
    },
