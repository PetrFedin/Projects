...
import { computeSupplyRiskPrediction } from '@/lib/production/workshop2-operational-heuristics';
...
export function dossierBomToSupplyLines(dossier: Workshop2DossierPhase1): SupplySnapshot['lines'] {
  ...
  return model.materialLines.map((line, idx) => ({
    ...
    status: line.materialLibraryId ? 'ordered' : 'draft',
...
export type Workshop2SupplyRiskSnapshot = SupplySnapshot['riskPrediction'] & {
  computedAt: string;
  source: 'dossier_bom';
  inputsUsed?: string[];
};
