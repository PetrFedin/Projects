...
function inferStyleTags(category: string, brand: string, tags: string[], color: string): string[] {
  const result: string[] = [];
  if (brand.toLowerCase().includes('syntha')) result.push('premium');
  if (
    category.includes('пальто') ||
    category.includes('coat') ||
    category.includes('тренч') ||
  ...
  if (category.includes('платок') || category.includes('scarf'))
    result.push('premium', 'statement');
  if (category.includes('лонгслив') || category.includes('longsleeve'))
    result.push('minimal', 'essential');
  if (category.includes('сумк') || category.includes('bag') || category.includes('tote'))
    result.push('city', 'versatile');
...
function libProductToStylist(p: {
  id: string;
  name: string;
  brand: string;
  category: string;
  ...
  const tags = inferStyleTags(p.category, p.brand, (p.tags ?? []) as string[], p.color ?? '');
  const image =
    Array.isArray(p.images) && p.images[0] && 'url' in p.images[0]
...
export function getStylistProductPool(): StylistProduct[] {
  ...
  const fromLib = LIB_PRODUCTS.filter((p) => p.brand.toLowerCase().includes('syntha')).map(
    libProductToStylist
  );
