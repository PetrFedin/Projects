...
export class WMSEngine {
   ...
   */
  public static generatePickingWave(
    orders: Array<{ orderId: string; items: Array<{ sku: string; qty: number }> }>
  ): PickingWave {
    ...
    for (const order of orders) {
      for (const item of order.items) {
        ...
        const zoneId = item.sku.includes('FAST')
          ? 'zone-fast'
          : item.sku.includes('BULKY')
            ? 'zone-bulky'
            : 'zone-slow';
