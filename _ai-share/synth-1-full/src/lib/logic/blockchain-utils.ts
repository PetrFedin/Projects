...
export const MOCK_IP_RECORDS: IPRecord[] = [
    ...
    },
    {
        ...
        owner: 'Syntha Co. LLC',
        attachments: [
            { url: '#', filename: 'pattern_repeat.ai', type: 'source' }
        ],
        metadata: {
    ...
    },
    {
        ...
        owner: 'Syntha Co. LLC',
        attachments: [
            { url: '#', filename: 'tech_pack_full.pdf', type: 'document' }
        ],
        metadata: {
            designer: 'Alex Vetrov',
            collection: 'SS26',
...
export const getIPStatusColor = (status: IPRecord['status']) => {
    switch (status) {
        case 'registered': return 'text-emerald-500 bg-emerald-50 border-emerald-100';
        case 'pending': return 'text-amber-500 bg-amber-50 border-amber-100';
        case 'disputed': return 'text-rose-500 bg-rose-50 border-rose-100';
