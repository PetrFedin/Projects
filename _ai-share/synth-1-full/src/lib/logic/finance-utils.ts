...
export function calculateLandedCost(breakdown: Partial<LandedCostBreakdown>): LandedCostBreakdown {
  ...
  const targetMargin =
    targetRetailPrice > 0 ? ((targetRetailPrice - totalLandedCost) / targetRetailPrice) * 100 : 0;
...
export function generateCostOptimizationInsights(lc: LandedCostBreakdown) {
  ...
  if (lc.calculatedDuty / lc.totalLandedCost > 0.15) {
    insights.push(
      'Высокая доля ввозной пошлины. Рассмотрите возможность пошива в странах с льготным режимом (например, ЕАЭС/СНГ).'
    );
  }

  if (lc.overheadRate > 25) {
    insights.push(
      'Уровень накладных расходов превышает среднерыночный (18-22%). Рекомендуется аудит операционных затрат.'
    );
  }

  if (lc.targetMargin < 65) {
    insights.push(
      'Маржинальность ниже целевой (75%). Рекомендуется поднять Retail Price на 15-20% или оптимизировать стоимость CMT.'
    );
  }
