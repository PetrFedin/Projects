export type SimulationShock = 'port_closure' | 'demand_spike' | 'supplier_bankruptcy' | 'raw_material_shortage';
  ...
  public static runSimulation(
    ...
    switch (scenario.shockType) {
      case 'port_closure':
        ...
        mitigations.push(`Reroute critical SKUs via Air Freight (Estimated cost: $${Math.round(revenueLoss * 0.3)})`);
      ...
      case 'demand_spike':
        ...
        mitigations.push('Increase retail price dynamically by 15% to cool down demand and maximize margin');
        break;

      case 'supplier_bankruptcy':
        // Банкротство ключевой фабрики
        slDrop = 0.25; // Жесткое падение уровня сервиса на 25%
        revenueLoss = currentMonthlyRevenue * 0.20;
        stockoutRisk = 0.60;
      ...
      case 'raw_material_shortage':
        ...
        mitigations.push('Substitute material in BOM (Bill of Materials) with recycled alternatives');
        mitigations.push('Delay launch of upcoming collection to preserve raw materials for core items');
        break;
    }
