...
interface B2BState {
  ...
  createSyncAgreement: (
    agreement: Omit<StockSyncAgreement, 'id' | 'createdAt' | 'updatedAt'>
  ) => void;
...
export function B2BStateProvider({ children }: { children: React.ReactNode }) {
  ...
  const [b2bNegotiations, setB2bNegotiations] = useState<B2BNegotiation[]>([
    {
      ...
      messages: [
        {
          id: 'm1',
          sender: { id: 'shop-001', name: 'Olga', role: 'shop' },
          text: 'Inquiry for Syntha Lab',
          timestamp: new Date().toISOString(),
          type: 'message',
        },
      ],
      ...
      messages: [
        {
          id: 'm3',
          sender: { id: 'shop-001', name: 'Olga', role: 'shop' },
          text: 'Silk Road Production started',
          timestamp: new Date().toISOString(),
          type: 'message',
        },
      ],
  ...
  const [customLinesheets, setCustomLinesheets] = useState<CustomLinesheet[]>([
    {
      ...
      brandId: 'org-brand-001',
      retailerId: 'org-shop-001',
      title: 'Exclusive FW26 Selection',
      products: ['1', '3', '5'],
      customPrices: {},
      status: 'sent',
      viewCount: 12,
  ...
  const [b2bTasks, setB2bTasks] = useState<B2BTask[]>([
    {
      id: 't1',
      title: 'Review FW26 Pre-orders',
      description: 'Check quantities for Premium Store',
      status: 'in_progress',
      priority: 'high',
      assigneeId: 'org-brand-001',
      dueDate: new Date().toISOString().split('T')[0],
      createdAt: new Date().toISOString(),
    },
    {
      id: 't2',
      title: 'Upload Quality Certs',
      description: 'Need for CTP-26 series',
      status: 'todo',
      priority: 'medium',
      assigneeId: 'org-brand-001',
      dueDate: new Date().toISOString().split('T')[0],
    ...
    {
      id: 't3',
      title: 'Nordic Peak Logistics',
      description: 'Coordinate shipment to Tokyo',
      status: 'todo',
      priority: 'high',
      assigneeId: 'org-brand-002',
      dueDate: new Date().toISOString().split('T')[0],
      createdAt: new Date().toISOString(),
    },
    {
      id: 't4',
      title: 'Подписать договор SS26',
      description: 'Radical Chic контракт ожидает подтверждения',
      status: 'todo',
      priority: 'high',
      assigneeId: 'org-shop-001',
      dueDate: new Date().toISOString().split('T')[0],
      createdAt: new Date().toISOString(),
    },
    {
      id: 't5',
      title: 'Проверить лайншит Syntha',
      description: 'Новая коллекция FW26 требует ревью',
      status: 'todo',
      priority: 'medium',
      assigneeId: 'org-shop-001',
    ...
    },
    {
      ...
      description: 'Актуализировать цены для осеннего сезона',
      status: 'done',
      priority: 'low',
      assigneeId: 'org-brand-001',
  ...
  const [b2bDocuments, setB2bDocuments] = useState<B2BDocument[]>([
    {
      ...
      partnerId: 'org-brand-001',
    },
    {
      id: 'doc-2',
      title: 'Nordic_Quality_Manual.pdf',
      type: 'spec',
      url: '#',
      size: '1.2 MB',
      uploadedBy: 'Admin',
      status: 'final',
      uploadedAt: new Date().toISOString(),
      partnerId: 'org-brand-002',
    },
    {
      id: 'doc-3',
      title: 'Radical_Chic_Contract_SS26.pdf',
      type: 'contract',
      url: '#',
      size: '890 KB',
      ...
      status: 'draft',
      uploadedAt: new Date().toISOString(),
      partnerId: 'org-brand-003',
    },
    {
      id: 'doc-4',
      title: 'Syntha_Lab_Spec_Sheet.pdf',
      type: 'spec',
      url: '#',
      size: '3.1 MB',
      uploadedBy: 'Admin',
  ...
  const [marketingAssets, setMarketingAssets] = useState<BrandAsset[]>([
    {
      id: 'a1',
      brandId: 'syntha-lab',
      title: 'Cyber Parka Promo',
      type: 'video',
      url: '#',
      thumbnail: 'https://images.unsplash.com/photo-1539109132304-372874a581ad?q=80&w=400',
      tags: ['Reels', 'FW26'],
    },
    {
      id: 'a2',
      brandId: 'syntha-lab',
      title: 'Studio Look 01',
      type: 'image',
      url: '#',
      thumbnail: 'https://images.unsplash.com/photo-1551488831-00ddcb6c6bd3?q=80&w=400',
      tags: ['Campaign', 'Web'],
    },
  ]);
      ...
      pages: [
        {
          ...
          hotspots: [{ id: 'h1', productId: '1', x: 45, y: 30 }],
          type: 'panorama',
        },
      ],
    },
  ...
  const [wholesaleLeads, setWholesaleLeads] = useState<WholesaleLead[]>([
    {
      id: 'lead-1',
      name: 'Tokyo Avant-Garde',
      location: 'Tokyo, JP',
      score: 92,
      status: 'qualified',
      source: 'Web Inquiry',
      estimatedVolume: 500000,
      tags: ['High-End', 'Concept Store'],
      lastActivity: new Date().toISOString(),
    },
    {
      id: 'lead-2',
      name: 'Berlin Tech-Wear',
      location: 'Berlin, DE',
      score: 85,
      status: 'negotiation',
      source: 'Trade Show',
      estimatedVolume: 300000,
      tags: ['Technical', 'Youth'],
      lastActivity: new Date().toISOString(),
    },
  ]);
  ...
  const [orderClaims, setOrderClaims] = useState<OrderClaim[]>([
    {
      ...
      retailerId: 'org-shop-001',
      type: 'damage',
      description: '2 units of Cyber Tech Parka arrived with torn sleeves',
    ...
    },
    {
      ...
      retailerId: 'org-shop-001',
      type: 'wrong_item',
      description: 'Incorrect size run delivered',
      status: 'pending',
      photos: ['#'],
      createdAt: new Date().toISOString(),
    },
  ...
  useEffect(() => {
    ...
    const unsubscribeNegotiations = B2BRepository.subscribeToNegotiations(
      activeOrgId,
      !!isBrand,
      (data) => {
  ...
  const filteredActivityLogs = useMemo(() => {
    ...
    if (isBrand || isRetailer) {
      return b2bActivityLogs.filter(
        (log) =>
          log.actor.id === activeOrgId ||
          log.target.id === activeOrgId ||
          (log.target.type === 'product' &&
            allProductsData.find((p) => p.id === log.target.id)?.brand === user?.partnerName)
      );
    }
    return [];
  ...
  const filteredClaims = useMemo(() => {
    ...
    if (isBrand)
      return orderClaims.filter(
        (c) => b2bNegotiations.find((n) => n.orderId === c.orderId)?.brandId === activeOrgId
      );
  ...
  const filteredDocuments = useMemo(() => {
    ...
    if (isBrand || isRetailer)
      return b2bDocuments.filter(
        (d) => d.partnerId === activeOrgId || d.uploadedBy === activeOrgId
      );
    return [];
  ...
  const addWholesaleLead = (lead: Omit<WholesaleLead, 'id' | 'lastActivity'>) => {
    setWholesaleLeads((prev) => [
      { ...lead, id: `lead-${Date.now()}`, lastActivity: new Date().toISOString() },
      ...prev,
    ]);
  };
  ...
  const addOrderClaim = (claim: Omit<OrderClaim, 'id' | 'createdAt'>) => {
    setOrderClaims((prev) => [
      { ...claim, id: `cl-${Date.now()}`, createdAt: new Date().toISOString() },
      ...prev,
    ]);
  };
  ...
  const updateWhiteLabelConfig = (brandId: string, config: Partial<WhiteLabelConfig>) => {
    setWhiteLabelConfigs((prev) => ({
      ...
      [brandId]: prev[brandId]
        ? { ...prev[brandId], ...config }
        : ({ brandId, ...config } as WhiteLabelConfig),
    }));
  };
  ...
  const addB2bOrderItem = (
    ...
    setB2bCart((prevCart) => {
      const existingItemIndex = prevCart.findIndex(
        (item) =>
          item.id === product.id && item.selectedSize === size && item.deliveryDate === deliveryDate
      );
  ...
  const updateB2bOrderItemQuantity = (
    productId: string,
    quantity: number,
    size: string,
    deliveryDate: string = 'Immediate'
    ...
    setB2bCart((prevCart) => {
      ...
      const itemIndex = newCart.findIndex(
        (item) =>
          item.id === productId && item.selectedSize === size && item.deliveryDate === deliveryDate
      );
      if (quantity > 0) {
  ...
  const reserveStock = (sku: string, quantity: number, retailerId: string) => {
    setInventoryATS((prev) =>
      prev.map((item) =>
        item.sku === sku
          ? { ...item, available: item.available - quantity, reserved: item.reserved + quantity }
          : item
      )
    );
    addB2bActivityLog({
  ...
  const addB2bTask = (task: Omit<B2BTask, 'id' | 'createdAt'>) => {
    setB2bTasks((prev) => [
      { ...task, id: `task-${Date.now()}`, createdAt: new Date().toISOString() },
      ...prev,
    ]);
  };
  ...
  const uploadB2bDocument = (doc: Omit<B2BDocument, 'id' | 'uploadedAt'>) => {
    setB2bDocuments((prev) => [
      { ...doc, id: `doc-${Date.now()}`, uploadedAt: new Date().toISOString() },
      ...prev,
    ]);
  };
    ...
    setProductVotes((prev) => {
      ...
      return {
        ...
        [productId]: {
          ...
          [currentType]: isCurrent
            ? (votes as any)[currentType].filter((id: string) => id !== userId)
            : [...(votes as any)[currentType], userId],
        },
  ...
  const createB2CListing = (listing: Omit<B2CProductListing, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    setB2cListings((prev) => [
      ...prev,
      { ...listing, id: `b2c-${Date.now()}`, createdAt: now, updatedAt: now },
    ]);
  };
  const updateB2CListing = (id: string, patch: Partial<B2CProductListing>) => {
    const now = new Date().toISOString();
    setB2cListings((prev) =>
      prev.map((l) => (l.id === id ? { ...l, ...patch, updatedAt: now } : l))
    );
  };
  ...
  const rejectStockSync = (id: string, reason: string) => {
    setStockSyncRequests((prev) =>
      prev.map((s) => (s.id === id ? { ...s, status: 'rejected' as const, notes: reason } : s))
    );
  };
  const updateRetailerStock = (syncId: string, quantity: number) => {
    setStockSyncRequests((prev) =>
      prev.map((s) => (s.id === syncId ? { ...s, retailerStock: quantity } : s))
    );
  };

  const createSyncAgreement = (
    agreement: Omit<StockSyncAgreement, 'id' | 'createdAt' | 'updatedAt'>
  ) => {
    const now = new Date().toISOString();
    setStockSyncAgreements((prev) => [
      ...prev,
      { ...agreement, id: `agr-${Date.now()}`, createdAt: now, updatedAt: now },
    ]);
  };
  ...
  const getUnifiedStock = useCallback(
    (productId: string): UnifiedStockView | null => {
  ...
  const createB2COrder = (order: Omit<B2COrder, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    setB2cOrders((prev) => [
      ...prev,
      { ...order, id: `b2co-${Date.now()}`, createdAt: now, updatedAt: now },
    ]);
  };
  ...
  const value = useMemo(
    ...
    [
      ...
      filteredConnections,
      filteredLinesheetRequests,
      filteredNegotiations,
      assortmentPlan,
      filteredCollections,
      filteredCustomLinesheets,
      inventoryATS,
      b2bEscrowTransactions,
      retailerLoyalty,
      marketingBanners,
