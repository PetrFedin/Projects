...
const mockPartner = {
  ...
  documents: [
    { id: 'DOC-001', title: 'Договор поставки SS27', type: 'Договор', status: 'Подписан' },
...
export function BrandRetailerDetailLegacyPage({ id }: { id: string }) {
  const partner = { ...mockPartner, id };
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(`/api/brand/retailers/${encodeURIComponent(id)}/b2b-orders`);
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Card className="p-4">
          ...
          <p className="text-text-primary text-xl font-black">
            {w2Orders.length > 0 ? w2Orders.length : partner.orders}
          </p>
        </Card>
      </div>
        ...
        >
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {w2Orders.length > 0 ? 'Заказы из W2 B2B store' : 'Связь с B2B Orders (демо)'}
              </CardDescription>
            </CardHeader>
            <CardContent>
