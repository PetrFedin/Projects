...
export async function suggestSupportReply(
  ...
  const DEFAULT_FAQ = [
    { question: "Сроки доставки?", answer: "Доставка по России 3-7 рабочих дней. Москва и МО — 1-2 дня. Экспресс — от 1 дня." },
    { question: "Как оформить возврат?", answer: "Возврат в течение 14 дней с момента получения. Товар должен быть в оригинальной упаковке, без следов использования. Заявка через личный кабинет или поддержку." },
    { question: "Способы оплаты?", answer: "Банковская карта, СБП, рассрочка (Тинькофф, Сбер) при заказе от 3000 ₽." },
    { question: "Как подобрать размер?", answer: "На странице товара есть таблица размеров. Рекомендуем снять мерки и сравнить с таблицей. При сомнениях — берите размер вверх." },
    { question: "Есть ли примерка?", answer: "Да, в шоурумах Москвы и Санкт-Петербурга. Запись по телефону или на сайте." },
    { question: "Обмен размера?", answer: "Бесплатный обмен в течение 14 дней. Курьер привезёт новый размер и заберёт старый." },
  ];

  const faqItems = input.faq?.length ? input.faq : DEFAULT_FAQ;
  const faqStr = faqItems.map((f, i) => `${i + 1}. В: ${f.question}
   О: ${f.answer}`).join("\n\n");
    ...
    const result = await withTokenAudit(
      ...
      async (i) => {
        const prompt = ai.definePrompt({
          ...
          input: { schema: z.object({ customerMessage: z.string(), faqContent: z.string(), context: z.string() }) },
