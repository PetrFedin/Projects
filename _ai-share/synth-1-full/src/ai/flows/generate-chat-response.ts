...
export async function generateChatResponse(
  ...
  if (historySize > 10) { // Example threshold
  ...
  return withTokenAudit('generateChatResponse', optimizedInput, input.userId, historySize, (i) => generateChatResponseFlow(i));
}
...
const generateChatResponseFlow = ai.defineFlow(
  ...
  async input => {
    try {
      ...
      let response = "Здравствуйте! Я готова помочь вам с подбором образа или аналитикой коллекции для российского рынка.";
      ...
      if (query.includes("привет") || query.includes("здравствуй")) {
        ...
        response = "Сейчас в России наблюдается большой спрос на качественный трикотаж и функциональный минимализм. Могу подобрать конкретные модели.";
      } else if (query.includes("цена") || query.includes("купить")) {
        response = "Я могу помочь сориентироваться по ценам на наши SKU. Какой сегмент вас интересует?";
