...
const OutputSchema = z.object({
  suggestions: z.array(z.string()).describe('2-4 альтернативных поисковых запроса на русском'),
});

export async function suggestSearchQueries(query: string): Promise<string[]> {
...
Верни JSON: { "suggestions": ["...", "..."] }`,
    ...
    return suggested
      .filter((s) => s && s.trim() && s.toLowerCase() !== q.toLowerCase())
      .slice(0, 4);
  } catch (e) {
