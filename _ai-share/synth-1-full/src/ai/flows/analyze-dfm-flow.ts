...
const DfmIssueSchema = z.object({
  ...
  description: z.string().describe("Detailed description of the manufacturability issue or complexity"),
...
const AnalyzeDfmOutputSchema = z.object({
  complexityLevel: z.enum(['simple', 'moderate', 'complex', 'highly_complex']).describe("Overall complexity level of sewing the article"),
...
export async function analyzeDfm(
  input: AnalyzeDfmInput
): Promise<AnalyzeDfmOutput> {
  return withTokenAudit('analyzeDfm', input, input.userId, 1, (i) =>
    analyzeDfmFlow(i)
  );
}
