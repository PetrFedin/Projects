...
export async function processBodyScan(input: BodyScanInput): Promise<BodyMeasurements> {
  return withTokenAudit('processBodyScan', input, input.userId, undefined, async (payload) => {
    console.log(
      `[AI_SCAN] Processing scan for User: ${payload.userId} with Height: ${payload.height} ${payload.unit}`
    );
    ...
    console.info(
      `[AI_SCAN] Successfully extracted measurements: Chest: ${mockMeasurements.chest.toFixed(1)}, Waist: ${mockMeasurements.waist.toFixed(1)}`
    );
