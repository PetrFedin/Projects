...
const MatchContractorsInputSchema = z.object({
  articleDescription: z
    .string()
    .describe('Description of the article, BOM, and required operations.'),
...
const ContractorMatchSchema = z.object({
  ...
  rationale: z
    .string()
    .describe('Explanation of why this contractor is a good fit for this article'),
});
...
export async function matchContractors(
  ...
  return withTokenAudit('matchContractors', input, input.userId, 1, (i) => matchContractorsFlow(i));
}
...
Верни массив из 3 лучших совпадений (matches) в формате JSON, где для каждого указан contractorId, оценка совпадения (score 0-100) и обоснование выбора (rationale).`,
      ...
      console.warn(
        'Matchmaker AI failed, using local fallback:',
