...
export async function simulateCollectionDemand(input: SKUPlannerInput): Promise<SKUDemandForecast[]> {
...
export async function analyzePlanningRisks(items: PlannedSKU[], forecasts: SKUDemandForecast[]) {
  return items.map(item => {
    ...
    const risk = (item.plannedQty > (forecast?.predictedDemand || 0) * 1.3) ? 'high_overstock' : 
                 (item.plannedQty < (forecast?.predictedDemand || 0) * 0.7) ? 'high_understock' : 'low';
