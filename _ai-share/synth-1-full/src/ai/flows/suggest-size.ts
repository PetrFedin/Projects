...
export async function suggestSize(input: SuggestSizeInput): Promise<SuggestSizeOutput> {
  const hasMeasurements = input.userHeight ?? input.userChest ?? input.userWaist ?? input.userHips;
...
Верни JSON: { "suggestedSize": "...", "recommendation": "...", "tips": [] }`,
        ...
        return (
          output ?? {
            suggestedSize: 'M',
            recommendation: 'Рекомендуем ориентироваться на таблицу размеров.',
            tips: [],
          }
        );
      }
    ...
    return {
      suggestedSize: 'M',
      recommendation: 'Обратитесь к таблице размеров на странице товара.',
      tips: [],
    };
  }
}
