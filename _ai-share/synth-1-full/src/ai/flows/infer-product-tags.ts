...
const OutputSchema = z.object({
  tags: z
    .array(z.string())
    .describe('Теги стиля: minimal, casual, formal, urban, classic, premium, sport, elegant, etc.'),
  occasion: z
    .array(z.string())
    .optional()
    .describe('Сценарии: Work, Daily, Evening, Sport, Travel'),
...
export async function inferProductTags(input: InferProductTagsInput): Promise<{ tags: string[] }> {
  const promptInput = {
...
Только англоязычные теги в нижнем регистре. Формат: { "tags": ["...", "..."] }`,
    ...
    const tags = (result?.tags ?? []).filter((t) => VALID_TAGS.includes(t.toLowerCase()));
