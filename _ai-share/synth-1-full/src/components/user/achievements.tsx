  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setAchievements((prev) =>
        prev.map((ach) => (ach.id === 'stylist' ? { ...ach, achieved: true } : ach))
      );
    }, 1000);
        ...
        {achievements.map((ach) => (
          <Card
            ...
            className={cn(
              ...
              !ach.achieved && 'opacity-40 hover:opacity-100'
            )}
          >
            <div
              className={cn(
                'flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-lg',
                ach.achieved ? 'bg-accent text-accent-foreground' : 'bg-muted text-muted-foreground'
              )}
            >
