...
import { Apple, ArrowRight, Camera, Chrome, Gift, Shirt, Sparkles, Star, Zap, Crown } from 'lucide-react';
...
const loyaltyPlans = {
  ...
  comfort: { name: 'Комфорт', icon: Gift, color: 'text-purple-600', bg: 'bg-purple-100', next: 'premium' },
  premium: { name: 'Premium', icon: Crown, color: 'text-yellow-600', bg: 'bg-yellow-100', next: null },
};
...
const planTheme = {
  ...
  comfort: {
    ...
    orb: 'bg-purple-200/40',
    accentText: 'text-purple-700',
    accentTextSoft: 'text-purple-600',
  },
  premium: {
...
export default function LoyaltyCard() {
  ...
  const transactions: LoyaltyTx[] = [
    ...
    { id: 'q1', date: '2025-12-18T10:30:00Z', title: 'Кэшбэк за покупку (заказ #1042)', amount: 3800, bucket: 'qualifying' },
    { id: 'q2', date: '2025-11-29T15:10:00Z', title: 'Кэшбэк за покупку (заказ #1011)', amount: 5200, bucket: 'qualifying' },
    { id: 'q3', date: '2025-10-03T09:05:00Z', title: 'Кэшбэк за покупку (заказ #998)', amount: 4450, bucket: 'qualifying' },
    // Non-qualifying (activity)
    { id: 'n1', date: '2025-12-25T08:00:00Z', title: 'Ежедневная активность', amount: 400, bucket: 'nonQualifying' },
    { id: 'n2', date: '2025-12-12T12:10:00Z', title: 'Создание лукборда', amount: 700, bucket: 'nonQualifying' },
    { id: 'n3', date: '2025-11-20T13:20:00Z', title: 'Отзывы и лайки', amount: 900, bucket: 'nonQualifying' },
    // Spend / expiration examples
    // Spend is always split 75/25, qualifying first
    { id: 's1', date: '2025-12-28T17:40:00Z', title: 'Списание при оплате части заказа бонусами', amount: -1200, bucket: 'mixed' },
    { id: 'e1', date: '2025-11-30T00:00:00Z', title: 'Сгорание неквалификационных баллов (истёк срок)', amount: -600, bucket: 'nonQualifying' },
  ...
  const expandedTransactions = transactions.flatMap((t) => {
    ...
    return ([
      { ...t, id: `${t.id}-q`, amount: parts.qualifying, bucket: 'qualifying' as const, title: `${t.title} (75% квалиф.)` },
      { ...t, id: `${t.id}-n`, amount: parts.nonQualifying, bucket: 'nonQualifying' as const, title: `${t.title} (25% неквалиф.)` },
    ] as LoyaltyTx[]);
  });
  ...
  const filteredTransactions = useMemo(() => expandedTransactions.filter((t) => inRange(t.date)), [expandedTransactions, txRange.from, txRange.to]);
    ...
    >
      <CardContent className="relative p-4">
        <div className="flex flex-col md:flex-row gap-3 items-stretch">
          ...
          <div className="w-full md:w-[552px] shrink-0 space-y-2">
            <Link
              href="/loyalty"
              className={cn(
                "relative block rounded-2xl overflow-hidden border bg-gradient-to-br p-3 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
                'border-border/60',
                theme.gradient,
            ...
            >
              <div className={cn("absolute top-0 right-0 w-28 h-28 rounded-full -mr-14 -mt-14 blur-2xl", theme.orb)} />
              <div className={cn("absolute bottom-0 left-0 w-24 h-24 rounded-full -ml-12 -mb-12 blur-xl", theme.orb)} />

              <div className="relative h-full flex flex-col justify-between">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-xs uppercase tracking-widest text-muted-foreground">Syntha</div>
                    <div className={cn("mt-1 text-sm font-semibold", theme.accentText)}>Bonus Card</div>
                  </div>
                  <div className={cn("p-2 rounded-xl border bg-background/60 backdrop-blur", 'border-border/60')}>
                    {/* Rotate 360° around its axis on hover */}
                    <PlanIcon className={cn("h-5 w-5 transition-transform duration-700 ease-in-out hover:rotate-[360deg]", planInfo.color)} />
                  </div>
                </div>

                <div className="relative">
                  <div className="text-xs uppercase tracking-widest text-muted-foreground">Владелец</div>
                  <div className={cn("text-sm md:text-base font-bold font-headline truncate", theme.accentText)}>
                    {displayName}
                  </div>
                  <div className="mt-2 flex items-center justify-between gap-3">
                    <Badge variant="outline" className={cn("bg-background/60", theme.accentTextSoft, 'border-border/60')}>
                      {planInfo.name}
                    </Badge>
                    <div className={cn("text-sm font-mono font-semibold", theme.accentTextSoft)}>
          ...
          <div className="flex flex-1 flex-col justify-between gap-3 min-w-0 w-full">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-1 flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs text-muted-foreground">
                      Активна до: <span className="font-medium text-foreground">{endDateFormatted}</span>
                    </span>
                  </div>
                </div>
                <Link
                  href="/loyalty?renew=1"
                  className="shrink-0 mt-[22px] inline-flex items-center gap-1.5 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors"
                >
                  <span className="relative after:content-[''] after:pointer-events-none after:absolute after:inset-[-6px] after:rounded-lg after:ring-2 after:ring-accent/15 after:animate-pulse" />
                ...
                <Dialog open={txOpen} onOpenChange={setTxOpen}>
                  <DialogTrigger asChild>
                    <button
                      type="button"
                      className={cn(
                        "w-full text-left rounded-md -mx-1 px-1 py-1 hover:bg-muted/40 transition-colors group",
                      ...
                      <TooltipProvider>
                        <Tooltip delayDuration={0}>
                          <TooltipTrigger asChild>
                            <div className={cn("text-base font-bold leading-tight", theme.accentText)}>
                  ...
                  <DialogContent className="sm:max-w-2xl">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Начисления и списания. Неквалификационные баллы могут сгорать согласно правилам программы.
                      </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid gap-3 md:grid-cols-3">
                      <div className="rounded-lg border p-3">
                        ...
                        <div className="text-sm font-bold">{totalPoints.toLocaleString('ru-RU')}</div>
                      </div>
                      <div className="rounded-lg border p-3">
                        <div className="text-xs text-muted-foreground">Квалификационные</div>
                        <div className="text-sm font-bold">{qualifying === null ? '—' : qualifying.toLocaleString('ru-RU')}</div>
                      <div className="text-[11px] mt-1 text-green-600 font-medium">Не сгорают</div>
                      </div>
                      <div className="rounded-lg border p-3">
                        <div className="text-xs text-muted-foreground">Неквалификационные</div>
                        <div className="text-sm font-bold">{nonQualifying === null ? '—' : nonQualifying.toLocaleString('ru-RU')}</div>
                      <div className="text-[11px] mt-1 text-red-600 font-medium">Могут сгорать</div>
                      </div>
                    </div>
                    ...
                    <div className="max-h-[50vh] overflow-y-auto rounded-lg border overscroll-contain">
                      <div className="grid grid-cols-[140px,1fr,120px] gap-3 px-4 py-2 text-xs font-semibold text-muted-foreground border-b bg-muted/30">
                        <div>Дата</div>
                        <div>Статья</div>
                      ...
                      <div className="divide-y">
                        {filteredTransactions.map((t) => {
                          ...
                          return (
                            <div key={t.id} className="grid grid-cols-[140px,1fr,120px] gap-3 px-4 py-3 text-sm">
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t.bucket === 'qualifying' ? 'Квалификационные' : 'Неквалификационные'}
                                </div>
                              </div>
                              <div className={cn("text-right font-semibold", isPlus ? "text-green-600" : "text-red-600")}>
                                {amountText}
                              </div>
                            </div>
                          );
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      <div className="rounded-lg border p-3">
                        ...
                        <div className={cn("text-base font-bold", periodTotals.net >= 0 ? "text-green-600" : "text-red-600")}>
                          {periodTotals.net >= 0 ? '+' : ''}
                      ...
                      <div className="rounded-lg border p-3">
                        ...
                        <div className={cn("text-base font-bold", periodTotals.qual >= 0 ? "text-green-600" : "text-red-600")}>
                          {periodTotals.qual >= 0 ? '+' : ''}
                      ...
                      <div className="rounded-lg border p-3">
                        ...
                        <div className={cn("text-base font-bold", periodTotals.non >= 0 ? "text-green-600" : "text-red-600")}>
                          {periodTotals.non >= 0 ? '+' : ''}
            ...
            <div className="grid grid-cols-3 gap-3">
              <TooltipProvider>
                <Tooltip delayDuration={0}>
                  <TooltipTrigger asChild>
                    <div className="rounded-lg border border-border/60 p-3 text-center cursor-help">
                      <div className="text-xs text-muted-foreground">Кэшбэк</div>
                      <div className={cn("text-base font-bold", theme.accentTextSoft)}>{benefits.cashback}</div>
                    </div>
                  </TooltipTrigger>
                  <TooltipContent side="top" className="max-w-xs text-left">
                    Процент бонусов, который возвращается от суммы покупок. Начисляется после подтверждения заказа.
                  </TooltipContent>
                </Tooltip>
                ...
                <Tooltip delayDuration={0}>
                  <TooltipTrigger asChild>
                    <div className="rounded-lg border border-border/60 p-3 text-center cursor-help">
                      ...
                      <div className={cn("text-base font-bold", theme.accentTextSoft)}>{benefits.pointsRate}</div>
                    </div>
                  </TooltipTrigger>
                  <TooltipContent side="top" className="max-w-xs text-left">
                ...
                <Tooltip delayDuration={0}>
                  <TooltipTrigger asChild>
                    <div className="rounded-lg border border-border/60 p-3 text-center cursor-help">
                      ...
                      <div className={cn("text-base font-bold", theme.accentTextSoft)}>{benefits.validity}</div>
                    </div>
                  </TooltipTrigger>
                  <TooltipContent side="top" className="max-w-xs text-left">
                    Срок действия преимуществ тарифа. Квалификационные баллы не сгорают, неквалификационные могут сгорать по правилам программы.
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
            ...
            {qualifying !== null && nonQualifying !== null && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="rounded-lg border border-border/60 p-4">
                  <div className="text-xs text-muted-foreground">Квалификационные</div>
                  <div className={cn("text-sm font-bold mt-1", theme.accentText)}>
            ...
            <div className="flex items-center justify-between gap-3">
              <TooltipProvider>
                <div className="flex items-center gap-3 text-muted-foreground whitespace-nowrap overflow-x-auto">
                  ...
                  <Tooltip delayDuration={0}>
                    <TooltipTrigger asChild>
                      <div className={cn("inline-flex items-center", hasArAccess ? "" : "opacity-40")}>
                        <Camera className={cn("h-4 w-4", hasArAccess ? theme.accentTextSoft : "text-muted-foreground")} />
                      </div>
                    </TooltipTrigger>
                  ...
                  <Tooltip delayDuration={0}>
                    <TooltipTrigger asChild>
                      <div className={cn("inline-flex items-center", hasWithWhatToWear ? "" : "opacity-40")}>
                        <Shirt className={cn("h-4 w-4", hasWithWhatToWear ? theme.accentTextSoft : "text-muted-foreground")} />
                      </div>
                    </TooltipTrigger>
                    <TooltipContent side="top">
