  ...
  const metrics: DataMetric[] = [
    {
      ...
      explanation:
        'Эта сумма включает все ваши заказы за выбранный период. Учитываются все товары, доставка и налоги.',
...
>>>>>>> recover/cabinet-wip-from-stash
    ...
    },
    {
      ...
      explanation:
        'Этот показатель отражает, сколько вы сэкономили за счет бонусов, персональных скидок и участия в закрытых распродажах.',
      ...
      week: [
        {
          ...
          value: 'Среда, 14:00',
          confidence: 92,
          description: 'На этой неделе клиент чаще всего заходил в приложение в середине дня.',
        },
        {
          label: 'Режим просмотра',
          value: 'Быстрый скроллинг',
          confidence: 85,
          description: 'Сессии короткие, упор на просмотр новинок без глубокого изучения.',
        },
        {
          label: 'Интерес к брендам',
          value: 'Новинки Syntha',
          confidence: 88,
          description: 'Высокий интерес к новой коллекции, вышедшей в понедельник.',
        },
        {
          label: 'Стиль недели',
          value: 'Casual / Аксессуары',
          confidence: 70,
          description: 'Поиск сфокусирован на дополнении текущих образов деталями.',
        },
        {
          label: 'Ценовой триггер',
          value: 'Акции и Скидки',
          confidence: 95,
          description: 'Все клики на этой неделе были по товарам со спецпредложениями.',
        },
        {
          label: 'Прогноз на Пт-Вс',
          value: 'Высокая вероятность покупки',
          confidence: 78,
          description: 'На основе активности в среду ожидаем оформление заказа в выходные.',
        },
      ],
      month: [
        {
          label: 'Время шопинга',
          value: 'Вечерний покупатель',
          confidence: 82,
          description: '80% добавлений в корзину за месяц произошло после 20:00.',
        },
        {
          label: 'Устройство',
          value: 'iOS App (90%)',
          confidence: 98,
          description: 'Клиент предпочитает мобильный интерфейс для принятия решений.',
        },
        {
          label: 'Лояльность',
          value: 'Syntha, COS',
          confidence: 90,
          description: 'Стабильное предпочтение товаров этих брендов на протяжении 30 дней.',
        },
        {
          label: 'Стилевой вектор',
          value: 'Минимализм',
          confidence: 85,
          description: 'Клиент игнорирует яркие принты, выбирая базовые силуэты.',
        },
        {
          label: 'Средний чек',
          value: 'Растущий (+12%)',
          confidence: 80,
          description: 'В этом месяце клиент выбирает более дорогие позиции, чем обычно.',
        },
        {
          label: 'AI Совет',
          value: 'Персональный оффер',
          confidence: 88,
          description: 'Оптимальное время для отправки push-уведомления — четверг вечер.',
        },
      ],
      year: [
        {
          ...
          value: 'Март - Апрель',
          confidence: 95,
          description: 'Ежегодный период максимальных трат на обновление гардероба.',
        },
        {
          label: 'Канал продаж',
          value: 'Omnichannel',
          confidence: 92,
          description: 'Использует приложение для выбора, но часто заказывает через сайт.',
        },
        {
          label: 'Бренд-амбассадор',
          value: 'Топ-1% лояльности',
          confidence: 99,
          description: 'Входит в число самых преданных покупателей бренда Syntha за год.',
        },
        {
          label: 'Гардероб',
          value: 'Капсульный подход',
          confidence: 90,
          description: 'За год собрана полноценная капсула, вещи хорошо сочетаются между собой.',
        },
        {
          label: 'LTV Прогноз',
          value: 'Высокий потенциал',
          confidence: 85,
          description: 'Ожидаемый доход от клиента в следующем году вырастет на 20%.',
        },
        {
          label: 'Статус',
          value: 'Лояльный чемпион',
          confidence: 96,
          description: 'Самый стабильный и предсказуемый паттерн поведения в базе.',
        },
      ],
    };
      ...
      {(variant === 'all' || variant === 'metrics') && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Детальный разбор метрик с AI-пояснениями</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {(variant === 'all' || variant === 'patterns') && filteredPatterns.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>AI-выводы о ваших долгосрочных привычках</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-3 md:grid-cols-2">
              {filteredPatterns.map((pattern, index) => (
                <div key={index} className="rounded-lg border bg-muted/50 p-4">
                  ...
                  <p className="mb-2 text-sm text-muted-foreground">{pattern.description}</p>
...
function MetricCard({ metric }: { metric: DataMetric }) {
  const TrendIcon =
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Card className="group relative cursor-pointer transition-colors hover:border-accent/50">
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between">
              ...
              <div className="opacity-0 transition-opacity group-hover:opacity-100">
                <Button variant="ghost" size="icon" className="pointer-events-none h-6 w-6">
...
>>>>>>> recover/cabinet-wip-from-stash
        ...
        <div className="mt-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {metric.trendValue && (
                <div>
                  ...
                  <div className={cn('flex items-center gap-1 text-base font-bold', trendColor)}>
          ...
          <div className="flex items-center gap-2 rounded-lg border border-accent/10 bg-accent/5 p-3 text-xs italic text-muted-foreground">
            <Sparkles className="h-3.5 w-3.5 text-accent" />
            <span>
              AI-анализ подтверждает стабильность данного показателя на основе вашей активности за
              выбранный период.
            </span>
          </div>
        </div>
