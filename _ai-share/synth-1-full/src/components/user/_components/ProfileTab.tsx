...
import { Crown, Check, Trash2, ArrowRight, Sparkles, Plus, Mail, Phone, Instagram, Send, Youtube, Facebook, MessageCircle, ShieldCheck } from 'lucide-react';
...
export const ProfileTab = ({ 
  ...
  const [nicknameStatus, setNicknameStatus] = useState<{ state: 'idle' | 'checking' | 'ok' | 'taken'; message?: string }>({
    state: 'idle',
  });
  const [nextNetwork, setNextNetwork] = useState<'instagram' | 'telegram' | 'youtube' | 'vk' | 'facebook' | 'max'>('instagram');
  ...
  const emailAddresses = useFieldArray({ control: form.control, name: 'personalInfo.emailAddresses' });
    ...
    if (path === 'personalInfo.phoneNumbers') {
        ...
        window.setTimeout(() => {
            ...
            next.forEach((_: any, i: number) => {
                form.setValue(`personalInfo.phoneNumbers.${i}.primary` as any, i === 0, { shouldDirty: true });
            });
        }, 0);
        ...
        window.setTimeout(() => {
            const next = (form.getValues(path as any) || []) as any[];
            next.forEach((_: any, i: number) => {
                form.setValue(`personalInfo.emailAddresses.${i}.primary` as any, i === 0, { shouldDirty: true });
            });
        }, 0);
    } else {
  ...
  const availableNetworks = (['instagram', 'telegram', 'youtube', 'vk', 'facebook', 'max'] as const).filter(n => !usedNetworks.has(n));
  ...
  return (
    <div className="mt-6">
      <div className="grid md:grid-cols-2 gap-3 items-stretch">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input className={compactInput} placeholder="Елена" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input className={compactInput} placeholder="Петрова" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input className={compactInput} placeholder="Elena" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input className={compactInput} placeholder="Petrova" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
                ...
                <div className="grid grid-cols-2 gap-3 items-end">
                  ...
                  <div className="flex items-end justify-end gap-3">
                    {nicknameStatus.state !== 'idle' && nicknameStatus.message && (
                      <div className={cn("text-[11px] leading-none", nicknameStatus.state === 'ok' ? "text-green-600" : "text-muted-foreground")}>
                        <span className="max-w-[220px] inline-block truncate">{nicknameStatus.message}</span>
                      </div>
                    )}
                    <button
                      type="button"
                      ...
                      onClick={() => {
                        ...
                        if (taken.has(raw)) {
                          ...
                          setNicknameStatus({ state: 'taken', message: `Занят • предложено: ${suggested}` });
                          toast({ title: 'Никнейм занят', description: `Подобрали свободный: ${suggested}` });
                          return;
                        }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input className={compactInput} placeholder="Маркетинг / IT / Финансы…" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <div className="flex flex-col min-h-0" style={profileLeftHeight ? { height: profileLeftHeight } : undefined}>
          <FormField
            control={form.control}
            name="bio"
            render={({ field }) => (
              <FormItem className="flex flex-col flex-1 min-h-0">
                <FormLabel className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-zinc-100 text-zinc-800 hover:bg-zinc-200 transition-colors"
                ...
                <FormControl className="flex-1 min-h-0">
                  <Textarea className="flex-1 min-h-0 resize-none h-full" placeholder="Коротко о себе..." {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            {[0, 1].map(idx => {
                ...
                const verifyState = exists ? (verified ? 'verified' : value.trim() ? 'pending' : 'empty') : 'empty';
                    ...
                    return (
                        <FormItem key="phone-1-add" className="m-0">
                            ...
                            <div className="flex items-center gap-2">
                                <Input className={compactInput + " flex-1"} disabled placeholder="Добавьте второй телефон" />
                                <Button type="button" variant="ghost" size="sm" onClick={() => phoneNumbers.append({ value: '', verified: false, primary: false })}>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="m-0">
                                <FormLabel>{idx === 0 ? 'Телефон (основной)' : 'Телефон (дополнительный)'}</FormLabel>
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        <Button
                                            type="button"
                                            size="icon"
                                            className={cn("h-5 w-5 rounded-full", idx === 0 ? "bg-zinc-900 text-white" : "bg-zinc-100 text-zinc-700")}
                                            onClick={() => idx !== 0 && setPrimaryInList('personalInfo.phoneNumbers', idx)}
                                        >
                                            <Crown className="h-3 w-3" />
                                        </Button>
                                        {idx > 0 && (
                                            <Button type="button" size="icon" variant="ghost" className="h-5 w-5 text-red-600" onClick={() => phoneNumbers.remove(idx)}>
          ...
          <div className="space-y-4">
            ...
            {[0, 1].map(idx => {
                ...
                if (idx === 1 && !exists) {
                    return (
                        <FormItem key="addr-1-add" className="m-0">
                            ...
                            <div className="flex items-center gap-2">
                                <Input className={compactInput + " flex-1"} disabled placeholder="Добавить адрес" />
                                <Button type="button" variant="ghost" size="sm" onClick={() => addressBook.append({ country: '', city: '', address: '', primary: false })}>
                                    <Plus className="h-4 w-4" />
                                </Button>
                            </div>
                        </FormItem>
                ...
                return (
                    <div key={exists ? addressBook.fields[idx].id : `addr-${idx}`} className="grid grid-cols-2 gap-2">
                        <FormField
                            control={form.control}
                                ...
                                <FormItem className="m-0">
                                    <FormLabel className="flex justify-between">
                                        ...
                                        <div className="flex gap-1">
                                            <Button type="button" size="icon" className={cn("h-4 w-4 rounded-full", isPrimary ? "bg-zinc-900 text-white" : "bg-zinc-100 text-zinc-700")} onClick={() => !isPrimary && setPrimaryInList('personalInfo.addressBook', idx)}>
                                                <Crown className="h-2.5 w-2.5" />
                                            </Button>
                                            <Button type="button" size="icon" variant="ghost" className="h-4 w-4 text-red-600" onClick={() => deleteAddressAt(idx)}>
                                                <Trash2 className="h-2.5 w-2.5" />
                                            </Button>
                                        </div>
                                    </FormLabel>
                                    <FormControl><Input className={compactInput} placeholder="Россия" {...field} /></FormControl>
                                </FormItem>
                            )}
                        />
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem className="m-0">
                                    ...
                                    <FormControl><Input className={compactInput} placeholder="Москва" {...field} /></FormControl>
                                </FormItem>
                            )}
                        />
                    </div>
      ...
      <div className="mt-4 rounded-lg border p-3">
        <div className="flex justify-between mb-2">
            ...
            <div className="flex gap-2">
                ...
                <Button type="button" size="sm" variant="outline" className="h-8" disabled={availableNetworks.length === 0} onClick={() => {
                    socialLinks.append({ network: nextNetwork, value: '', synced: false, verified: false });
                }}>
                    <Plus className="h-4 w-4" />
                </Button>
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-3">
            {socialLinks.fields.map((field, idx) => (
                <div key={field.id} className="flex gap-2 items-end border p-2 rounded-lg">
                    <div className="flex-1">
                        <span className="text-[10px] uppercase font-bold text-muted-foreground">{form.getValues(`socialLinks.${idx}.network`)}</span>
                        <FormField
                            control={form.control}
                            name={`socialLinks.${idx}.value`}
                            render={({ field }) => (
                                <FormControl><Input className={compactInput} {...field} /></FormControl>
                            )}
                        />
                    </div>
                    <Button type="button" size="icon" variant="ghost" className="text-red-600 h-8 w-8" onClick={() => socialLinks.remove(idx)}>
      ...
      <div className="mt-4 rounded-lg border p-3">
        <div className="flex items-center justify-between">
            ...
            <Button type="button" size="sm" variant={identityVerified ? "secondary" : "outline"} disabled={identityVerified} onClick={() => {
