...
export function NotificationSettings({ draft, updateDraft }: NotificationSettingsProps) {
  const setFrequency = (v: string) => updateDraft((s: any) => ({ ...s, notifications: { ...s.notifications, frequency: v } }));
  const setToggle = (key: string, v: boolean) => updateDraft((s: any) => ({ ...s, notifications: { ...s.notifications, [key]: v } }));
    ...
    <div className="space-y-4">
      <div className="space-y-2 max-w-xs">
        ...
        <Select value={draft.notifications.frequency} onValueChange={setFrequency}>
          ...
          <SelectContent>
            {Object.entries(frequencyLabels).map(([k, label]) => <SelectItem key={k} value={k}>{label}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      <div className="grid gap-3 md:grid-cols-2">
        {[
          ...
          <div key={item.key} className="flex items-center justify-between rounded-lg border p-4">
            ...
            <Switch checked={draft.notifications[item.key as keyof typeof draft.notifications]} onCheckedChange={(v) => setToggle(item.key, v)} />
          </div>
        ))}
      </div>
    </div>
