...
export default function AiStylist() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const chatResponse = await generateChatResponse({ query: values.prompt, history: messages.map(msg => ({ role: msg.type === 'user' ? 'user' : 'model', content: msg.text })), userId });
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
                variant="ghost"
                size="icon"
                className="fixed bottom-6 right-6 h-12 w-12 rounded-full shadow-2xl bg-card border-2 border-border group"
                        ...
                        <div className={cn("max-w-sm md:max-w-md space-y-2", msg.type === 'user' && "text-right")}>
                           ...
                           {msg.recommendedProducts && msg.recommendedProducts.length > 0 && (
                                <div className="space-y-2">
                                    <p className="text-xs font-medium text-muted-foreground">Рекомендуемые товары:</p>
...
function ProductRecommendationCard({ product }: { product: Product }) {
  ...
  return (
    <Link href={`/products/${product.slug}`} className="group">
      <div className="relative aspect-[4/5] rounded-md border bg-card overflow-hidden hover:border-accent transition-colors">
        <Image
