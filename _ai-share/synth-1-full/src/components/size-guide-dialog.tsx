...
export function SizeGuideDialog({ product, isOpen, onOpenChange }: SizeGuideDialogProps) {
  ...
  const getFieldsForCategory = () => {
    ...
    return [
      { id: 'height', label: 'Рост (см)', placeholder: 'Напр. 175' },
      { id: 'weight', label: 'Вес (кг)', placeholder: 'Напр. 65' },
    ];
  };
    ...
    if (isOpen) {
      ...
      fields.forEach((field) => {
        if (userMeasurements[field.id as keyof typeof userMeasurements]) {
          initialInputs[field.id] = String(
            userMeasurements[field.id as keyof typeof userMeasurements]
          );
        } else {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setRecommendation(
        `Рекомендуем размер ${recommendedSize}. Ориентируйтесь на таблицу размеров на странице товара.`
      );
    } finally {
      setIsLoading(false);
    }
        ...
        {fields.length > 0 ? (
          ...
          <div className="py-4">
            <Alert>
              ...
              <AlertDescription>Этот товар представлен в едином размере.</AlertDescription>
            </Alert>
          </div>
        )}
      </DialogContent>
