...
import { LogOut, Settings as SettingsIcon, User as UserIcon, ChevronDown, Check, Sparkles } from 'lucide-react';
...
export default function UserNav() {
  ...
  const isB2B = profile?.navigation?.length > 0 || 
                user?.roles?.some(r => ['admin', 'brand', 'distributor', 'supplier', 'manufacturer', 'shop'].includes(r)) || 
  ...
  const isOwner = (isB2B && (user?.team?.[0]?.nickname === user?.nickname || user?.team?.[0]?.name === user?.displayName || `${user?.team?.[0]?.firstName} ${user?.team?.[0]?.lastName}` === user?.displayName)) || (!isB2B && user?.roles?.includes('client'));
  ...
  const handleSwitchTeamMember = async (member: any) => {
    ...
    try {
      ...
      const targetUrl = primaryRole === 'admin' ? '/admin' : 
                       primaryRole === 'brand' ? '/brand' :
                       primaryRole === 'shop' ? '/shop' :
                       primaryRole === 'manufacturer' ? '/factory?role=manufacturer' :
                       primaryRole === 'supplier' ? '/factory?role=supplier' :
                       primaryRole === 'distributor' ? '/distributor' : '/u';
  ...
  if (user) {
    return (
      <div className="flex h-9 min-w-0 max-w-[200px] sm:max-w-[240px] items-stretch rounded-[2px] border border-slate-200 bg-white">
        <Button
          type="button"
          variant="ghost"
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2">
            <div className="relative shrink-0">
              <Avatar className="h-7 w-7 rounded-[2px] border border-white shadow-sm ring-1 ring-slate-100">
                ...
                />
                <AvatarFallback className="rounded-[2px] bg-slate-100 font-black text-[9px] text-slate-600">
        ...
        >
          {isB2B && user.organizations && user.organizations.length > 1 && (
            <div className="bg-slate-900 p-2 border-b border-slate-800">
              <p className="text-[7px] font-black uppercase tracking-[0.2em] text-white/40 mb-2 px-1 flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 gap-1">
                {user.organizations.map((org: any) => (
                  <DropdownMenuItem 
                    ...
                    className={cn(
                      "flex items-center justify-between cursor-pointer rounded-[1px] p-2 transition-all border",
                      user.activeOrganizationId === org.organizationId 
                        ? "bg-slate-800 text-white border-white/20 shadow-sm" 
                        : "bg-transparent text-slate-400 hover:bg-slate-800/50 hover:text-white border-transparent"
                    )}
                  >
                    ...
                    {user.activeOrganizationId === org.organizationId && (
                      <Check className="h-3 w-3 text-indigo-400 stroke-[3]" />
                    )}
                  </DropdownMenuItem>
                ))}
              </div>
          ...
          {profile?.navigation?.length > 0 && (
            <div className="p-1 border-b border-slate-100">
              <p className="text-[7px] font-black uppercase tracking-widest text-slate-400 mb-1 px-2 py-1">
                БЫСТРЫЙ ПЕРЕХОД
              </p>
              <div className="grid grid-cols-2 gap-0.5">
                {profile.navigation.slice(0, 4).map((nav: any) => (
                  <DropdownMenuItem
                    key={nav.path}
                    className="rounded-[1px] cursor-pointer py-1.5 hover:bg-slate-50"
                    onSelect={() => router.push(nav.path)}
                  >
                    <span className="text-[8px] font-black uppercase tracking-tight text-slate-600">{nav.title}</span>
                  </DropdownMenuItem>
                ))}
              </div>
            </div>
          ...
          <div className="px-4 py-3 bg-slate-50 border-b border-slate-100">
            <div className="flex flex-col space-y-0.5">
              ...
              <p className="text-xs font-black leading-none text-slate-900 uppercase tracking-tight">{user.displayName}</p>
              <p className="text-[9px] leading-none text-slate-500 mt-1 font-medium">
                {user.email}
              </p>
            </div>
          </div>
          ...
          {isB2B && user.team && user.team.length > 0 && (
            <div className="p-1">
              ...
              <div className="space-y-0.5">
                {user.team.filter(m => m.invitationStatus !== 'pending').map((member) => {
                  const isCurrent = user.nickname === member.nickname || 
                                   (!user.nickname && user.displayName === (member.name || `${member.firstName} ${member.lastName}`));
                  ...
                  return (
                    <DropdownMenuItem 
                      ...
                      className={cn(
                        "flex items-center justify-between cursor-pointer rounded-[1px] p-1.5 transition-all border outline-none",
                        isCurrent 
                    ...
                    >
                      <div className="flex items-center gap-2 overflow-hidden flex-1">
                        <Avatar className="h-6 w-6 rounded-[2px] border border-white ring-1 ring-slate-100 flex-shrink-0">
                          <AvatarImage src={member.avatar} />
                          <AvatarFallback className="text-[8px] font-black rounded-[2px]">{(member.name || member.firstName)[0]}</AvatarFallback>
                        </Avatar>
                        <div className="flex flex-col min-w-0">
                          <span className="text-[9px] font-black uppercase truncate text-slate-900 leading-tight">
                            {member.name || `${member.firstName} ${member.lastName}`}
                          </span>
                          <span className="text-[7px] font-bold text-slate-400 uppercase tracking-widest truncate leading-tight mt-0.5">
                            {member.role}
                          </span>
                        </div>
                      </div>
                      {isCurrent && (
                        <Check className="h-2.5 w-2.5 text-indigo-500 stroke-[3] ml-2" />
                      )}
                    </DropdownMenuItem>
                  );
                })}
  ...
  return (
    <Button type="button" variant="outline" size="sm" className="h-8 text-[10px] font-black uppercase" onClick={() => void handleSignInGuest()}>
      Войти
    </Button>
  );
}
