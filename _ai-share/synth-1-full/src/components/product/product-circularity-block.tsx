...
import { Recycle, RefreshCcw, Leaf, Wind } from 'lucide-react';
...
export const ProductCircularityBlock: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    <Card className="relative overflow-hidden border-2 border-green-50 bg-green-50/10 p-4 shadow-sm">
      ...
      <div className="mt-4 flex items-center border-t border-green-50 pt-3 text-[9px] font-bold uppercase italic text-slate-400">
        <span className="flex items-center gap-1.5">
          <Wind className="h-3 w-3" /> Circularity snapshot (demo)
        </span>
      </div>
    </Card>
  );
