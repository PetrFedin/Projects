      ...
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-[10px] font-bold uppercase text-muted-foreground">Points to earn</p>
          </div>
          <Badge
            variant="outline"
            className="gap-1 border-amber-300 bg-white text-[10px] text-amber-700"
          >
            <Zap className="h-3 w-3" />x{rewards.bonusMultiplier} Multiplier
          </Badge>
        </div>

        {rewards.perks.length > 0 && (
          <div className="flex flex-wrap gap-1.5 border-t border-amber-200/30 pt-2">
            {rewards.perks.map((p) => (
              <span
                key={p}
                className="flex items-center gap-1 text-[9px] font-medium text-amber-800"
              >
