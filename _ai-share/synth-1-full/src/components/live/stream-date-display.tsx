...
export function StreamDateDisplay({ date, isLive = false }: StreamDateDisplayProps) {
    ...
    if (isClient && date) {
      if (isLive) {
          ...
          const updateElapsedTime = () => {
             ...
             setFormattedDate(`${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`);
          }
  ...
  return <Tag className={`text-xs ${isLive ? 'bg-black/50 text-white font-mono px-2 py-1 rounded-md' : 'text-muted-foreground'}`}>{formattedDate}</Tag>;
}
