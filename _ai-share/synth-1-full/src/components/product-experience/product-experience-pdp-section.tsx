...
const GlassesVirtualTryOn = dynamic(
  ...
  { ssr: false, loading: () => <p className="text-sm text-muted-foreground py-4">Загрузка…</p> }
);
...
export function ProductExperiencePdpSection({ product, isQuickView }: Props) {
  ...
  const frameUrl = resolveEyewearFrameUrl(product) ?? (isEyewearCategory(product) ? DEMO_GLASSES_DATA_URL : undefined);
  ...
  return (
    <div
      className={cn(
        'space-y-6 w-full',
        isQuickView ? 'mt-4' : 'col-span-1 md:col-span-2 mt-6 max-w-5xl mx-auto'
      )}
    >
        ...
        <Card className="border-dashed">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs">
              ...
              <Link href={ROUTES.brand.virtualTryonGlasses} className="underline underline-offset-2">
                Хаб бренда
              </Link>
            </CardDescription>
          </CardHeader>
      ...
      {showFootwear && footwearExp && (
        <Card className="border-dashed">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs">
              Данные из <code className="text-[10px] bg-muted px-1 rounded">Product.footwear</code>.{' '}
          ...
          <CardContent className="max-h-[min(80vh,720px)] overflow-y-auto">
            <Footwear360PairingModule bundle={footwearExp.bundle} pairingPresets={footwearExp.pairing} />
          </CardContent>
        </Card>
      )}
    </div>
