...
const initialInventory: InventoryItem[] = allProducts.slice(0, 7).map((p, i) => ({
  ...
  promotion:
    i === 5
      ? { type: 'outlet', value: 30, requestedBy: 'brand', status: 'pending' }
      : i === 6
        ? {
            type: 'price_change',
            value: p.price * 1.15,
            requestedBy: 'brand',
            status: 'pending',
            comment: 'Корректировка в связи с новым курсом.',
          }
        : undefined,
  lastRejectionDate:
...
export function InventoryPageContent() {
  ...
  const category1Options = useMemo(
    () => (fullCategoryStructure ? ['all', ...Object.keys(fullCategoryStructure)] : []),
  ...
  const skuOptions = useMemo(
    () => inventory.map((p) => ({ value: p.sku, label: `${p.sku} - ${p.name}` })),
    [inventory]
  );
    ...
    <div className="space-y-6">
      ...
      <div className="mb-2 grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <Card className="group relative overflow-hidden rounded-[1.5rem] border-slate-100 bg-emerald-50/50 shadow-sm">
          ...
          <CardHeader className="p-3 pb-2">
            <Badge className="mb-1 w-fit border-none bg-emerald-100 text-[8px] font-black uppercase tracking-widest text-emerald-700">
              Dropshipping Hub
            </Badge>
          ...
          <CardContent className="p-3 pt-0">
            <div className="flex items-end justify-between">
              ...
              <Button
                variant="ghost"
                size="sm"
                className="h-7 text-[9px] font-bold uppercase text-emerald-600 hover:bg-emerald-100/50"
              >
                Трекинг
              </Button>
            </div>
        ...
        <CardHeader>
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <CardDescription>
                Просматривайте, редактируйте и управляйте своим каталогом.
              </CardDescription>
            </div>
          </div>
          ...
          {selectedRows.size > 0 && (
            <div className="flex items-center gap-2 pt-4">
              ...
              <Button size="sm" onClick={() => handleOpenBulkDialog('promo')}>
                Предложить промо
              </Button>
              <Button size="sm" onClick={() => handleOpenBulkDialog('outlet')}>
                Предложить в аутлет
              </Button>
