...
export function BrandSyncConfirmation({
  ...
  const handleConfirm = (link: PendingBrandLink) => {
    ...
    onConfirm?.(link);
    toast({ title: 'Подтверждено', description: `Синхронизация с брендом «${link.brandName}» активна.` });
  };

  const handleReject = (link: PendingBrandLink) => {
    setRejected((prev) => new Set(prev).add(link.storeEntryId));
    onReject?.(link);
    toast({ title: 'Отклонено', description: 'Связь с брендом отклонена.', variant: 'destructive' });
  };

  const handleClaim = (link: PendingBrandLink) => {
    setClaimed((prev) => new Set(prev).add(link.storeEntryId));
    onClaim?.(link);
    toast({ title: 'Связь установлена', description: `Вы подтвердили: «${link.storeNameInBrand}» — это ваш магазин. Бренд «${link.brandName}» получит уведомление.` });
  };

  const hasDirect = directMatches.length > 0 && directMatches.some((l) => !confirmed.has(l.storeEntryId) && !rejected.has(l.storeEntryId));
  const hasPossible = possibleMatches.length > 0 && possibleMatches.some((l) => !claimed.has(l.storeEntryId));
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Бренды добавили ваш магазин в свой профиль. Подтвердите связь или найдите себя в списках под другим названием.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-lg bg-white border flex items-center justify-center overflow-hidden">
                        {link.brandLogo ? (
                          <Image src={link.brandLogo} alt="" width={40} height={40} className="object-cover" />
                        ) : (
                          <Store className="h-5 w-5 text-slate-400" />
                        )}
                      </div>
                      <div>
                        <p className="font-semibold text-sm">{link.brandName}</p>
                    ...
                    <div className="flex items-center gap-2">
                      <Button size="sm" className="h-8 text-xs" onClick={() => handleConfirm(link)}>
                        <Check className="h-3.5 w-3.5 mr-1" /> Подтвердить
                      </Button>
                      <Button size="sm" variant="outline" className="h-8 text-xs" onClick={() => handleReject(link)}>
        ...
        {possibleMatches.length > 0 && (
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-500">
              В профилях брендов найдены магазины с похожим названием. Если это вы — нажмите «Это мы».
            </p>
            <div className="space-y-2">
              {possibleMatches.map((link) => {
                ...
                return (
                  ...
                  >
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          ...
                          {link.storeProductUrl && (
                            <a href={link.storeProductUrl} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline ml-1">
                              ссылка
                            </a>
                          )}
                        </p>
                    ...
                    >
                      <Link2 className="h-3.5 w-3.5 mr-1" /> Это мы
                    </Button>
                  </div>
                );
              })}
