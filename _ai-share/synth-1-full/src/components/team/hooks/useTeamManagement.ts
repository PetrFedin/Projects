...
export function useTeamManagement() {
  ...
  const [activeTab, setActiveTab] = useState<
    'active' | 'archive' | 'structure' | 'finance' | 'audit' | 'analytics' | 'onboarding' | 'matrix'
  >('active');
  ...
  const isProjectAdmin =
    user?.roles?.includes('admin') || user?.activeOrganizationId === 'org-hq-001';

  // Scoped to active organization
  const [activeOrgId, setActiveOrgId] = useState<string>(
    user?.activeOrganizationId || 'org-hq-001'
  );
  ...
  const filteredTeam = useMemo(() => {
    return activeTeam.filter((m: any) => {
      const searchStr =
        `${m.firstName} ${m.lastName} ${m.nickname} ${m.email} ${m.role} ${m.department || ''}`.toLowerCase();
  ...
  const handleArchive = async (memberId: string, archive: boolean) => {
    if (!user?.team) return;
    const updatedTeam = user.team.map((m) =>
      m.id === memberId ? { ...m, isArchived: archive } : m
    );
    await updateProfile({ team: updatedTeam as any });
  };
