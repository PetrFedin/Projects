...
export const Header = dynamic(() => import('@/components/layout/header'), {
  loading: () => (
    <header className="border-border-subtle bg-bg-surface/95 h-16 shrink-0 border-b" aria-hidden />
  ),
});
...
export const GlobalPodcastPlayer = dynamic(
  () => import('@/components/layout/global-podcast-player'),
  {
    ssr: false,
  }
);
