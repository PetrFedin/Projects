...
const ProductFilters = ({ 
  ...
  const filterConfig = useMemo(() => {
    const allColors = products.reduce((acc, p) => {
  ...
  const handleSingleSelectFilterChange = (category: string, option: string | string[] | number | number[] | undefined, type: 'radio' | 'slider' = 'radio') => {
     if (setContext) {
      setContext('');
    }
    setActiveFilters(prev => {
  ...
  const hasActiveFilters = Object.values(activeFilters).some(v => Array.isArray(v) && v.length > 0 && !(v.length === 2 && v.includes('in_stock') && v.includes('pre_order'))) || !!context || !!selectedCategory;
  
  const AccordionItemWithReset = ({value, title, children}: {value: string, title: string, children: React.ReactNode}) => (
      <AccordionItem value={value}>
          <div className="flex items-center justify-between p-2">
            <AccordionTrigger className="py-2 px-0 text-base font-semibold hover:no-underline flex-1 text-left">
                {title}
            </AccordionTrigger>
            {activeFilters[title] && (
                <Button variant="ghost" size="sm" className="h-auto px-2 py-0.5 text-xs mr-2" onClick={(e) => { e.stopPropagation(); resetSingleFilter(title); }}>
                    Сбросить
                </Button>
            )}
          </div>
    ...
    <div className="sticky top-20 flex flex-col gap-3">
        ...
        <Card>
            <CardContent className="p-2">
                ...
                <Accordion type="multiple" defaultValue={["Категории", "Цвет", "Материал", "Посадка", "Характеристики обуви", "Экологичность", VALUES_FILTER_KEY, "Технологии"]} className="w-full">
                     <AccordionItem value="Категории">
                        <AccordionTrigger className="p-2 text-base font-semibold hover:no-underline">Категории</AccordionTrigger>
                        <AccordionContent>
                           <CategoryTreeFilter 
                    ...
                    <AccordionItemWithReset value="Бренд" title="Бренд">
                         <div className="grid gap-2 max-h-60 overflow-y-auto px-2">
                            {(filterConfig['Бренд'].options as string[]).map(option => (
                                <div key={option} className="flex items-center space-x-2">
                                    ...
                                    />
                                    <Label htmlFor={`brand-${option}`} className="font-normal cursor-pointer">
                                        {option}
                                    </Label>
                                </div>
                            ))}
                    ...
                    <AccordionItemWithReset value="Посадка" title="Посадка/Силуэт">
                       ...
                       <AccordionItemWithReset value="Характеристики обуви" title="Характеристики обуви">
                            <ShoeFilters
                                heelHeight={localHeelHeight || activeFilters['Высота каблука'] as [number, number] | undefined}
                                ...
                                onSoleMaterialChange={(v, checked) => handleFilterChange('Материал подошвы', v, checked)}
                                upperMaterial={activeFilters['Материал верха'] as string[] | undefined}
                                onUpperMaterialChange={(v, checked) => handleFilterChange('Материал верха', v, checked)}
                            />
                       </AccordionItemWithReset>
                        ...
                        <div className="grid gap-2 max-h-60 overflow-y-auto px-2">
                          {VALUES_FILTER_OPTIONS.map((opt) => (
                            <div key={opt.id} className="flex items-center space-x-2">
                              <Checkbox
                                id={`values-${opt.id}`}
                                checked={(activeFilters[VALUES_FILTER_KEY] as string[])?.includes(opt.id) ?? false}
                                onCheckedChange={(checked) => handleFilterChange(VALUES_FILTER_KEY, opt.id, !!checked)}
                              />
                              <Label htmlFor={`values-${opt.id}`} className="font-normal cursor-pointer">{opt.shortLabel}</Label>
                            </div>
                          ))}
                        </div>
                    </AccordionItemWithReset>
                    ...
                    <AccordionItemWithReset value="Технологии" title="Технологии">
                        <div className="p-4 space-y-4">
                           <ArFilter 
                            ...
                            onCheckedChange={(checked) => handleSingleSelectFilterChange('AR', checked ? 'true' : undefined, 'radio')}
                           />
                           <div className="flex items-center space-x-2">
                                <Switch id="3d-mode" 
                                    checked={activeFilters['3D']?.includes('true')}
                                    onCheckedChange={(checked) => handleSingleSelectFilterChange('3D', checked ? 'true' : undefined, 'radio')}
                                />
                                <Label htmlFor="3d-mode" className="flex items-center gap-2 font-normal cursor-pointer">
                    ...
                    {Object.entries(filterConfig).filter(([title]) => !['Бренд', 'Цвет'].includes(title)).map(([title, config]) => (
                            ...
                            {config.type === 'color' ? (
                                ...
                                {(config.options as string[]).map(option => (
                                    <div key={option} className="flex items-center space-x-2">
                                        <Checkbox
                                            id={`${title}-${option}`}
                                            checked={(activeFilters[title] as string[])?.includes(option) ?? false}
