...
export function CreateParameterDialog({ isOpen, onOpenChange, parameterGroups, segments, onSave }: CreateParameterDialogProps) {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-6xl h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Заполните информацию о новом параметре, его веса для сегментов и варианты ответов.</DialogDescription>
        </DialogHeader>
        ...
        <ScrollArea className="h-full pr-4 -mr-6">
            <div className="space-y-6 px-1 py-4">
                    ...
                    <div className="space-y-2">
                     ...
                     <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                             <Select onValueChange={(v) => { setGroup(v); setIsNewGroup(v === 'new'); }} value={isNewGroup ? 'new' : group}>
                                <SelectTrigger>
                                    <SelectValue placeholder="Выберите группу..." />
                                </SelectTrigger>
                                <SelectContent>
                                    {parameterGroups.map(g => <SelectItem key={g} value={g}>{g}</SelectItem>)}
                ...
                <div className="space-y-2">
                    ...
                    <Textarea id="param-desc" value={description} onChange={(e) => setDescription(e.target.value)} />
                </div>
                    ...
                    <Label>Матрица весов для нового параметра</Label>
                     <div className="overflow-x-auto border rounded-lg">
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    {segments.map(s => <TableHead key={s.code} className="p-1 text-center text-xs font-mono">{s.code}</TableHead>)}
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                <TableRow>
                                    {weights.map((w, i) => (
                                        <TableCell key={i} className="p-0">
                                            <Select value={w} onValueChange={(v) => updateWeight(i, v)}>
                                                <SelectTrigger className={cn("border-none focus:ring-0 text-center font-mono h-auto p-2", valueClasses[w])}>
                                                    <SelectValue />
                                                </SelectTrigger>
                                                <SelectContent>
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                        ...
                        <Button variant="outline" size="sm" onClick={addOption}><PlusCircle className="mr-2 h-4 w-4" />Добавить ответ</Button>
                    </div>
                     <div className="overflow-x-auto border rounded-lg">
                        <Table>
                            <TableHeader>
                                ...
                                {options.map((opt, optIndex) => (
                                    <TableRow key={optIndex}>
                                        <TableCell className="p-1">
                                            <Input value={opt.label} onChange={(e) => updateOptionLabel(optIndex, e.target.value)} placeholder={`Вариант ${optIndex + 1}`}/>
                                        </TableCell>
                                        {opt.scores.map((score, segIndex) => (
                                            <TableCell key={segIndex} className="p-1">
                                                <Input type="number" min="0" max="100" value={score} onChange={(e) => updateScore(optIndex, segIndex, parseInt(e.target.value))} className="w-12 h-8 text-center" />
                                            </TableCell>
                                        ))}
                                        <TableCell className="p-1">
                                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => removeOption(optIndex)}><Trash2 className="h-4 w-4 text-destructive"/></Button>
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
