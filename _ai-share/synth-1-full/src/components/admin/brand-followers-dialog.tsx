...
import { LineChart, CartesianGrid, XAxis, YAxis, Tooltip, Legend, Line, ResponsiveContainer, Area, AreaChart } from 'recharts';
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle className="text-sm font-bold">Статистика подписчиков: {brand.name}</DialogTitle>
          <DialogDescription>
                 ...
                 <CardContent>
                    <ChartContainer config={{}} className="h-[250px] w-full">
                        <AreaChart data={followersChartData}>
                            ...
                            <Area dataKey="value" name="Подписчики" type="natural" fill="url(#colorFollowers)" stroke="hsl(var(--chart-2))" stackId="a" />
                        </AreaChart>
                    </ChartContainer>
                </CardContent>
            </Card>
