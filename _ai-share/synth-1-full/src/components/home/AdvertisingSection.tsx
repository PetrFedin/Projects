...
const heroAds = [
  ...
  },
  {
    ...
    description: "Примерьте коллекцию на свой 3D-аватар (XS-XXL) прямо сейчас. Идеальная посадка еще до покупки.",
...
const heroAdsB2B = [
  {
    ...
    description: "Интеллектуальная экосистема уровня JOOR и NuORDER: от виртуальных шоурумов до IoT-контроля фабрик.",
  ...
  },
  {
    ...
    description: "Встроенный факторинг, эскроу-счета и Syntha Wallet. Ваша финансовая безопасность в каждой транзакции.",
  ...
  },
  {
    ...
    description: "Единое хранилище ТЗ, лекал и сертификатов для всех ваших фабрик с Live-мониторингом линий.",
  ...
  },
  {
    ...
    description: "Презентуйте коллекции в 3D и автоматически пополняйте стоки ритейлеров через Universal Retail API.",
...
export function AdvertisingSection() {
  ...
  return (
    <section className="pt-6 pb-3 bg-white relative group/section overflow-hidden">
      <div className="container mx-auto px-4 relative">
        <div className="relative overflow-hidden rounded-xl border border-slate-100 bg-[#fcfcfc] group/card shadow-2xl shadow-slate-200/50 h-[500px] flex items-center">
          ...
          <div className="relative z-10 py-12 pr-12 pl-12 md:pl-[120px] w-full">
            <div className="max-w-4xl space-y-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  {viewRole === "b2b" && (
                    <div className="flex items-center gap-2 px-2.5 py-1 rounded-full bg-emerald-500 text-white text-[10px] font-bold uppercase tracking-wide shadow-lg shadow-emerald-500/20">
                      B2B
                    </div>
                  )}
                  {currentAd.subtitle && (
                ...
                <div className="relative">
                  <h1
                    className={cn(
                      "text-3xl md:text-5xl text-white leading-[0.9] tracking-tight font-bold",
              ...
              <div className="min-h-[80px] flex flex-col justify-center">
                {currentAd.description && (
                  <p className="text-slate-300 text-sm font-medium border-l-2 border-indigo-500/50 pl-6 max-w-2xl leading-relaxed">
                    "{currentAd.description}"
                  </p>
                )}
              </div>
                    ...
                    href={
                      currentAd.ctaText === "Панель управления"
                        ? (user?.roles?.includes('admin') ? "/admin/home" :
                           user?.roles?.includes('brand') ? "/brand" :
                           user?.roles?.includes('shop') ? "/shop/b2b" :
                           user?.roles?.includes('distributor') ? "/distributor" :
                           user?.roles?.includes('manufacturer') ? "/factory" :
                           user?.roles?.includes('supplier') ? "/factory" :
                           "/brand")
                        : (currentAd.id === 'b2b-fintech' ? "/brand/finance" :
                           currentAd.id === 'b2b-production-archive' ? "/factory/production" :
                           currentAd.id === 'b2b-commerce' ? "/brand/showroom" :
                           currentAd.id === 'b2b-distributors' ? "/distributor" :
                           currentAd.id === 'b2b-suppliers' ? "/factory/materials" :
                           currentAd.id === 'b2b-production' ? "/factory/production" :
                           currentAd.ctaLink)
                    }
            ...
            <div className="flex gap-3 items-center">
              {activeAds.map((_, i) => (
                <button
                  ...
                  className={cn(
                    "rounded-full transition-all duration-500 border border-white/10",
                    currentAdIndex === i
