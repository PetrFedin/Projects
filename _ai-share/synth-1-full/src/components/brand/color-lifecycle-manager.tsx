...
function ColorLifecycleCard({
  ...
  const handleDiscountChange = (
    ...
    setValue(`availableColors.${index}.discounts.${discountIndex}.${field}`, value, {
      shouldDirty: true,
    });
  };

  return (
    <Collapsible defaultOpen={false} className="rounded-lg border bg-muted/30">
        ...
        <div className="space-y-4 p-4 pt-0">
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                setValue(`availableColors.${index}.status`, checked ? 'active' : 'disabled')
              }
            />
          </div>
          <div className="flex items-center justify-between">
            <Label htmlFor={`base-${index}`} className="text-sm">
              Базовый цвет
            </Label>
            <Switch
              id={`base-${index}`}
              checked={color.isBase}
              onCheckedChange={(checked) => setValue(`availableColors.${index}.isBase`, checked)}
            />
          </div>
          <div className="space-y-2">
            ...
            >
              {Object.entries(statusLabels).map(([key, label]) => (
                <div key={key}>
                  <RadioGroupItem value={key} id={`lc-${index}-${key}`} className="peer sr-only" />
                  <Label
                    htmlFor={`lc-${index}-${key}`}
                    className="flex h-full flex-col items-center justify-between rounded-md border-2 border-muted bg-popover p-2 text-xs hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary"
                  >
                    {label}
                  </Label>
                </div>
            ...
            <div className="space-y-2 border-t pt-2">
              ...
              <div className="grid grid-cols-[1fr_2fr] items-center gap-2">
                <Input
                  ...
                  onChange={(e) =>
                    handleDiscountChange('percentage', parseInt(e.target.value, 10) || 0, 0)
                  }
                />
                <Popover>
                      ...
                      {date?.from ? (
                        date.to ? (
                          <>
                            ...
                            {format(date.to, 'LLL dd', { locale: ru })}
                          </>
                        ) : (
                          format(date.from, 'LLL dd', { locale: ru })
                        )
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      ...
                      onSelect={(range) => {
                        setDate(range);
                        if (range?.from)
                          handleDiscountChange('startDate', range.from.toISOString(), 0);
          ...
          <div className="grid grid-cols-2 gap-2 border-t pt-2">
            <div className="flex items-center gap-2">
              <Switch
                id={`noSale-${index}`}
                checked={color.noSale}
                onCheckedChange={(checked) => setValue(`availableColors.${index}.noSale`, checked)}
                disabled={lifecycleStatus === 'outlet'}
              />
              <Label htmlFor={`noSale-${index}`} className="text-xs font-normal">
            ...
            <div className="flex items-center gap-2">
              <Switch
                id={`carryOver-${index}`}
                checked={color.carryOver}
                onCheckedChange={(checked) =>
                  setValue(`availableColors.${index}.carryOver`, checked)
                }
                disabled={lifecycleStatus === 'outlet'}
              />
              <Label htmlFor={`carryOver-${index}`} className="text-xs font-normal">
                Перенести в новый сезон
              </Label>
            </div>
          </div>
        </div>
...
export default function ColorLifecycleManager({
  control,
  watch,
  setValue,
}: ColorLifecycleManagerProps) {
  ...
  const handleApplyToAll = () => {
    ...
    fields.forEach((_, index) => {
      if (index !== parseInt(masterColorIndex, 10)) {
        setValue(`availableColors.${index}.lifecycleStatus`, masterSettings.lifecycleStatus, {
          shouldDirty: true,
        });
        setValue(`availableColors.${index}.noSale`, masterSettings.noSale, { shouldDirty: true });
        setValue(`availableColors.${index}.carryOver`, masterSettings.carryOver, {
          shouldDirty: true,
        });
        setValue(`availableColors.${index}.status`, masterSettings.status, { shouldDirty: true });
        if (masterSettings.discounts) {
          setValue(
            `availableColors.${index}.discounts`,
            JSON.parse(JSON.stringify(masterSettings.discounts)),
            { shouldDirty: true }
          );
        } else {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Настройте жизненный цикл и статусы для каждого цветового варианта.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4 p-4">
        <div className="space-y-2 rounded-lg border border-dashed p-2">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={handleApplyToAll} disabled={masterColorIndex === undefined}>
