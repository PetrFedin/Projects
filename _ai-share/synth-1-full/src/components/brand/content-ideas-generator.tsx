    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-3">
          <div>
            <Label>Бренд</Label>
            <Input value={brandName} onChange={(e) => setBrandName(e.target.value)} placeholder="Syntha" />
          </div>
          <div>
            <Label>Тема</Label>
            <Input value={theme} onChange={(e) => setTheme(e.target.value)} placeholder="новая коллекция" />
          </div>
          <div>
            <Label>Канал</Label>
          ...
          <div className="space-y-3 mt-4">
            {ideas.map((idea, i) => (
              <div key={i} className="p-3 rounded-lg border bg-muted/30">
                ...
                <p className="text-sm text-muted-foreground mt-1">{idea.caption}</p>
                {idea.hashtags?.length && (
                  <p className="text-xs text-muted-foreground mt-2">{idea.hashtags.join(" ")}</p>
                )}
              </div>
            ))}
          </div>
