...
export function OrderChat() {
    ...
    const handleSendMessage = (e: React.FormEvent) => {
        e.preventDefault();
        if (newMessage.trim()) {
            setChatHistory(prev => [{ user: 'Вы (бренд)', text: newMessage, time: new Date().toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }) }, ...prev]);
            setNewMessage('');
        }
    };
                    ...
                    <ScrollArea className="flex-1 mb-4 pr-3">
                        <div className="space-y-4">
                            {chatHistory.map((msg, i) => (
                                <div key={i} className={cn("flex flex-col max-w-[80%]", msg.isSystem ? 'items-center w-full' : (msg.user.includes('бренд') ? 'items-end self-end' : 'items-start self-start'))}>
                                    <div className={cn("p-3 rounded-lg", msg.isSystem ? 'bg-transparent border-none p-0' : (msg.user.includes('бренд') ? 'bg-primary text-primary-foreground' : 'bg-background border'))}>
                                        {msg.isSystem ? (
