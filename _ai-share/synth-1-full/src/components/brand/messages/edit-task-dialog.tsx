...
export function EditTaskDialog({ isOpen, onOpenChange, task, participants, onSave, onDelete }: EditTaskDialogProps) {
    ...
    const handleSave = () => {
        ...
        if (status !== task.status) {
            history.push({ user, action: 'изменил(а) статус', date: now, from: statusOptions.find(s => s.value === task.status)?.label, to: statusOptions.find(s => s.value === status)?.label });
        }
        // Priority change
        if (priority !== task.priority) {
            history.push({ user, action: 'изменил(а) приоритет', date: now, from: priorityOptions.find(p => p.value === task.priority)?.label, to: priorityOptions.find(p => p.value === priority)?.label });
        }
        // Deadline change
        const oldDeadline = task.deadline ? new Date(task.deadline).toISOString().split('T')[0] : undefined;
        const newDeadline = deadline ? deadline.toISOString().split('T')[0] : undefined;
        if (oldDeadline !== newDeadline) {
            history.push({ user, action: 'изменил(а) срок', date: now, from: oldDeadline ? format(new Date(oldDeadline), 'PPP', {locale: ru}) : 'нет', to: newDeadline ? format(new Date(newDeadline), 'PPP', {locale: ru}) : 'нет' });
        }
        ...
        if (addedAssignees.length > 0) {
            history.push({ user, action: `добавил(а) исполнителей: ${addedAssignees.join(', ')}`, date: now });
        }
        if (removedAssignees.length > 0) {
            history.push({ user, action: `удалил(а) исполнителей: ${removedAssignees.join(', ')}`, date: now });
        }
        ...
        newSubtasks.forEach((newSub, id) => {
            ...
            if (!oldSub) {
                ...
                if (oldSub.text !== newSub.text) history.push({ user, action: `изменил(а) текст подзадачи c "${oldSub.text}" на "${newSub.text}"`, date: now });
                if (oldSub.assignee !== newSub.assignee) history.push({ user, action: `изменил(а) исполнителя подзадачи "${newSub.text}"`, date: now, from: oldSub.assignee || 'нет', to: newSub.assignee || 'нет' });
                if (oldSub.status !== newSub.status) history.push({ user, action: `изменил(а) статус подзадачи "${newSub.text}"`, date: now, from: statusOptions.find(s => s.value === oldSub.status)?.label, to: statusOptions.find(s => s.value === newSub.status)?.label });
            }
        });
    ...
    const addSubtask = () => {
        setSubtasks(prev => [...prev, { id: `new-sub-${Date.now()}`, text: '', assignee: '', status: 'pending' }]);
    };
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-2xl">
                        ...
                        <div className="space-y-2">
                            {subtasks.map((sub, index) => (
                                <div key={sub.id} className="flex items-center gap-2">
                                    <Select value={sub.status} onValueChange={(v) => updateSubtask(index, 'status', v)}>
                ...
                <DialogFooter className="justify-between pt-4 border-t">
                    <Button variant="destructive" onClick={() => onDelete(task.id)}>Удалить</Button>
