...
export function ManageParticipantsDialog({
  ...
  onParticipantsChange: (systemMessage: string) => void;
}) {
  ...
  const handleSaveChanges = () => {
    const initialParticipantNames = new Set(
      (conversation.participants || []).map((p: any) => p.name)
    );
    ...
    const removed = (conversation.participants || []).filter(
      (p: any) => !finalParticipantNames.has(p.name)
    );

    if (added.length > 0) {
      onParticipantsChange(
        `Вы добавили пользователя ${added.map((p: any) => `"${p.name}"`).join(', ')}.`
      );
    }
    if (removed.length > 0) {
      onParticipantsChange(
        `Вы удалили пользователя ${removed.map((p: any) => `"${p.name}"`).join(', ')}.`
      );
    }
        ...
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            {participants.map((p: any) => (
              <div key={p.name} className="flex items-center justify-between rounded-md border p-2">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Combobox
                ...
                className="flex-1"
              />
              <Button onClick={handleAddParticipant} disabled={!newMember}>
                Добавить
              </Button>
            </div>
          </div>
