...
export function POOverviewAndPayments({ orders, onPayClick, onNavigateToFinance }: POOverviewAndPaymentsProps) {
  const pending = orders.filter((o) => o.paymentStatus === 'pending' || o.paymentStatus === 'advance' || o.paymentStatus === 'overdue');
  ...
  return (
    <Card className="rounded-xl border border-slate-100 shadow-sm">
      <CardHeader className="py-2 px-4 flex flex-row justify-between items-center">
        ...
        {onNavigateToFinance && (
          <Button variant="ghost" size="sm" className="h-6 text-[8px]" onClick={onNavigateToFinance}>
            Финансы →
          </Button>
        )}
      </CardHeader>
      <CardContent className="px-4 pb-4">
        {totalPending > 0 && (
          <div className="flex items-center gap-2 p-2 rounded-lg bg-amber-50 border border-amber-100 mb-3">
            <AlertCircle className="h-4 w-4 text-amber-600" />
            <span className="text-[10px] font-bold">К оплате: {(totalPending / 1000).toFixed(0)}k ₽</span>
          </div>
        )}
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-[9px] text-right">Действие</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
                ...
                <TableCell>
                  <Badge
                    ...
                    className={cn(
                      ...
                      (o.paymentStatus === 'pending' || o.paymentStatus === 'advance') && 'border-amber-200 text-amber-600'
                    )}
                  >
                    {o.paymentStatus === 'paid' ? 'Оплачено' : o.paymentStatus === 'advance' ? 'Аванс' : o.paymentStatus === 'overdue' ? 'Просрочено' : 'Ожидает'}
                  </Badge>
                </TableCell>
                <TableCell className="text-right">
                  {o.paymentStatus !== 'paid' && onPayClick && (
                    <Button size="sm" variant="ghost" className="h-6 text-[8px]" onClick={() => onPayClick(o.id)}>
                      <CreditCard className="h-3 w-3 mr-0.5" /> Оплатить
                    </Button>
                  )}
                </TableCell>
              </TableRow>
