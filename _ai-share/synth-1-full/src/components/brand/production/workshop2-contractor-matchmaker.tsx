...
export function Workshop2ContractorMatchmaker({ articleDescription }: Workshop2ContractorMatchmakerProps) {
  ...
  const runMatchmaker = async () => {
    ...
    try {
      ...
      console.error(error);
      toast({ title: 'Ошибка', description: 'Не удалось подобрать подрядчиков', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-xs">Рекомендации на основе требований и оборудования</CardDescription>
          </div>
          <Button size="sm" variant="outline" onClick={runMatchmaker} disabled={loading} className="h-8 text-xs">
      ...
      {result && (
        <CardContent className="space-y-3 pt-0">
          {result.matches.map((match, idx) => {
            ...
            return (
              <div key={idx} className="bg-bg-surface2 rounded-md border border-border-subtle p-3 text-sm">
                ...
                <p className="text-text-secondary mt-2 text-xs leading-relaxed">{match.rationale}</p>
                
                {contractor?.capabilities && (
                  <div className="mt-3 flex flex-wrap gap-1">
                    {contractor.capabilities.map(cap => (
                      <Badge key={cap} variant="outline" className="text-[9px] h-4 px-1">{cap}</Badge>
                    ))}
                  </div>
                )}
              </div>
