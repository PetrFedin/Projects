...
export function Workshop2BottleneckDashboard({
  operations,
  className,
}: Workshop2BottleneckDashboardProps) {
  ...
  return (
    <div
      className={`border-border-default flex flex-col rounded-xl border bg-white p-4 shadow-sm ${className || ''}`}
    >
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-text-primary flex items-center gap-1.5 text-sm font-semibold">
          <LucideIcons.BarChart3 className="h-4 w-4 text-indigo-500" />
          Real-time WIP Status
        </h3>
        <div className="flex items-center gap-3">
          <span className="text-text-muted border-border-subtle rounded border bg-slate-50 px-2 py-1 text-[10px] font-medium">
            {progressPct}% Done
          </span>
          {hasBottleneck && (
            <span className="flex items-center gap-1 rounded border border-amber-200 bg-amber-50 px-2 py-1 text-[10px] font-semibold text-amber-700">
