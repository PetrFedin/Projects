    ...
    <div className="space-y-2 rounded-lg border border-zinc-200 bg-white p-2.5">
      ...
      {collectCategorySketchPinValidationIssues(activeAnn).length > 0 ? (
        <div className="rounded border border-amber-200 bg-amber-50 px-2 py-1 text-[10px] text-amber-950">
          Довести до готовности: {collectCategorySketchPinValidationIssues(activeAnn).join(', ')}.
        </div>
      ) : (
      ...
      {!readOnly && attributeOptions.some((o) => o.id === 'mat') ? (
        <Button
          ...
          onClick={() => updateAnnotation(activeAnn.annotationId, { linkedAttributeId: 'mat' })}
        >
          ...
          <div className="flex items-center gap-2">
            ...
            {!readOnly && (
              <Button
                ...
                variant="outline"
                size="sm"
                className="h-8 border-emerald-200 bg-emerald-50 px-2 text-[10px] text-emerald-800 hover:bg-emerald-100"
                  ...
                  if (typeof window !== 'undefined') {
                    ...
                    if (toast) {
                      toast({
                        ...
                        description:
                          'Режим распознавания слоев SVG активен. Кликните по детали (например, воротнику) на скетче для авто-привязки.',
                      });
                    }
                  }
                }}
