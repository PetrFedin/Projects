...
export const SECTION_TAB_ROLE_HINT: Record<string, string> = {
  ...
  material:
    'Состав, BOM, маркировка; подписи бренда и технолога (по правилам секции).',
  construction:
    'CAD/ZIP, мерки; подписывает технолог (роль в правилах секции).',
  assignment:
  ...
  sustainability:
    'Эко-показатели и цифровой паспорт изделия (DPP) — в шаге «Снабжение и закупка».',
  b2b_sales:
    'Оптовые предзаказы и витрина B2B — в шаге «План заказа» разработки артикула.',
};
