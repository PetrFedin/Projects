...
export interface ExtendedAnalyticsProps {
  ...
  sampleStatuses: Array<{ skuId: string; skuName: string; status: string; slaOverdue: boolean; factory: string; collection: string }>;
  productionOrders: Array<{ id: string; status: string; qty: number; collection: string; factory: string }>;
...
export function ProductionExtendedAnalytics({
  ...
  const handleExport = (format: 'csv' | 'pdf') => {
    ...
    exportToCSV(rows, [{ key: 'kpi', label: 'KPI' }, { key: 'value', label: 'Значение' }], 'analytics');
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" className="h-8 text-[9px] font-bold uppercase gap-1" onClick={() => handleExport('pdf')}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {[
          { key: 'production', label: 'Производство', value: productionKpis.production, icon: Package, color: 'indigo' },
          { key: 'cargo', label: 'В пути', value: productionKpis.cargo, icon: Truck, color: 'cyan' },
          { key: 'qc', label: 'QC', value: productionKpis.qc, icon: ShieldCheck, color: 'emerald' },
          { key: 'finance', label: 'К оплате', value: productionKpis.finance, icon: CreditCard, color: 'rose' },
          { key: 'risk', label: 'Риск', value: productionKpis.risk, icon: AlertTriangle, color: 'amber' },
          { key: 'efficiency', label: 'Эффективность', value: productionKpis.efficiency, icon: TrendingUp, color: 'green' }
        ].map((item) => (
          <Card
            key={item.key}
            className={cn(
              'border border-slate-100 shadow-sm rounded-xl cursor-pointer transition-all hover:border-indigo-200',
          ...
          >
            <CardContent className="p-4">
              ...
              <p className="text-[9px] font-bold uppercase text-slate-400 mt-1">{item.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="border border-slate-100 shadow-sm rounded-2xl overflow-hidden">
          <CardHeader className="p-4 border-b border-slate-50 bg-slate-50/30">
            <CardTitle className="text-[11px] font-black uppercase">On-time delivery по фабрикам</CardTitle>
            <CardDescription className="text-[9px]">% вовремя доставленных партий</CardDescription>
          </CardHeader>
          <CardContent className="p-4">
            <div className="h-[220px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={otifData} layout="vertical" margin={{ top: 5, right: 20, left: 80, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                  <XAxis type="number" domain={[0, 100]} tick={{ fontSize: 10 }} />
                  <YAxis dataKey="factory" type="category" width={75} tick={{ fontSize: 9 }} />
                  <Tooltip contentStyle={{ fontSize: 10 }} formatter={(v: number) => [`${v}%`, 'On-time']} />
        ...
        <Card className="border border-slate-100 shadow-sm rounded-2xl overflow-hidden">
          <CardHeader className="p-4 border-b border-slate-50 bg-slate-50/30">
            ...
            <CardDescription className="text-[9px]">Доля артикулов, средний QC score</CardDescription>
          </CardHeader>
          <CardContent className="p-4">
            <div className="h-[220px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(v: number, n: string, p: { payload: { score: number } }) => [`${v} арт., score ${p.payload.score}%`, n]} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
      ...
      <Card className="border border-slate-100 shadow-sm rounded-2xl overflow-hidden">
        <CardHeader className="p-4 border-b border-slate-50 bg-slate-50/30">
          ...
          <CardDescription className="text-[9px]">Плановый объём против фактического производства</CardDescription>
        </CardHeader>
        <CardContent className="p-4">
          <div className="h-[200px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={planVsFact} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <Legend />
                <Line type="monotone" dataKey="plan" stroke="#4f46e5" strokeWidth={2} name="План" dot={{ r: 4 }} />
                <Line type="monotone" dataKey="fact" stroke="#10b981" strokeWidth={2} name="Факт" dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
