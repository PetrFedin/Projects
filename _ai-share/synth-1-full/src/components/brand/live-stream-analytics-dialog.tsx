...
export function LiveStreamAnalyticsDialog({
  isOpen,
  onOpenChange,
  initialMetric = 'views',
}: LiveStreamAnalyticsDialogProps) {
          ...
          <div className="mb-4 flex flex-wrap gap-3">
            {Object.entries(metricConfig).map(([key, config]) => (
              <div key={key} className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor={key} className="flex cursor-pointer items-center gap-1.5">
            ...
            <ResponsiveContainer>
              <LineChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                ...
                <YAxis
                  yAxisId="left"
                  orientation="left"
                  stroke={metricConfig.views.color}
                  tick={{ fontSize: 12 }}
                />
                <YAxis
                  yAxisId="right"
                  orientation="right"
                  stroke={metricConfig.reactions.color}
                  tick={{ fontSize: 12 }}
                />
                <Tooltip
                  formatter={(value, name) => [value, metricConfig[name as MetricKey]?.label]}
                />
                <Legend />
                ...
                {selectedMetrics.includes('comments') && (
                  <Line
                    ...
                    dataKey="comments"
                    name="Комментарии"
                    stroke={metricConfig.comments.color}
                    strokeWidth={2}
                    dot={false}
                  />
                )}
                {selectedMetrics.includes('cart') && (
                  <Line
                    yAxisId="right"
                    type="monotone"
                    dataKey="cart"
                    name="Товар выбран"
                    stroke={metricConfig.cart.color}
                    strokeWidth={2}
                    dot={false}
                  />
                )}
