...
function RrpSuggestionDialog({
  isOpen,
  onOpenChange,
  suggestion,
  onAccept,
  isLoading,
}: RrpSuggestionDialogProps) {
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Wand2 className="text-accent" /> Рекомендация AI по цене
          </DialogTitle>
        </DialogHeader>
        {isLoading && (
        ...
        {suggestion && !isLoading && (
          <div className="space-y-4 py-4">
            ...
            <p className="text-center text-sm font-bold">
              {suggestion.suggestedRrp.toLocaleString('ru-RU')} ₽
            </p>
            <Alert>
        ...
        <DialogFooter>
          ...
          <Button
            disabled={!suggestion}
            onClick={() => {
              if (suggestion) onAccept(suggestion.suggestedRrp);
            }}
          >
            Применить
          </Button>
...
const potentialConfig = {
  Хит: {
    icon: Flame,
    color: 'text-red-500',
    description: 'Высокий спрос, высокая маржа. Рекомендуется увеличить закупку.',
  },
  Стабильный: {
    icon: TrendingUp,
    color: 'text-green-500',
    description: 'Стабильные продажи, хорошая маржинальность. Надежный выбор.',
  },
  Рискованный: {
    icon: AlertTriangle,
    color: 'text-amber-500',
    description: 'Низкая прогнозируемая маржа или sell-out. Требует пересмотра.',
  },
};
...
const riskConfig = {
  ...
  Высокий: {
    color: 'text-red-500',
    description: 'Высокий риск Out-of-Stock или низкая оборачиваемость.',
  },
};
...
export default function MarginAnalysisPage() {
  ...
  const [rrpSuggestion, setRrpSuggestion] = useState<{
    suggestedRrp: number;
    reasoning: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const initialInventoryData: ProductWithAnalytics[] = allProducts.map((p, i) => ({
        ...
        aiRecommendation:
          Math.round(
            ((Math.floor(Math.random() * 20) + 5) * 4 - Math.floor(Math.random() * 30)) / 5
          ) * 5,
  ...
  useEffect(() => {
    ...
    const calculatedInventory = filteredBaseInventory.map((p) => {
      ...
      const forecastProfit =
        (finalRrp - cost) * orderQty * (forecastSellOut / 100) * modifier.revenue;
  ...
  const compareScenario1Totals = compareScenarioIds[0]
    ? calculateScenarioTotals(compareScenarioIds[0])
    : null;
  const compareScenario2Totals = compareScenarioIds[1]
    ? calculateScenarioTotals(compareScenarioIds[1])
    : null;
  ...
  const handleSaveScenario = () => {
    const newScenario: SavedScenario = {
      ...
      name: scenarioName,
      edits: JSON.parse(JSON.stringify(editedProducts)) as Record<
        string,
        Partial<ProductWithAnalytics>
      >,
    };
    ...
    toast({
      ...
      description: `Сценарий "${scenarioName}" был успешно сохранен.`,
    });
  };
  ...
  const handleApplyAllRecommendations = () => {
    ...
    baseInventory.forEach((p) => {
      newEdits[p.id] = {
        ...(editedProducts[p.id] || {}),
        orderQty: p.aiRecommendation > 0 ? p.aiRecommendation : 0,
      };
    });
    setEditedProducts(newEdits);
    toast({
      title: 'Рекомендации AI применены',
      description: 'Количество товаров в заказе обновлено.',
    });
  };
  ...
  const handleOptimizeOrder = () => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      toast({
        title: 'Заказ оптимизирован',
        description: 'Количества обновлены для соответствия бюджету и целям.',
      });
    }, 1500);
  };
  ...
  const seasons = useMemo(
    () => ['all', ...new Set(baseInventory.map((d) => d.season))],
    [baseInventory]
  );
  const brands = useMemo(
    () => ['all', ...new Set(baseInventory.map((d) => d.brand))],
    [baseInventory]
  );
  ...
  return (
    <TooltipProvider>
      <div className="space-y-4">
        <header>
          <h1 className="font-headline text-base font-bold">Маржинальность и Прогноз</h1>
          <p className="text-muted-foreground">
            Управляйте ценообразованием, планируйте прибыльность и принимайте решения о закупках.
          </p>
        </header>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard
            ...
            value={`${totals.totalCost.toLocaleString('ru-RU')} ₽`}
            description={`Всего ${totals.totalQty.toLocaleString('ru-RU')} ед.`}
            icon={ShoppingCart}
          />
          <StatCard
            title="Прогноз выручки"
            value={`${totals.totalRevenue.toLocaleString('ru-RU')} ₽`}
            description={`Сценарий: ${activeScenario}`}
            icon={DollarSign}
          />
          <StatCard
            title="Прогноз прибыли"
            value={`${totals.totalProfit.toLocaleString('ru-RU')} ₽`}
            description="С учетом sell-through"
            icon={TrendingUp}
          />
          <StatCard
            title="Средняя маржа"
            value={`${avgMargin.toFixed(1)}%`}
            description="По всему заказу"
            icon={Percent}
          />
        </div>
        {(budgetExceeded || profitTargetNotMet) && (
          <Alert
            variant={budgetExceeded ? 'destructive' : 'default'}
            className={
              !budgetExceeded && profitTargetNotMet
                ? 'border-amber-500/50 bg-amber-100 text-amber-900 dark:bg-amber-900/50 dark:text-amber-200'
                : ''
            }
          >
            ...
            <AlertDescription>
              {budgetExceeded &&
                `Бюджет превышен на ${(totals.totalCost - budget).toLocaleString('ru-RU')} ₽. `}
              {profitTargetNotMet &&
                `Цель по прибыли не достигнута (не хватает ${(profitTarget - totals.totalProfit).toLocaleString('ru-RU')} ₽). `}
            ...
            <Card>
              <CardHeader>
                <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
                  <div>
                    ...
                    <CardDescription>
                      Введите количество к заказу и РРЦ для расчета маржинальности.
                    </CardDescription>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-2 rounded-md border p-1">
                      <Label className="pl-2 text-xs font-semibold">Сценарий:</Label>
                      <ToggleGroup
                        type="single"
                        value={activeScenario}
                        onValueChange={(value: Scenario) => value && setActiveScenario(value)}
                        className="mt-0"
                      >
                        <ToggleGroupItem
                          value="pessimistic"
                          aria-label="Пессимистичный"
              ...
              <CardContent className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="sticky left-0 z-10 min-w-[250px] bg-background">
                        Товар
                      </TableHead>
                    ...
                    {Object.entries(groupedByBrand).map(([brand, brandProducts]) => {
                      ...
                      return (
                        <React.Fragment key={brand}>
                          <TableRow
                            className="bg-muted/50 font-semibold hover:bg-muted/50"
                            onClick={() =>
                              setOpenBrandGroups((p) => ({ ...p, [brand]: !p[brand] }))
                            }
                          >
                            ...
                            <TableCell className="text-center">
                              {brandTotals.totalQty.toLocaleString('ru-RU')}
                            </TableCell>
                            <TableCell></TableCell>
                            <TableCell></TableCell>
                            <TableCell></TableCell>
                            ...
                            <TableCell className="text-right">
                              {brandTotals.totalRevenue.toLocaleString('ru-RU')} ₽
                            </TableCell>
                            <TableCell className="text-right">
                              {brandTotals.totalProfit.toLocaleString('ru-RU')} ₽
                            </TableCell>
                          </TableRow>
                          {isGroupOpen &&
                            brandProducts.map((item) => {
                              ...
                              return (
                                <TableRow
                                  ...
                                  className={cn(
                                    'cursor-pointer',
                                    selectedSku?.id === item.id && 'bg-secondary'
                                  )}
                                >
                                  <TableCell className="sticky left-0 z-10 bg-inherit font-medium">
                                    <div className="flex items-center gap-3">
                                      ...
                                      />
                                      <div>
                                        <p className="text-sm font-semibold group-hover/link:underline">
                                          {item.name}
                                        </p>
                                  ...
                                  <TableCell>
                                    <Badge
                                      variant="outline"
                                      className={`${lifecycleConfig[item.lifecycleStage].color}`}
                                    >
                                      {lifecycleConfig[item.lifecycleStage].label}
                                    </Badge>
                                  </TableCell>
                                  <TableCell>
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <div
                                          className={cn(
                                            'flex cursor-help items-center gap-1.5 font-semibold',
                                            potentialColor
                                          )}
                                        >
                                      ...
                                      <TooltipContent>
                                        <p>{potentialConfig[item.aiPotential].description}</p>
                                      </TooltipContent>
                                    </Tooltip>
                                  </TableCell>
                                  <TableCell>
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <div className={cn('cursor-help font-semibold', riskColor)}>
                                          {item.riskProfile}
                                        </div>
                                      </TooltipTrigger>
                                      <TooltipContent>
                                        <p>{riskConfig[item.riskProfile].description}</p>
                                      </TooltipContent>
                                    </Tooltip>
                                  </TableCell>
                                  <TableCell>
                                    <div
                                      className={cn(
                                        ...
                                        getStockIndicatorClass(item.stock)
                                      )}
                                    >
                                      {item.stock}
                                    </div>
                                      ...
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                          <Button
                                            ...
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              handleQuantityChange(item.id, item.aiRecommendation);
                                            }}
                                          >
                                            <BrainCircuit className="h-4 w-4" />
                                          </Button>
                                        ...
                                        <TooltipContent>
                                          <p>
                                            Применить рекомендацию AI ({item.aiRecommendation} шт.)
                                          </p>
                                        </TooltipContent>
                                      </Tooltip>
                                    </div>
                                  ...
                                  <TableCell>
                                    {(item.productionCost || item.price * 0.4).toLocaleString(
                                      'ru-RU'
                                    )}{' '}
                                    ₽
                                  </TableCell>
                                  <TableCell>
                                    <div className="relative">
                                      <Input
                                        type="number"
                                        value={Math.round(item.rrp)}
                                        onClick={(e) => e.stopPropagation()}
                                        onChange={(e) =>
                                          handleRrpChange(item.id, Number(e.target.value))
                                        }
                                  ...
                                  <TableCell
                                    className={cn('font-semibold', getMarkupColor(item.markup))}
                                  >
                                    {item.markup.toFixed(1)}%
                                  </TableCell>
                                  <TableCell
                                    className={cn('font-semibold', getMarginColor(item.margin))}
                                  >
                                  ...
                                  <TableCell className="font-semibold">
                                    {item.forecastProfit.toLocaleString('ru-RU')} ₽
                                  </TableCell>
                                </TableRow>
                              );
                            })}
                  ...
                  <TableFooter>
                    <TableRow className="text-base font-bold">
                      <TableCell className="sticky left-0 z-10 bg-muted">Итого</TableCell>
                      <TableCell colSpan={4}></TableCell>
                      <TableCell className="text-center">
                        {totals.totalQty.toLocaleString('ru-RU')}
                      </TableCell>
                      <TableCell className="text-right">
                      ...
                      <TableCell className="text-right">
                        {totals.totalRevenue.toLocaleString('ru-RU')} ₽
                      </TableCell>
                      <TableCell className="text-right">
                        {totals.totalProfit.toLocaleString('ru-RU')} ₽
                      </TableCell>
                    </TableRow>
                  </TableFooter>
                </Table>
        ...
        <Dialog open={isCompareOpen} onOpenChange={setIsCompareOpen}>
          <DialogContent className="max-w-4xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Выберите два сценария для сравнения их ключевых показателей.
              </DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-2 gap-3 py-4">
              <Select onValueChange={(val) => setCompareScenarioIds((prev) => [val, prev[1]])}>
                <SelectTrigger>
                  <SelectValue placeholder="Выберите сценарий 1..." />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="default">Основной заказ</SelectItem>
                  {savedScenarios.map((s) => (
              ...
              <Select onValueChange={(val) => setCompareScenarioIds((prev) => [prev[0], val])}>
                <SelectTrigger>
                  <SelectValue placeholder="Выберите сценарий 2..." />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="default">Основной заказ</SelectItem>
                  {savedScenarios.map((s) => (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">
                      {savedScenarios.find((s) => s.id === compareScenarioIds[0])?.name ||
                        'Основной заказ'}
                    </TableHead>
                    <TableHead className="text-right">
                      {savedScenarios.find((s) => s.id === compareScenarioIds[1])?.name ||
                        'Основной заказ'}
                    </TableHead>
                ...
                <TableBody>
                  <TableRow>
                    ...
                    <TableCell className="text-right">
                      {compareScenario1Totals.totalCost.toLocaleString('ru-RU')} ₽
                    </TableCell>
                    <TableCell className="text-right">
                      {compareScenario2Totals.totalCost.toLocaleString('ru-RU')} ₽
                    </TableCell>
                    <TableCell className="text-right">
                      {(
                        compareScenario2Totals.totalCost - compareScenario1Totals.totalCost
                  ...
                  <TableRow>
                    ...
                    <TableCell className="text-right">
                      {compareScenario1Totals.totalRevenue.toLocaleString('ru-RU')} ₽
                    </TableCell>
                    <TableCell className="text-right">
                      {compareScenario2Totals.totalRevenue.toLocaleString('ru-RU')} ₽
                    </TableCell>
                    <TableCell className="text-right">
                      {(
                        compareScenario2Totals.totalRevenue - compareScenario1Totals.totalRevenue
                      ).toLocaleString('ru-RU')}{' '}
                      ₽
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    ...
                    <TableCell className="text-right">
                      {compareScenario1Totals.totalProfit.toLocaleString('ru-RU')} ₽
                    </TableCell>
                    <TableCell className="text-right">
                      {compareScenario2Totals.totalProfit.toLocaleString('ru-RU')} ₽
                    </TableCell>
                    <TableCell className="text-right">
                      {(
                        compareScenario2Totals.totalProfit - compareScenario1Totals.totalProfit
                      ).toLocaleString('ru-RU')}{' '}
                      ₽
                    </TableCell>
                  </TableRow>
        ...
        <Dialog open={!!selectedSku} onOpenChange={(open) => !open && setSelectedSku(null)}>
          <DialogContent className="flex h-[90vh] max-w-5xl flex-col">
            ...
            <div className="-mr-6 flex-1 overflow-y-auto pr-4">
              {selectedSku && (
                <SkuAnalytics
                  brandProducts={baseInventory}
                  initialSku={selectedSku.id}
                  isDialogMode
                />
              )}
            </div>
