...
function SizeGrid({
  ...
  product: RailProduct;
  onBulkAddToCart: (items: { variantId: string; size: string; quantity: number }[]) => void;
}) {
  ...
  return (
    <div className="border-t bg-card p-3">
          ...
          <TableHeader>
            <TableRow>
              ...
              {allSizes.map((size, index) => (
                <TableHead key={`${size}-${index}`} className="w-20 p-1 text-center">
                  {size}
                </TableHead>
              ))}
            </TableRow>
              ...
              <TableRow key={variant.variantId}>
                <TableCell className="p-1">
                  <div className="flex items-center gap-2">
                    ...
                    ></div>
                    <span className="truncate text-xs">{variant.colorName}</span>
                  </div>
                </TableCell>
                {allSizes.map((size, index) => (
                  <TableCell key={`${variant.variantId}-${size}-${index}`} className="p-1">
                    {variant.sizeRun.includes(size) ? (
                      ...
                      <div className="flex h-7 w-12 items-center justify-center text-muted-foreground">
                        -
                      </div>
                    )}
                  </TableCell>
      ...
      <Button size="sm" className="mt-2 w-full" onClick={handleBulkAdd}>
        Добавить в заказ
      </Button>
    </div>
  );
...
const ProductDetail = forwardRef<
  ...
  {
    product: RailProduct;
    onAddToCart: (variantId: string, size: string, quantity: number) => void;
    onBack: () => void;
  }
>(({ product, onAddToCart, onBack }, ref) => {
  ...
  return (
    <motion.div
      ...
      className="absolute right-0 top-0 z-10 flex h-full w-full flex-col border-l bg-card shadow-2xl md:w-1/2 lg:w-1/3"
    >
      ...
      <ScrollArea className="flex-1">
        <div className="space-y-4 p-4">
          <div className="relative aspect-[4/5] w-full overflow-hidden rounded-lg bg-muted">
            <Image
              src={activeVariant.images[0] || product.heroRailImageUrl}
              alt={product.title}
              fill
            ...
            {product.variants.map((v, i) => (
              <button
                ...
                className={cn(
                  ...
                  activeVariant.variantId === v.variantId ? 'border-primary' : 'border-border'
                )}
          ...
          <p className="text-sm font-bold">
            {product.pricing.currency} {product.pricing.currentPrice.toLocaleString('ru-RU')}
          </p>
            ...
            <div className="grid grid-cols-3 gap-2">
              {activeVariant.sizeRun.map((size, index) => (
                <div key={`${size}-${index}`} className="flex flex-col gap-1">
                  <Label htmlFor={`qty-${size}`} className="text-center text-xs">
                    {size}
                  </Label>
                  <Input
                    id={`qty-${size}`}
            ...
            {lookProducts.map((p) => (
              <div key={p.id} className="flex gap-3">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => onAddToCart(p, p.sizes?.[0].name || 'One Size', 1)}
                >
...
export function BrandRail() {
  ...
  const allBrands = useMemo(
    () => [...new Set(PRODUCTS.map((p) => p.brand))].map((b) => ({ value: b, label: b })),
    []
  );
  ...
  return (
    <section className="flex h-full flex-col overflow-hidden bg-secondary/30 p-4 md:p-4 lg:p-4">
      <div className="mb-4">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-2 rounded-md border bg-background p-1">
            ...
            <Button
              variant={viewMode === 'looks' ? 'secondary' : 'ghost'}
              size="sm"
              className="h-6 px-2 text-xs"
              onClick={() => setViewMode('looks')}
            >
              <Trello className="mr-1.5 h-4 w-4" />
              Образы
            </Button>
          </div>
        </div>
      </div>
      <div className="relative -mr-4 flex-1 overflow-y-auto pr-4">
        <AnimatePresence>
          <div className="space-y-4">
            {Object.entries(groupedItems).map(([groupName, items]) => (
              <div key={groupName}>
                {groupBy !== 'none' && <h3 className="mb-2 font-semibold">{groupName}</h3>}
                <div
                  className={cn(
                    'grid gap-3',
                    viewMode === 'products'
                  ...
                  {items.map((item) => {
                    if ('variants' in item) {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            {totalInCart > 0 && (
                              <div className="absolute left-2 top-2 flex h-8 w-8 items-center justify-center rounded-full bg-primary/80 text-sm font-bold text-primary-foreground">
                                {totalInCart}
                              </div>
                            )}
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-3">
                              ...
                              <p className="text-xs text-white/80">
                                ...
                                {item.pricing.currentPrice.toLocaleString('ru-RU')}
                              </p>
                            </div>
                          </div>
                          <div className="mt-auto">
                            <Button
                              ...
                              onClick={() =>
                                setExpandedSizeGridId(
                                  expandedSizeGridId === item.id ? null : item.id
                                )
                              }
                            >
                              Размеры
                            ...
                            {expandedSizeGridId === item.id && (
                              <SizeGrid product={item} onBulkAddToCart={handleBulkAddToCart} />
                            )}
                          </div>
                        </div>
                      );
                      ...
                      return (
                        <div
                          key={item.id}
                          className="group flex cursor-pointer flex-col overflow-hidden rounded-lg border bg-card shadow-sm"
                          onClick={() => handleSelectLook(item.id)}
                        >
      ...
      <AnimatePresence>
        ...
        {totalQuantity > 0 && (
          ...
          >
            <div className="flex items-center gap-3 rounded-lg border bg-card p-3 shadow-2xl">
              <div className="flex items-center gap-3 text-sm">
                <p>
                  Товаров: <span className="font-bold">{b2bCart.length}</span>
                </p>
                <p>
                  Всего единиц: <span className="font-bold">{totalQuantity}</span>
                </p>
                <p>
                  Сумма: <span className="font-bold">{totalAmount.toLocaleString('ru-RU')} ₽</span>
                </p>
              </div>
              <Button asChild>
