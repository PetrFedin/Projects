...
import { PlusCircle, Loader2, Wand2, TrendingUp, Cpu, Users, ShoppingBag, Eye, Calendar as CalendarIcon, X, Rocket } from 'lucide-react';
...
export function PromotionDialog({ product: initialProduct, isOpen, onOpenChange, initialType, initialName }: PromotionDialogProps) {
  const { toast } = useToast();
  const [brandProducts, setBrandProducts] = useState<Product[]>([]);
  const [selectedProductIds, setSelectedProductIds] = useState<string[]>(initialProduct ? [initialProduct.id] : []);
  const [promotionType, setPromotionType] = useState<PromotionType | 'discount' | 'outlet' | 'promo_code' | 'kickstarter_boost'>(initialType || 'catalog_boost');
  ...
  const [aiForecast, setAiForecast] = useState<{views: string, clicks: string, conversion: string} | null>(null);

  const [scenarios, setScenarios] = useState<{ condition: string; value: number }[]>([
    { condition: 'default', value: 30 },
  ]);

  const [targetType, setTargetType] = useState<'brand' | 'products' | 'categories' | 'kickstarter'>(initialProduct ? 'products' : (initialType === 'kickstarter_boost' ? 'kickstarter' : 'brand'));
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  
  const productOptions = useMemo(() => brandProducts.map(p => ({ value: p.id, label: `${p.sku} - ${p.name}` })), [brandProducts]);
  const categoryOptions = useMemo(() => [...new Set(brandProducts.map(p => p.category || p.subcategory).filter(Boolean) as string[])].map(c => ({value: c, label: c})), [brandProducts]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTargetType(initialProduct ? 'products' : (initialType === 'kickstarter_boost' ? 'kickstarter' : 'brand'));
      ...
      <DialogContent className="sm:max-w-4xl">
        ...
        <div className="space-y-6 py-4 max-h-[70vh] overflow-y-auto pr-4">
             <div className="space-y-2">
                ...
                <RadioGroup value={targetType} onValueChange={(v) => setTargetType(v as any)} className="flex gap-3">
                    <div className="flex items-center space-x-2"><RadioGroupItem value="brand" id="target-brand" /><Label htmlFor="target-brand">Весь бренд</Label></div>
                    <div className="flex items-center space-x-2"><RadioGroupItem value="categories" id="target-categories" /><Label htmlFor="target-categories">Категории</Label></div>
                    <div className="flex items-center space-x-2"><RadioGroupItem value="products" id="target-products" /><Label htmlFor="target-products">Товары</Label></div>
                    <div className="flex items-center space-x-2"><RadioGroupItem value="kickstarter" id="target-kickstarter" /><Label htmlFor="target-kickstarter">Kickstarter</Label></div>
                </RadioGroup>
            </div>
            ...
            {!isActionRequest && (
                <div className="space-y-2">
                    ...
                    <Input id="promo-name" value={promotionName} onChange={(e) => setPromotionName(e.target.value)} placeholder={'Имиджевая кампания'} />
                </div>
            )}
             <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-2">
                    <Label>Тип акции</Label>
                    <Select onValueChange={(v) => setPromotionType(v as PromotionType)} value={promotionType}>
                        <SelectTrigger>
                 ...
                 <div className="grid grid-cols-2 gap-2">
                     {!isActionRequest && (
                        <>
                            <div className="space-y-2">
                                <Label>Бюджет, ₽</Label>
                                <Input type="number" value={budget} onChange={(e) => setBudget(Number(e.target.value))} placeholder="5000"/>
                            </div>
                            <div className="space-y-2">
                                <Label>Ставка ({budgetModel.toUpperCase()})</Label>
                                <Input type="number" value={bid} onChange={(e) => setBid(Number(e.target.value))} placeholder="300"/>
                            </div>
                        </>
                     )}
                </div>
                        ...
                        <Label>Дата переноса в Аутлет</Label>
                         <Popover>
                            ...
                            >
                                ...
                                {date?.from ? format(date.from, "PPP", { locale: ru }) : <span>Выберите дату</span>}
                            </Button>
                            </PopoverTrigger>
                            <PopoverContent className="w-auto p-0" align="start">
                                <Calendar mode="single" selected={date?.from} onSelect={(d) => setDate(prev => ({from: d, to: prev?.to}))} initialFocus />
                            </PopoverContent>
                        </Popover>
                    </div>
                    <div className="space-y-2">
                        ...
                        {scenarios.map((scenario, index) => (
                            <div key={index} className="flex items-center gap-2">
                                 <Select value={scenario.condition} onValueChange={(v) => updateScenario(index, 'condition', v)}>
                                    <SelectTrigger><SelectValue /></SelectTrigger>
                                    <SelectContent>
                                        {scenarioConditions.map(c => <SelectItem key={c.value} value={c.value} disabled={index > 0 && c.value === 'default'}>{c.label}</SelectItem>)}
                                    </SelectContent>
                                </Select>
                                 <div className="relative">
                                    <Input type="number" value={scenario.value || ''} onChange={(e) => updateScenario(index, 'value', e.target.value)} className="w-28 pr-6"/>
                                    <span className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">%</span>
                                </div>
                                {index > 0 && <Button type="button" variant="ghost" size="icon" className="h-8 w-8" onClick={() => removeScenario(index)}><Trash2 className="h-4 w-4 text-destructive" /></Button>}
                            </div>
                        ))}
                         <Button type="button" variant="outline" size="sm" onClick={addScenario}><PlusCircle className="mr-2 h-4 w-4" />Добавить сценарий</Button>
                    </div>
                </div>
            )}
              ...
              <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="space-y-2">
                            ...
                            <Select value={age} onValueChange={setAge}>
                                <SelectTrigger><SelectValue /></SelectTrigger>
                                <SelectContent>{ageSegments.map(s => <SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>)}</SelectContent>
                            </Select>
                        </div>
                        <div className="space-y-2">
                            <Label>Геотаргетинг</Label>
                            <Combobox options={geoSegments} value={geo} onChange={(v) => setGeo(v as string[] || [])} multiple placeholder="Выберите регионы..." />
                        </div>
                    </div>
                </div>
                ...
                <Collapsible>
                    <CollapsibleTrigger asChild>
                        <Button variant="link" className="p-0 text-accent">Расширенный таргетинг</Button>
                    </CollapsibleTrigger>
                    <CollapsibleContent className="space-y-4 pt-4">
                         <div className="p-4 border rounded-lg space-y-4">
                            <div className="flex items-center justify-between">
                                <Label htmlFor="lookalike-audience" className="flex items-center gap-2"><Users className="h-4 w-4"/> Похожая аудитория (Lookalike)</Label>
                                <Switch id="lookalike-audience" checked={isLookalike} onCheckedChange={setIsLookalike} />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                               <div className="space-y-2">
                                    ...
                                    <Select value={rfm} onValueChange={setRfm}>
                                        <SelectTrigger><SelectValue /></SelectTrigger>
                                        <SelectContent>{rfmSegments.map(s => <SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>)}</SelectContent>
                                    </Select>
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <Select value={engagement} onValueChange={setEngagement}>
                                        <SelectTrigger><SelectValue /></SelectTrigger>
                                        <SelectContent>{engagementLevels.map(s => <SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>)}</SelectContent>
                                    </Select>
                                </div>
                            </div>
                            <div className="space-y-2">
                                <Label>История покупок</Label>
                                <Combobox options={productOptions.map(p => ({value: p.label, label: p.label}))} multiple placeholder="Выберите категории..." />
                            </div>
                         </div>
                    </CollapsibleContent>
                </Collapsible>
                ...
                <div className="space-y-2 pt-4 border-t">
                    <div className="flex items-center justify-between">
                        <Label htmlFor="ai-optimisation" className="flex items-center gap-2"><Cpu className="h-5 w-5 text-accent"/> AI-оптимизация бюджета</Label>
                        <Switch id="ai-optimisation" checked={isAiOptimizationEnabled} onCheckedChange={setIsAiOptimizationEnabled} />
                    </div>
                 ...
                 <div className="space-y-4 pt-4 border-t">
                    <div className="flex justify-between items-center">
                         ...
                         <Button type="button" variant="outline" size="sm" onClick={handleGenerateForecast} disabled={isLoading}>
                    ...
                    {aiForecast && (
                         <div className="grid grid-cols-3 gap-3 text-center">
                            <div>
                                ...
                                <p className="font-bold text-sm">{aiForecast.views}</p>
                            </div>
                            <div>
                                <p className="text-sm text-muted-foreground">Клики</p>
                                <p className="font-bold text-sm">{aiForecast.clicks}</p>
                            </div>
                            <div>
                                <p className="text-sm text-muted-foreground">Конверсия</p>
                                <p className="font-bold text-sm">{aiForecast.conversion}</p>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-2 pt-4 border-t">
                    ...
                    <p className="text-xs text-muted-foreground">Загрузите один или несколько вариантов для A/B-тестирования.</p>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleLaunchPromotion} disabled={isLoading || (isActionRequest && (promotionType === 'discount' || promotionType === 'promo_code') && !promoCode)}>
