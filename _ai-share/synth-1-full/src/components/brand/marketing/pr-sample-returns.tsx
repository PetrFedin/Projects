    ...
    return (
        <Card className="rounded-xl border-slate-100 shadow-xl overflow-hidden mt-8">
            <CardHeader className="bg-slate-50/50 flex flex-row items-center justify-between">
                ...
                <Button onClick={handleReportReturn} size="sm" className="rounded-xl bg-slate-900 text-white font-black uppercase text-[10px] tracking-widest">
                    <Plus className="h-4 w-4 mr-1" /> Выдать образец
                </Button>
            </CardHeader>
                    ...
                    <Table>
                        <TableHeader className="bg-white/50">
                            <TableRow>
                                ...
                                <TableHead className="text-[10px] font-black uppercase">Ожидаемый возврат</TableHead>
                                <TableHead className="pr-8 text-right text-[10px] font-black uppercase">Действие</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {samples.length > 0 ? samples.map((sample) => {
                                const isOverdue = new Date(sample.expected_return_date) < new Date() && sample.status === 'out';
                                return (
                                    <TableRow key={sample.id} className="hover:bg-slate-50/50 transition-colors">
                                        <TableCell className="pl-8 py-4 font-bold text-sm">{sample.editorial_name}</TableCell>
                                        <TableCell>
                                            <Badge className={cn(
                                                "text-[8px] font-black uppercase px-2 h-5 border-none",
                                                sample.status === 'out' ? (isOverdue ? "bg-rose-100 text-rose-600" : "bg-indigo-50 text-indigo-600") : "bg-emerald-50 text-emerald-600"
                                            )}>
                                                {sample.status === 'out' ? (isOverdue ? 'Просрочено' : 'Выдано') : 'Возвращено'}
                                            </Badge>
                                        </TableCell>
                                        ...
                                        <TableCell className="pr-8 text-right">
                                            <Button variant="ghost" size="sm" className="h-8 rounded-xl text-[9px] font-black uppercase text-indigo-600">
                                                Вернуть
                                            </Button>
                                        </TableCell>
                                    </TableRow>
