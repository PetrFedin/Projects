...
import { CalendarIcon, ChevronDown, Copy, Trash2, X, Image as ImageIcon, Shirt, Plus, Save } from "lucide-react";
...
import { Dialog, DialogContent, DialogClose, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from "../ui/dialog";
import { Textarea } from "../ui/textarea";

function ColorSizeGrid({ color, colorIndex, control, removeColor, watch, setValue, masterSizes, colorPalette }: { color: any, colorIndex: number, control: any, removeColor: any, watch: any, setValue: any, masterSizes: SizeInfo[], colorPalette: Color[] }) {
    ...
    const handleHexChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const matchingPaletteColor = colorPalette.find(p => p.hex.toLowerCase() === newHex.toLowerCase());
        if (matchingPaletteColor) {
            setValue(`availableColors.${colorIndex}.name`, matchingPaletteColor.name, { shouldDirty: true });
        }
    };
    ...
    const handleCellChange = (size: string, field: 'status' | 'quantity' | 'preOrderDate', value: any) => {
    ...
    return (
        <>
            <Collapsible defaultOpen className="border rounded-lg space-y-4 relative bg-muted/20">
                <div className="p-4 w-full text-left flex items-center justify-between group">
                    <CollapsibleTrigger asChild>
                         <div className="flex items-center gap-2 flex-1 cursor-pointer">
                            <div className="h-6 w-6 rounded-full border flex-shrink-0" style={{ backgroundColor: colorHex }}></div>
                            <p className="font-semibold">{colorName || "Новый цвет"}</p>
                            <ChevronDown className="h-5 w-5 transition-transform duration-300 group-data-[state=open]:-rotate-180" />
                        </div>
                    </CollapsibleTrigger>
                    <div className="flex items-center gap-2">
                        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); removeColor(colorIndex); }}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                    </div>
                </div>
                <CollapsibleContent>
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                ...
                                <Popover open={isPaletteOpen} onOpenChange={setIsPaletteOpen}>
                                    <PopoverTrigger asChild>
                                        <Button variant="outline" className="w-full justify-start font-normal">
                                            <div className="flex items-center gap-2">
                                                <div className="h-4 w-4 rounded-full border" style={{ backgroundColor: colorHex }}></div>
                                    ...
                                    <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                                         <div className="p-2 grid grid-cols-5 gap-1">
                                            {colorPalette.map(pColor => (
                                                ...
                                                >
                                                    <div className="h-6 w-6 rounded-full border" style={{backgroundColor: pColor.hex}}></div>
                                                </Button>
                                            ))}
                                         </div>
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl><Input placeholder="Напр. PANTONE 19-4052" {...field} /></FormControl>
                                    </FormItem>
                                )}
                            />
                             <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl><Input placeholder="Напр. глянцевый" {...field} /></FormControl>
                                    </FormItem>
                                )}
                            />
                        </div>
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-4 lg:grid-cols-6 gap-2 mt-2">
                                {(allImages || []).filter((img: any) => img.colorName === colorName).map((image: any) => (
                                    <div key={image.id} className="relative aspect-square group">
                                        <Image src={image.url} alt={image.alt} fill className="rounded-md object-cover" />
                                ...
                                <Button type="button" variant="outline" className="aspect-square h-full w-full" onClick={() => setIsUploadDialogOpen(true)}>
                                        ...
                                        return (
                                            <TableRow key={sizeInfo.name}>
                                                ...
                                                <TableCell>
                                                    ...
                                                    onValueChange={(value) => handleCellChange(sizeInfo.name, 'status', value)}
                                                    >
                                                    ...
                                                    {sizeData?.status === 'in_stock' && (
                                                        <div className="relative">
                                                            <Input 
                                                                ...
                                                                onChange={(e) => handleCellChange(sizeInfo.name, 'quantity', parseInt(e.target.value, 10) || 0)}
                                                                className={cn("pl-6", getStockIndicatorClass(sizeData?.quantity))}
                                                            />
                                                            <div className={cn("absolute left-2 top-1/2 -translate-y-1/2 h-3 w-3 rounded-full", getStockIndicatorClass(sizeData?.quantity).replace('bg-', 'border-2 border-'))}></div>
                                                        </div>
                                                    )}
                                                    {sizeData?.status === 'pre_order' && (
                                                        <Input type="date" value={sizeData?.preOrderDate || ''} onChange={(e) => handleCellChange(sizeInfo.name, 'preOrderDate', e.target.value)} />
                                                    )}
                                                </TableCell>
                                            </TableRow>
                                        )
            ...
            <UploadImageDialog open={isUploadDialogOpen} onOpenChange={setIsUploadDialogOpen} onSave={handleSaveImage} title={`Загрузить для цвета "${colorName}"`} />
        </>
    );
}
...
export default function ProductVariantManager({ control, watch, setValue }: ProductVariantManagerProps) {
    ...
    const handleApplyTemplate = (sizes: string[]) => {
        setValue('sizes', sizes.map(s => ({name: s})), { shouldDirty: true });
    ...
    return (
      <>
                 ...
                 <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                    {(masterSizes || []).map((size: SizeInfo, index: number) => (
                        <div key={index} className="relative">
                            <Input value={size.name} onChange={(e) => handleSizeNameChange(index, e.target.value)} />
                             <Button type="button" variant="ghost" size="icon" className="absolute top-0 right-0 h-full w-8" onClick={() => handleRemoveSize(index)}>
                                <X className="h-3.5 w-3.5 text-muted-foreground"/>
                            </Button>
                        </div>
                    ))}
                     <Button type="button" variant="outline" className="border-dashed" onClick={handleAddSize}>
                        ...
                        <DialogContent>
                            ...
                            <div className="space-y-2">
                                {defaultSizeTemplates.map(template => (
                                    <Button key={template.value} variant="outline" className="w-full justify-start" onClick={() => handleApplyTemplate(template.sizes)}>
        ...
        <CardFooter>
            <Button type="button" variant="outline" size="sm" onClick={() => append({ id: `new-color-${Date.now()}`, name: 'Новый цвет', hex: '#ffffff', colorCode: '', colorDescription: '', sizeAvailability: [] })}><PlusCircle className="mr-2 h-4 w-4" /> Добавить цвет</Button>
        </CardFooter>
      </>
    );
}
