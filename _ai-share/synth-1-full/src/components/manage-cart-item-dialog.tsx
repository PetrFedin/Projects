...
export function ManageCartItemDialog({ product, cartItem, isOpen, onOpenChange, onUpdate, onRemove }: ManageCartItemDialogProps) {
    ...
    <AlertDialog open={isOpen} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Товар "{product.name}" (размер: <span className="font-semibold">{cartItem.selectedSize}</span>) уже в корзине.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <div className="flex flex-col items-center justify-center gap-2 py-4">
            <div className="flex items-center gap-2">
                ...
                <Input readOnly value={quantity} className="w-20 h-10 text-center text-base font-bold" />
                <Button variant="outline" size="icon" className="h-10 w-10" onClick={() => handleQuantityChange(1)} disabled={maxQuantity !== undefined && quantity >= maxQuantity}>
             ...
             {maxQuantity !== undefined && (
                <p className="text-center text-sm text-muted-foreground mt-2">
                    ...
                    {quantity >= maxQuantity && <span className="text-destructive ml-1">(Больше нет)</span>}
                </p>
            )}
        </div>
