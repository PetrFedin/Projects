...
export function useUserContext(): UserContext {
  ...
  const context = useMemo(() => {
    ...
    const isOrgMember =
      currentOrgAccess?.roleInOrg === 'member' || currentOrgAccess?.roleInOrg === 'co-admin';

    // Business Type
    const isRetailer =
      currentOrg?.type === 'shop' ||
      user.roles?.includes('shop') ||
      user.roles?.includes('b2b') ||
      false;
    ...
    const isManufacturer =
      currentOrg?.type === 'manufacturer' || user.roles?.includes('manufacturer') || false;
