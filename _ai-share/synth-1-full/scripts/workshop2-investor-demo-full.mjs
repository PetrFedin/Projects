...
function curlHttpStatus(baseUrl, pathSuffix) {
  ...
  if ((r.status ?? 1) !== 0) {
    return { ok: false, url, status: 0, passRu: 'FAIL', errorRu: `Недоступен: ${url}` };
  }
...
function curlJson(baseUrl, pathSuffix, opts = {}) {
  const retries = Number(opts.retries ?? process.env.WORKSHOP2_INVESTOR_CURL_RETRIES ?? 4);
  const url = `${baseUrl}${pathSuffix}`;
  const maxTime = String(process.env.WORKSHOP2_INVESTOR_CURL_MAX_SEC ?? '60');
  let lastErr;
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    ...
    if ((r.status ?? 1) !== 0) {
      lastErr = {
        ...
        passRu: 'FAIL',
        attempt,
        errorRu: `Сервер недоступен (попытка ${attempt}/${retries}): ${url}. ${devServerHintRu()} (порт ${process.env.PLAYWRIGHT_E2E_PORT || 3123}).`,
      };
  ...
  return lastErr ?? { ok: false, url, passRu: 'FAIL', errorRu: `curl failed: ${url}` };
}

function curlPostJson(baseUrl, pathSuffix, bodyObj, opts = {}) {
  const retries = Number(opts.retries ?? process.env.WORKSHOP2_INVESTOR_CURL_RETRIES ?? 3);
  const url = `${baseUrl}${pathSuffix}`;
  const maxTime = String(process.env.WORKSHOP2_INVESTOR_CURL_MAX_SEC ?? '60');
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    ...
    if ((r.status ?? 1) !== 0) {
      lastErr = {
        ...
        passRu: 'FAIL',
        attempt,
        errorRu: `POST недоступен (попытка ${attempt}/${retries}): ${url}`,
      };
      if (attempt < retries) continue;
      return lastErr;
    }
    try {
      ...
      lastErr = { ok: false, url, passRu: 'FAIL', attempt, errorRu: `POST не JSON: ${url}` };
      if (attempt < retries) continue;
      return lastErr;
    }
  }
  return lastErr ?? { ok: false, url, passRu: 'FAIL', errorRu: `POST failed: ${url}` };
}
...
if (envCheckHit.ok) {
  serverDemoMode =
    envCheckHit.json.demoModeComputed === true ||
    envCheckHit.json.demoMode === true;
  steps.push({
...
const checkoutHttp = curlHttpStatus(probeBase, '/shop/b2b/checkout');
steps.push({
  id: 'b2b-checkout-http',
  labelRu: 'B2B checkout page HTTP 200/307',
  ...checkoutHttp,
  noteRu: checkoutHttp.status === 307 ? '307 redirect — OK для wholesale checkout entry' : undefined,
});
if (!checkoutHttp.ok) ok = false;

const publishReady = curlPostJson(probeBase, '/api/workshop2/collections/SS27/publish-showroom-readiness', {
  articleIds: ['demo-ss27-01'],
});
steps.push(
  publishReady.ok
    ? {
        id: 'publish-showroom-readiness',
        labelRu: 'Publish showroom gate (demo-ss27-01)',
        ok: publishReady.json.ready === true,
        passRu: publishReady.json.ready === true ? 'PASS' : 'FAIL',
        ready: publishReady.json.ready,
        messageRu: publishReady.json.messageRu,
        errorRu:
          publishReady.json.ready === true
            ? undefined
            : publishReady.json.messageRu ?? 'publish gate blocked — npm run workshop2:investor-prep (SS27 seed)',
      }
    : {
        id: 'publish-showroom-readiness',
        labelRu: 'Publish showroom gate (demo-ss27-01)',
        ok: false,
        passRu: 'FAIL',
        errorRu: publishReady.errorRu,
      }
);
if (!publishReady.ok || publishReady.json.ready !== true) ok = false;

const perfBudget = curlJson(probeBase, '/api/workshop2/performance-budget');
...
let probes = null;
const probeHit = curlJson(probeBase, '/api/workshop2/integration-probes', { retries: 5 });
if (probeHit.ok) {
...
const investorStatusHit = curlJson(probeBase, '/api/workshop2/investor-demo/status', { retries: 5 });
...
const briefHit = curlJson(probeBase, '/api/workshop2/investor-demo/brief', { retries: 5 });
if (briefHit.ok) {
  const demoMode =
    briefHit.json.demoMode === true || briefHit.json.humanSignoff?.demoMode === true;
  ...
  steps.push({
    ...
    presentationTipsCount: briefHit.json.presentationTipsRu?.length ?? 0,
    qaDocPath: briefHit.json.qaDocPath,
