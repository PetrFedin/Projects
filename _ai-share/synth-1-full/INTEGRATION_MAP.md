...
Смок **не заменяет** долгосрочную системную модель: границы агрегатов Order / Article / Inventory / Availability / Control — в **`docs/B2B_AND_PRODUCTION_CORE_SPEC.md`**, **`docs/domain-model/event-matrix.md`**, **`TASK_QUEUE.md`** (и в `docs/domain-model/*` по мере расширения); чеклисты границ — `docs/*-boundaries-checklist.md`, если добавлены. Политика монорепо / канон full: **`Projects/docs/MIGRATION_FULL_CUTOVER.md`**, **`docs/CANONICAL_FULL.md`**. Субмодуль **`synth-1/`** из монорепо **удалён**; корневой **`Projects/src/`** (если присутствует) не канон.
