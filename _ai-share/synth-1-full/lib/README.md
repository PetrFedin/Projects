...
- Live-probe контракта health: без **`DOMAIN_EVENTS_HEALTH_URL`** скрипт **`check:domain-events-health-contract`** **пропускается** (exit `0`); в CI задайте URL и при необходимости **`DOMAIN_EVENTS_HEALTH_CONTRACT_STRICT=1`**, чтобы отсутствие URL было ошибкой. Агрегаторы **`npm run check:contracts`** и **`npm run check:contracts:ci`** (CI / **`smoke:fast`**) вызывают и этот шаг.
