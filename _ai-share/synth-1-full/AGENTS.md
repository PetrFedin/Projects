...
| --------------------------------------- | --------------------------- | ------------------------------------------------------------------ |
| **`_ai-share/synth-1-full`**            | Канон, prod и demo          | Остаётся единственным основным деревом                             |
| **`synth-1/`** (корень репозитория)     | Замороженный донор / legacy | Отключить от разработки и деплоя; убрать после выравнивания с full |
| **`Projects/src/`** (если присутствует) | Legacy-дубликаты            | То же                                                              |

Политика и критерии «только full»: **`docs/CANONICAL_FULL.md`**, **`SOURCE_OF_TRUTH.md`**. Новые PR и продуктовый код — только в **full**; в доноры не мержим фичи.

### Порядок: перенос → улучшение в full → связи → затем отключение доноров

1. **Перенос из `synth-1/` / legacy** — не слепой `cp`: diff, встраивание в паттерны full (`routes.ts`, навигация, `entity-links`, RBAC, BFF/API).
2. **Улучшение** — доводим UX, типы, контракты и наблюдаемость в full; донор не считается источником истины по API.
3. **Связи** — перекрёстные ссылки между кабинетами, `data-testid`, смоки/матрица **`UNIFIED_ECOSYSTEM_VERIFICATION.md`**.
4. **Заморозка и отключение** донора — только после того, как соответствующий срез перенесён и принят по чеклисту; до этого донор остаётся для сравнения diff.
...
- Дорожная карта Q1 / strict CI / монорепо: `docs/roadmap/ENTERPRISE-Q1.md`, `docs/roadmap/STRICT_CI.md`, `docs/MONOREPO_INTEGRATION.md` — жёсткий контур: **`npm run typecheck:order-subset`** / **`lint:order-subset`** (`tsconfig.strict-order.json` + `package.json`; сейчас B2B operational/v1, shop cart/orders/session, **`b2c-shop-request`** / **`shop-inventory-stock-upload-client`**, inventory **`GET|POST …/stock-upload`** (+ audit types/client), processes, realtime poll, auth guard, **`env-safety-warnings`**). **Прод = только full**; каталоги **`synth-1/`**, корневой **`Projects/src/`** не удалять, пока не зафиксирован запуск только на full и не пройдены фазы D/E из **`MONOREPO_INTEGRATION.md`** (конечное состояние — **`docs/CANONICAL_FULL.md`**).
