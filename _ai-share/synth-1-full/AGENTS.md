...
**Platform Core (`:3001`, 5 столпов × 4 роли):** **`NEXT_PUBLIC_PLATFORM_CORE_MODE=1`** — hub **`/platform`** (investor walkthrough на hub), кабинеты **`/brand|shop|factory/*/core`**, demo-коллекции **SS27**, **FW27**, контраст **EMPTY27** (`/platform?collection=`). Dev: **`npm run dev:core`**. PG: **`docker compose -f docker-compose.workshop2.yml up -d`** (порт **5433**), bootstrap **`npm run db:core:bootstrap`** (PO в очереди) или **`npm run db:core:bootstrap:interactive`** (handoff через UI бренда). Verify: **`npm run core:verify`** / **`npm run core:verify:interactive`**, **`npm run smoke:core`**. **Responsive UI (кабинет + workspace):** **`docs/platform-core-cabinet-responsive.md`** — breakpoints 393 / 834 / 1280, e2e **`core-92`**. **UI-дедуп (обязательно перед PR в platform/core):** **`npm run audit:platform-core-ui`** — канон **`src/lib/platform-core-ui-surfaces.ts`**, правило **`../../.cursor/rules/platform-core-ui-dedup.mdc`**. Без PG hub показывает bootstrap-баннер — статическая матрица, без «живой цепочки». Staging: **`env.core.staging.example`**.
