...
**Investor demo (Workshop2 Wave 58):** runbook — **`.planning/INVESTOR-DEMO-RUNBOOK-RU.md`**; сценарий 25 мин — **`.planning/INVESTOR-DEMO-SCRIPT-RU.md`**. Перед показом: **`npm run workshop2:investor-prep`** (merge `.env.e2e.investor.example` → `.env.local`, `dev:e2e:investor` на :3123). Проверка: **`GET /api/workshop2/investor-demo/env-check`** → `demoModeComputed: true`. Полный gate-runner: **`npm run workshop2:investor-demo:full`** (локально; в **`smoke:fast`** только статический **`check:investor-demo-contract`**, без live Next). Env merge: **`scripts/merge-investor-env-local.mjs`** (тест: **`merge-investor-env-local.test.ts`**). Не параллелить **`dev:fast`** (:3000) и **`dev:e2e:investor`** (:3123).
