...
### B2. Расширить CI
- **Ruff** — в CI для **`scripts/*.py`** (инкрементально; полный `app/` — отдельная фаза).
...
### C2. Фронтенд-монорепо (актуально)
- Субмодуль **`synth-1/`** из корня **удалён**; канон — только **`_ai-share/synth-1-full`**. Исторический репозиторий при необходимости — вне этого дерева (см. **`docs/MIGRATION_FULL_CUTOVER.md`**).
